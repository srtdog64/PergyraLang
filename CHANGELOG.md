...
- **docs**: Add `review/object_vs_tobject_semantics.md` — semantic distinction
  between object (internal view) and tobject (transfer TObject).
...
- Split `examples/shopping_mall_checkout_refund/` into explicit `api/` and
  ...
  `request tobject -> adapter handler -> intent -> zone/world -> response/report tobject`
...
- Ontology-first scaffold guidance in `pgy scaffold` / `pgy new`, including a
  ...
  from the `subject/class/object/tobject` split instead of only dumping a raw world
  shell
...
- Nested vessel-backed projection paths in `ToObject` / `ToTObject` and domain `refresh` / `publish`
...
- `subject`, `class`, `struct`, `object`, and `tobject` declarations now preserve distinct nominal flavors in the parser AST
- `subject slot` and `ToObject` / `ToTObject` source validation now require `subject` declarations instead of accepting bare `class`
...
- Added `tobject` keyword as a struct-compatible projection/declaration alias
- Added `ToObject(TargetObject, subjectBinding)` as a minimal subject-to-object projection surface
- Added `ToTObject(TargetDto, subjectBinding)` as a minimal subject-to-tobject projection surface
- Added `tobject slot` surface to relation/effect/zone domain bodies
- Added LLVM lowering parity for `ToObject` / `ToTObject` subject projection built-ins
...
- Added `publish dtoSlot from subjectSlot` as an explicit zone tobject-projection surface
...
- Added `HasProjection(slotName)` as a relation/effect/zone projection-query builtin for declared `object slot` / `tobject slot` names
...
- Domain-local `refresh` / `publish` now accept object sources as well as subject sources, while still rejecting tobject sources
...
- `object` / `tobject` declarations now allow passive helper methods again
...
- `ToObject` now accepts only `object` declarations, `ToTObject` only `tobject` declarations, and `refresh` / `publish` follow the same nominal projection split
- Direct `ToObject` / `ToTObject` outside relation/effect/zone/world context now emit semantic warnings so domain-local projection flow is the preferred path
...
- Added `pgy scaffold` and `pgy new` CLI support for generating starter `subject`, `vessel`, `object`, `tobject`, `zone`, `world`, `simulator`, and `project` templates
...
- Vessel/action docs now match implementation more closely: `subject` uses `action` only, while `object` / `tobject` keep passive helper `func`
- Core docs now define `object` as a passive state target that can hold state and react without initiating intent
- Core ontology docs now define `tobject` as the compact external-boundary projection of an object representation
...
- Ontology/projection docs now emphasize `subject -> relation/effect/zone/world context -> object/tobject projection` instead of treating `tobject` as a peer ontology axis
