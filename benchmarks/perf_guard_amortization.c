 ...
 * checks on every access. The preflight path validates the same facts once and
 * then consumes an evidence view in the hot loop.
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
main(int argc, char **argv)
{
    ...
    printf("invalid=%lld\n", (long long)run_invalid_preflight(&slot));
    return per_access == preflight ? 0 : 1;
}
