...
`.tmp/` 안의 9월 이전 실행에 남은 생성 파일 1,715개(16.332 GiB)를 삭제하고,
여유 공간 20.001 GiB에서 멈췄다. 소스·문서·로그·검증 기록과 현재 bootstrap,
설치 바이너리·진행 중 반례는 제외했다. 핵심 파일 14개의 전후 SHA-256도 모두
일치했다(`79e84b`). 영구 삭제 목록과 결과는 `.tmp/storage_cleanup_20260906/`에
남겼다. 저장 공간 결정 때문에 멈췄던 상태는 해소됐으며, C: 이동이나 IO 정책
변경은 없었다. 이 정리는 컴파일러 수정·재빌드·SoT 대체 진척으로 세지 않는다.

별도 작업이 실행형 단어 삭제 매트릭스를 `16b2f894`로 커밋·푸시했다.
35개 실험·104개 프로그램과 양 파이프라인 결과 각 104개, 결과 차이 34개를
확인했다. primary는 enum match·capability bound·subject 인자 실험의 11개
프로그램을 양쪽에서 재실행했고, 22개 결과가 기존 기록과 일치했다(`6bf836`).
따라서 이 범위의 픽스처·런타임·양 파이프라인 기록은 더 이상 빈칸이 아니다.
다만 러너는 기대 결과를 판정하는 gate가 아닌 수집기이며, 전체 단어/문맥 동치,
키워드 삭제 또는 self-host 치환 진척은 아니다. 상세 범위는 인테이크의 후속
실행 절과 `docs/audits/2026-09-06_language_word_deletion_execution_matrix.md`에 있다.
...
남아 있어 수정 없이 긴 Platform full을 다시 요청하지 않았다.
...
기존 CI가 끝난 뒤 최종 profile/docs/syntax/UTF-8/공백 검사를 통과했고, 검토한
6개 파일만 `5653165b7b372a84622ee5837e9375c0828fecd6`으로 커밋·푸시했다.
새 일반 CI `33984709173`는 29/30 통과 후 FAILURE로 끝났고 macOS job `101356033822`는 8/9단계
통과 후 실패했다. 크기 검사기는 실제 macOS에서 통과했지만 컴포넌트 검사의
반례 생성용 GNU `sed` 문장이 BSD sed에서 거절됐다. 동일한 5줄 입력을 만드는
AWK 문장으로 바꿨고, 바이트 동일성·실제 검사 전체·profile/docs/syntax를 로컬에서
통과했다(`82951`). 독립 읽기 전용 리뷰도 정확한 거절 조건 보존을 확인했다.
Linux 23단계와 전체 driver 고정점 177,559줄, 설치 후 CLI/transaction 검사 및
정책 소스 3개도 통과했다. 남은 macOS 수정은 최종 검사(`35121`) 뒤
`91119c45683c10efdccaad18c95fdc2a9ebf0d41`으로 커밋·푸시했다.
새 CI `33987047080`에서 macOS job `101362486942`는 실제 검사기 둘과
전체 9단계를 통과했다. 해당 일반 CI는 이후 30/30 SUCCESS로 종료됐다.
CI 통과 수는 self-host 대체 진척이 아니다.

별도 대화의 전수 삭제 실험 인계는
`docs/audits/2026-09-06_language_word_deletion_intake.md`에 보존했다.
146개 단어/문법 문맥과 기존 KEEP-CORE도 재검토 대상이다. 기존 타입·함수로
구현한 라이브러리 대체물을 단순 재도입으로 기각하던 기준을 정정했고, 기존 소스
6개의 native MIR/admission만 재확인했다. side의 메모리 실험 전수, runtime 및
self-host 동치 완료는 아니다. 이 인계와 출판 상태 갱신은 로컬 문서 변경이다.
...
거절했다. 두 건 모두 수정 완료가 아니며 원본 MIR는 보존했다. 큰 로컬 빌드를
막던 저장 공간 문제는 위의 제한된 임시 산출물 정리로 해소했다. 기존 절대경로
출력 정책은 바꾸지 않았으며, 공간 확보 후 새 컴파일러 빌드는 아직 실행하지 않았다.
...
유지한다. 해당 실행의 현재 C/MIR 증거는 보존했다. 위 정리는 오래된 생성물만
대상으로 했고, 저장소 루트의 사용자 임시 소스는 삭제·출판 대상으로 삼지 않았다.
...
변경 소스로 만든 DRV-2에서 source-C, direct C, direct LLVM은 모두 정확히
...
green이다. 구현 `62e4b0c5`를 publish했고 exact-head CI가 위 fixed-point와
corrected corpus census를 확인했다.
...
source graph, native compiler, C toolchain, runtime headers, compile profile, owner
scripts를 하나의 seed prebuild key로 소유하고 gen2 C/binary를 별도 exact output
receipt로 묶는다. 최초 기준 생성은 307.2초였지만 두 번째 seed-only 호출은 oracle
build 전에 20.9초로 종료했다. 첫 DRV-2 갱신은 304.3초였고 같은
...
`selfhost.source_format_layout` 행은 local `CLOSED`다. Formatter 완료 당시 census는
