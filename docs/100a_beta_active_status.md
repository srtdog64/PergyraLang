...
Passive nominal literal update (2026-08-02): executable checkpoint `f5eedd97`
closes `nominal_tobject.pgy` as a bounded target-specific `SUBSTITUTING` slice.
One 2,857-byte self MIR drives real C named construction/field read and LLVM
`insertvalue -> extractvalue`; both installed public paths execute exact 12.
The route is single-shot and claims malformed nominal envelopes before scalar
routing. A sealed instruction-capture digest proves typed physical-ABI absence,
while target projection admits only capability/representation and each emitter
owns its syntax. Four variants, 28 C negatives, nine LLVM sentinels, the
class/vessel 41/42 regression, hard contract and installed C/LLVM gates are
green. No fixture/type-name dispatch, value constant-fold, scalar retry or
native fallback was added. The next executable falsifier is
`nominal_subject.pgy`: its 2,791-byte self MIR reaches the route but both targets
reject it at passive nominal program identity. The next delta must preserve
