...
### 모델링 경계: 생명주기가 아니라 행위 계약

`ability`는 subject의 완전한 생명주기나 세계의 닫힌 상태 기계를
소유하지 않는다. 현실의 subject는 외부 개입, 관찰되지 않은 변화, 권한 이동,
고장, 동시 사건 때문에 고정된 `created -> active -> destroyed` 서사로 완전히
기술할 수 없기 때문이다. ability가 보장하는 것은 현재 확보된 자원, 권한,
관찰 fact 아래에서 어떤 행위를 시도할 수 있고 그 호출이 어떤 결과 계약을
갖는지까지다.

이 때문에 현재 표면은 더 작은 세 문법으로 기계적으로 분해하지 않는다.

- trait처럼 보이지만 모든 타입에 붙는 일반 기능 집합이 아니다.
- 상속처럼 보이지만 `is-a`, 필드·상태 계승, 생명주기 전파가 없다.
- composition처럼 보이지만 값이나 객체를 조립하는 대신 subject에 행위 계약과
  필요한 자원·자격을 결합한다.
- role은 그 결합의 구체적인 구현을 소유하고, intent는 현실 목적에서 필요한
  ability를 요구한다. action/effect는 실제 개입과 관찰된 결과를 별도로 소유한다.

따라서 현재의 가장 좁은 정의는 **subject에 결합되는 행위 계약**이다. 계약,
권한, 구현이 실제로 서로 다른 수명과 이동·실패 경계를 갖는다는 실행 증거가
생기기 전에는 별도 capability/trait/lifecycle 문법으로 다시 쪼개지 않는다.
