...
- 2026-07-15 MIR type/declaration owner split: source provenance/location/order
  ...
  layout lookup API owned by `mir_abi_layout.h`. MIR representation facts now
  live in `mir_types.h`, declaration metadata in `mir_decl.h`, and the lowered
  program aggregate in `mir_program.h`; `mir.h` is the public operation API and
  umbrella. Each named owner is independently ratcheted below 600 LOC by
...
regressions. At 661 LOC it remains below the test-fragment cap; merging it into
an unrelated MIR suite would hide the ABI owner boundary merely to reduce the
fragment count.

The 143rd fragment is
`src/tests/mir/test_mir_lowering_part_b_3.cases.h`. It owns the typed intent
transition-plan execution and crosswired-plan rejection regressions. Keeping
