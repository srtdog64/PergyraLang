...
| --- | --- | --- | --- | --- |
| Program entry | `main`, `static void Main`, `fn main` | `func Main()` and lowercase `func main()` driver entry paths | `partial` | `entry_lowercase_main` gates native C/LLVM, installed self-host MIR/source-C, and the bounded one-routine/one-block/one-instruction literal-Log direct/public LLVM route with exact runtime output. Richer return/args policy and other direct-MIR projections remain partial. |
...
| Passive utility class | `class` | `class` | `stable` | Avoid using `class` for domain authority; class-vs-subject method dispatch parity is gated by `subject_class_dispatch`. |
| Active domain aggregate | C# class + DDD aggregate root | `subject` | `native-different` | Pergyra splits active domain identity from passive classes; subject projection parity is gated by `subject_projection`. |
| Internal state container | private state holder | `vessel` | `partial` | Keep out of broad claims until examples are closed. |
| Read projection DTO | DTO/read model | `object` | `stable` | Projection target surface; subject-to-object parity is gated by `subject_projection`, and object layer binding by `object_layer_binding`. |
| Transfer DTO | transport DTO / serialized shape | `tobject` | `stable` | Transport-boundary shape; subject-to-tobject parity is gated by `subject_projection`. |
| Enum / ADT | `enum`, Rust enum, tagged union | `enum` | `stable` | Frozen subset only; tagged payload member access parity is gated by `tagged_union`. |
...
| Constructor / initializer | `new T(...)`, object initializer | declaration-specific constructors and calls; `Type { field: value }` parses as named-argument construction sugar | `partial` | Keep construction ordinary-call shaped; object-initializer dispatch remains tied to named-argument policy. |
...
| Lambda / closure literal | `x => x + 1`, `|x| x+1` | `=>` lambda AST/parser surface | `partial` | Standalone lambda callables are active and expression-lambda parity is gated by `lambda_expr`; block-return lambda parity is gated by `lambda_block_return`. Lambda-local `let`/destructure/for bindings are not captures; callable-let lambdas can copy-capture value-type locals through the Stage A closure environment ABI, while escaping lambdas and resource/authority captures still reject until boundary lifetime facts are frozen. |
| Function reference | method group / function pointer | function name as value | `partial` | Enough for narrow callbacks. |
| Higher-order function | `Func<T,R>`, `Fn` | callable parameters | `partial` | No HKT/functor hierarchy. Direct generic call parity is gated by `generic_call`; generic spawn/await monomorphization parity is gated by `generic_future_spawn_int`, `generic_future_spawn_multi_arg`, `generic_future_spawn_string`, and same-unit mixed specialization in `generic_future_spawn_mixed`. |
...
| Match expression | Rust `match`, C# switch expression | `match` | `stable` | Simple scalar return match parity is gated by `match_stmt`; guarded/or-pattern parity by `match_guard_or_pattern`. |
...
| Pattern | Common shape | Pergyra mapping | Status | Notes |
| --- | --- | --- | --- | --- |
| Integer / long / float | `1`, `1L`, `1.0` | numeric literals | `stable` | Backend parity required; mixed Int/Bool/String return logging is gated by `multi_types`. |
...
| Scalar utility builtins | `abs`, `min`, `max`, `clamp`, constants | `Abs`, `Min`, `Max`, `Clamp`, `PI`, `E` | `stable` | Integer utility lowering and float constants are C/LLVM parity-gated by `scalar_utility_builtins`. |
...
| Slicing | `xs[a..b]`, `xs[..]` | `xs[a..b]`, `xs[a..]`, and `xs[..]` desugar to `.Slice(start, len)`; `.Slice(start, len)` and `SliceCopy(view)` remain explicit forms | `partial` | Range slicing is active for borrowed views; owned snapshots still go through `SliceCopy(view)`. |
...
| Set API | membership collection | `SetNew`, `SetAdd`, `SetHas`, `SetRemove`, `SetSize`, `SetValues` | `stable` | Int membership/add/remove/size paths are C/LLVM parity-gated by `set_membership_ops` and `set_ops`; stable scalar snapshots are gated by `stage4_determinism_smoke`. |
...
| Map/set literal | dict/hashmap/set literal | bare `{ key: value }` parses as `AST_MAP_LITERAL`; bare `{ value }` and `{}` parse as `AST_SET_LITERAL`; `{:}` is the empty map marker | `partial` | Set literal C/LLVM parity is gated by `set_literal_basic`; broader list literal sugar remains out-of-beta. |
...
| Pattern | Common shape | Pergyra mapping | Status | Notes |
| --- | --- | --- | --- | --- |
| C FFI | `extern "C"` / PInvoke | `extern` / ABI spec | `partial` | ABI non-leakage contract must remain enforced. Minimal extern-block parse/codegen parity is gated by `extern_fn`. |
...
| `pin` / `ReadView<T>` / `WriteView<T>` | Scoped typed view and amortized access. | Rust borrow/pin ideas, but no lifetime annotation model; secure read/write view parity is gated by `secure_slot_view` plus the pin-specific fixtures. |
...
| Element access | Arrays, lists, strings, maps all need indexed/keyed access. | `xs[i]` array/slice access is active; `xs[a..b]`, `xs[a..]`, `xs[..]`, and `.Slice(start, len)` create borrowed local views. | Keep index diagnostics stable; keep `SliceCopy(view)` as the owned snapshot escape hatch; reserve keyed map indexing after collection ABI closure. |
| String formatting/interpolation | Logs, diagnostics, CLI, examples all need it. | string literal + active interpolation for simple fragments | Keep backend parity and escaping diagnostics stable before adding format specifiers. |
...
| String interpolation | Every log/output example wants it. | Parser surface and C/LLVM parity exist for simple fragments. | Beta-adjacent; nested interpolation/format specifiers remain post-beta. |
...
| Range slicing and keyed indexing | Data-heavy code quickly wants `xs[a..b]`, `xs[..]`, and map-like `m[k]`. | Keep `xs[i]`, `xs[a..b]`, `xs[a..]`, `xs[..]`, `.Slice(start, len)`, and `SliceCopy(view)` stable; design keyed indexing after collection ABI and Option failure policy. | Range slicing is beta-adjacent; keyed indexing remains post-beta. |
...
| Closure literals | Callbacks, UI, async, streams. | Stage A landed (docs/135): value-type **copy-capture** works in a callable-let initializer with C/LLVM parity. Escaping captures (event handler/arg/return/spawn) and ref/own/resource capture still reject. | Stage A beta-adjacent; ref/own/escaping capture remain post-beta (Stages B-D). |
...
| Object initializer / builder sugar | Industrial APIs need ergonomic construction. | `Type { field: value }` parses as named-argument construction sugar; use constructors/factory functions for stable beta examples. | Parser-active, semantic policy partial; avoid broad partial-init claims. |
...
| String interpolation | Active lexer/parser surface exists: normal strings with `${...}` and `f"..."` route to interpolation parsing; escaped `\${` / `\{` openers and malformed fragments remain literal. | Keep basic C/LLVM output parity gated; defer nested interpolation and format specifiers. |
| Collection literals | Array literals and bare `{ key: value }` map literals exist; `List`/`Set`/`HashMap` lowering is not frozen. | Reserve typed collection literal claims until collection ABI/key policy is stable. |
| Closure literal | Active AST surface exists as `AST_LAMBDA_EXPR`; parser recognizes lambda starts and `=>`. | Current beta contract: callable-let lambdas can copy-capture value-type locals; lambda-local bindings stay local; escaping lambdas and resource/authority captures still reject with borrow-escape diagnostics. |
...
| Member/index access | `x.y`, `x.f()`, `xs[i]`, and range slicing are active expression surfaces. | Keep member/index/slice lowering ordinary and stable; add keyed indexing only after collection ABI policy is fixed. |
...
| Object initializer syntax | `Type { field: value }` parses as named-argument construction sugar. | Prefer constructors/factory functions in stable examples until named-argument dispatch and partial-construction ownership are frozen. |
| Range slicing/spread/rest | `xs[a..b]`, `xs[a..]`, and `xs[..]` are active and lower to borrowed `Slice<T>`; `...` spread/rest is reserved with explicit parser diagnostics. | Keep range slicing on the existing slice ABI; keep spread/rest out of beta until ownership and call ABI are closed. |
...
| Block expression policy | Blocks are statements; expression-position `{ key: value }` is a map literal, not a statement block. | Keep statement blocks statement-only; expression blocks require CFG/type-inference/cleanup ownership before any future promotion. |
