...
| Element access | Arrays, lists, strings, maps all need indexed/keyed access. | `xs[i]` array/slice access is active; `xs[a..b]` / `.Slice(start, len)` create borrowed local views. | Keep index diagnostics stable; keep `SliceCopy(view)` as the owned snapshot escape hatch; reserve open-ended slicing/keyed map indexing after collection ABI closure. |
...
| Range slicing and keyed indexing | Data-heavy code quickly wants `xs[a..b]`, `xs[..]`, and map-like `m[k]`. | Keep `xs[i]`, `xs[a..b]`, `.Slice(start, len)`, and `SliceCopy(view)` stable; design open-ended slicing/keyed indexing after collection ABI and Option failure policy. | `xs[a..b]` is beta-adjacent; open-ended slicing and keyed indexing remain post-beta. |
