...
| Map/set literal | dict/hashmap/set literal | bare `{ key: value }` parses as `AST_MAP_LITERAL`; list/set/hashmap lowering is still not frozen | `partial` | Keep parser acceptance separate from collection ABI/key-policy claims. |
