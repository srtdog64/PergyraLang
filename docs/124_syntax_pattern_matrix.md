...
| Slicing | `xs[a..b]`, `xs[..]` | `.Slice(start, len)` plus `SliceCopy(view)` | `partial` | Method-form borrowed slices are stable for beta; range-slicing syntax stays reserved/rejected until the public grammar/policy is frozen. |
...
| Element access | Arrays, lists, strings, maps all need indexed/keyed access. | `xs[i]` array/slice access is active; `.Slice(start, len)` creates borrowed local views; richer range-slicing/keyed indexing is not frozen. | Keep index diagnostics stable; keep `SliceCopy(view)` as the owned snapshot escape hatch; reserve range-slicing/keyed policies after collection ABI closure. |
...
| Range slicing and keyed indexing | Data-heavy code quickly wants `xs[a..b]`, `xs[..]`, and map-like `m[k]`. | Keep `xs[i]`, `.Slice(start, len)`, and `SliceCopy(view)` stable; design range-slicing/keyed indexing after collection ABI and Option failure policy. | Post-beta unless dogfood proves blocker. |
...
| Range slicing/spread/rest | Range slice syntax and `...` spread/rest are reserved with explicit parser diagnostics; no public `xs[a..b]`, spread argument, or rest destructure baseline. | Keep range syntax/spread/rest out of beta; method-form `.Slice(start, len)` and `SliceCopy(view)` remain the stable borrowed-view path. |
