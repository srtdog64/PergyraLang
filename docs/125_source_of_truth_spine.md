...
| Source/provenance payload | Treating `source_ast` as semantic inventory truth | `mir_decl_header_source_decl(...)` and `mir_routine_source_decl_of_type(...)` are the remaining compiler-owned compatibility boundaries; `mir_decl_method_source_ast(...)`, `mir_decl_field_source_ast(...)`, and backend `*_source_ast` compatibility accessors are retired | Smoke rejects generic fallback naming, old AST method-array state, backend raw `decl_header->source_ast` / `method->source_ast` reads, backend raw routine `ast` reads, backend `*_source_ast` routine wrappers, the retired method/field source accessors, the retired LLVM method body AST compatibility accessor, synthetic-executable AST-returning function lookup, C hosted-method body emission that recovers method source AST instead of passing the linked MIRRoutine directly, C class/zone specialization scans that recover method source AST instead of linked MIR routine facts, and LLVM MIR body source-decl recovery; ast_read_surface ratchets source_decl codegen 2 / compiler 1 and routine_source_decl_codegen 2 | Backend `source_ast` frontier closed; C/LLVM hosted method body emission is MIR-routine-only; C class/zone collection-specialization scans are MIR-routine/source-local-fact based; LLVM generic function templates register and specialize through MIRRoutine entries; LLVM MIR body emission consumes routine kind/signature/current-routine metadata without recovering source declarations; declaration-header source_decl and remaining routine source-decl boundaries remain until final Phase 2 field removal |
