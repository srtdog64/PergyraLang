...
| Type-alias target metadata | C/LLVM alias handling reopened `ast_type_alias_target_type(...)` after declaration lookup | `MIRDeclHeader.type_alias_target_type_name`, captured by `mir_decl_headers.c` and validated by `mir_decl_header_validate.c` | `mir-declaration-inventory-test-smoke` requires the header field, accessor, lifecycle cleanup, validator drift diagnostic, C alias emitter consumption, LLVM alias type-map consumption, LLVM render-context consumption, and MIR-active fallback guards | Closed for C type-alias declaration emission and LLVM alias type mapping/rendering in MIR-active paths; non-MIR AST compatibility fallback remains explicit |
