...
- ABI layout facts live in `src/runtime/pgy_abi_spec.h`, with compile-time
  ...
  source-level slot builtins (`Read`/`Write`/`Release`), expression-dispatch
  slot assignment/auto-read, and block auto-release cleanup also consume
  ...
  not allowed to synthesize `pgy_read_*`, `pgy_write_*`, `pgy_release_*`, or
