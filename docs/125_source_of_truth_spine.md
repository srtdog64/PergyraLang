...
| Zone refresh metadata | Projection lowering reopened `AST_ZONE_REFRESH` to read object/source slot names and explicit target-to-source field maps | `MIRDeclZoneRefresh` rows populated by `mir_decl_header_refresh.c` and validated by `mir_decl_header_validate.c`; C zone declaration emission consumes them through `TranspilerHostedZoneRefreshView` and the MIR refresh literal entry point | `mir-declaration-inventory-test-smoke` requires refresh metadata storage, accessors, validation drift checks, MIR tests that preserve field-map rows and reject metadata-count drift, the C zone refresh view, and rejects `ast_zone_refreshes(node, &refresh_count)` in `transpiler_zone_decl_emit.c` | Metadata capture plus C zone projection sync consumer cutover closed; remaining relation/effect and LLVM projection refresh consumers still need MIR refresh rows/views before their AST refresh accessors can be retired |
