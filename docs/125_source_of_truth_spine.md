...
| Dedicated declaration IR | `MIRProgram` still carries AST-shaped declaration payloads and some backend consumers still use compatibility field views | `MIRDeclHeader` / `MIRDeclMethod` / `MIRDeclField` / `MIRDeclRoleInclude` / `MIRDeclZoneAuthority` / `MIRDeclZoneRefresh` / `MIRDeclZoneState` metadata model, consumed through compiler accessors | Partial gate exists for methods, fields, role includes, zone authority, refresh, and zone-state rows; full closure requires projection/declaration emitters to stop reopening compatibility views when MIR facts exist | Closing (MIR-only decision, 2026-06-23). Production codegen hard-fails without MIR (`driver_app.c:334`); the non-MIR fallback was load-bearing only for in-process codegen unit tests. **Field-shape fallback family closed**: `ast_compat_decl` (C+LLVM class/shared field hosted views) fully retired — accessors fail closed when MIR metadata is absent, the dead struct field is removed, the synthetic-AST isolation test is rewritten through the real parse->lower->emit pipeline, and `mir-declaration-inventory-test-smoke` forbids `ast_compat_decl` reintroduction. **Method-shape fallback family closed**: `ast_compat_methods` (C+LLVM hosted method views) retired — the compat-method accessors fail closed, the struct field/constructor parameter are removed, and the non-MIR consumers (`find_nominal_host_method_decl` etc., all guarded by `if (transpiler_active_has_mir) return` so dead in production) now no-op. **Remaining family**: `ast_compat_slots` (role/roster/zone/world slot views; isolation tests are MIR-wired via the `g_last_mir` ctx-create macro, so the fallback is already dead) — retire by the same pattern to fully close this row. |
