...
- ABI layout facts live in `src/runtime/pgy_abi_spec.h`, with compile-time
  ...
  source-level slot builtins (`Read`/`Write`/`Release`) and block auto-release
  cleanup also consume row-backed runtime names. C source-level pin block
  emitters consume row-backed PinRead/PinWrite and UnpinCleanup names; cleanup
  attributes are not allowed to synthesize `pgy_read_*`, `pgy_write_*`,
