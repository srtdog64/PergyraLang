...
| Source/provenance payload | Treating `source_ast` as semantic inventory truth | `mir_decl_header_source_decl(...)` and `mir_routine_source_decl_of_type(...)` are the remaining compiler-owned compatibility boundaries; `mir_decl_method_source_ast(...)`, `mir_decl_field_source_ast(...)`, and backend `*_source_ast` compatibility accessors are retired | Smoke rejects generic fallback naming, old AST method-array state, backend raw `decl_header->source_ast` / `method->source_ast` reads, backend raw routine `ast` reads, backend `*_source_ast` routine wrappers, the retired method/field source accessors, synthetic-executable AST-returning function lookup, and C/LLVM hosted-method forward/body paths that require source AST when MIR metadata/routine source is available; ast_read_surface ratchets source_decl codegen 2 / compiler 1 and routine_source_decl_codegen 5 | Backend `source_ast` frontier closed; declaration-header source_decl and routine source-decl boundaries remain until final Phase 2 field removal |
