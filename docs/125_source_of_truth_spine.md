...
| Zone refresh metadata | Projection lowering reopened `AST_ZONE_REFRESH` to read object/source slot names and explicit target-to-source field maps | `MIRDeclZoneRefresh` rows populated by `mir_decl_header_refresh.c` and validated by `mir_decl_header_validate.c`; field maps are carried as ordered target/source name rows | `mir-declaration-inventory-test-smoke` requires refresh metadata storage, accessors, validation drift checks, and MIR tests that preserve field-map rows and reject metadata-count drift | Metadata capture closed; C/LLVM projection consumers still need to cut over from AST refresh accessors to these MIR rows |
