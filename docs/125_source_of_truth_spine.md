...
| Zone refresh metadata | Projection lowering reopened `AST_ZONE_REFRESH` to read object/source slot names and explicit target-to-source field maps | `MIRDeclZoneRefresh` rows populated by `mir_decl_header_refresh.c` and validated by `mir_decl_header_validate.c`; C zone declaration emission, zone constructor projection-dirty initialization, and C overlay/assignment projection invalidation consume them through `TranspilerHostedZoneRefreshView`; LLVM zone projection sync, projection slot counting, projection-state struct fields, projection value lowering, zone constructor projection-dirty initialization, zone assignment projection invalidation, and current-zone subject projection sync consume them through `LLVMHostedZoneRefreshView` and MIR refresh metadata accessors | `mir-declaration-inventory-test-smoke` requires refresh metadata storage, accessors, validation drift checks, MIR tests that preserve field-map rows and reject metadata-count drift, C/LLVM zone refresh views, C/LLVM projection value consumers, C/LLVM projection sync/layout/constructor/assignment-invalidation consumers, LLVM current-zone subject projection-sync consumption, and rejects reopened zone refresh inventory in C zone declaration/constructor/overlay-invalidation emission and LLVM declaration-parts/struct-field/constructor/assignment-invalidation/projection-sync-call paths | Metadata capture plus C and LLVM zone projection sync/struct-layout/constructor consumer cutover, C/LLVM assignment invalidation cutover, and LLVM current-zone subject projection-sync cutover closed; remaining relation/effect refreshes and other zone bind/sync compatibility consumers still need MIR refresh rows/views before all AST refresh accessors can be retired |
