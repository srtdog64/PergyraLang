...
| Hosted method signature/action contract | AST method param/return/action reads in LLVM/C prototype emitters, member-call argument policy, zone action runtime lowering, or world embedded action/effect sync | `mir_decl_header_method(...)`, `mir_decl_method_param_count(...)`, `mir_decl_method_param(...)`, `mir_decl_method_return_type(...)`, `mir_decl_method_is_async(...)`, `mir_decl_method_is_action_like(...)`, `mir_decl_method_within_zone(...)`, and `mir_decl_method_causes_effect(...)` | MIR validator rejects signature metadata drift; smoke rejects direct method param/return reads in guarded consumers, hosted-method view wrappers, hosted-view MIR method-array exposure, LLVM member-call pointer-self argument adjustment, LLVM zone action method-contract rediscovery, and LLVM world embedded sync contract rediscovery | Closed for frozen hosted-method prototypes, LLVM member-call argument policy, LLVM zone action method-contract lowering, and LLVM world embedded sync action-contract input |
