...
| Zone state metadata | Zone lifecycle alias/state fields were emitted by reopening `ast_zone_states(...)` from backend declaration emitters | `MIRDeclZoneState` rows populated by `mir_decl_header_zone_state.c` and validated by `mir_decl_header_validate.c`; rows carry owner, state name, layer slot, left/target slot, optional right slot, and relation/effect classification | `mir-declaration-inventory-test-smoke` requires zone-state storage/accessors/capture/validation, C hosted zone-state view, and C zone struct emission through `TranspilerHostedZoneStateView`; it rejects direct `ast_zone_states(node, &state_count)` in `transpiler_zone_struct_emit.c`; `test-mir` preserves effect/relation state rows and rejects zone-state metadata drift | Closed for C zone struct `__state_*` field emission. Remaining C zone sync/frontier/query consumers and LLVM zone struct/sync consumers still need cutover to the same row before physical compatibility removal |
