...
- MIR source-local type facts are keyed by the source local name. LLVM MIR
  alloca/type consumers and C MIR pending-use materialization may see
  ...
  parallel capture structs follow the same rule: callable declarations are
  rendered from source-local callable facts, not by reopening EventHandler
  annotation AST.
...
- Hosted method shape is owned solely by MIR declaration metadata. The C/LLVM
  hosted method views expose only `MIRDeclMethod` metadata and linked routine
  indexes; the non-MIR AST method fallback (`ast_compat_methods` and the
  `*_hosted_method_view_compat_method` accessors) is retired (MIR-only,
  2026-06-23). Backend consumers must not index `ast_compat_methods` directly.
- LLVM MIR parameter self-field slot registration consumes
  ...
  code must use declaration-field type-name facts. Hosted field views must not
  expose AST compatibility field arrays; the remaining non-MIR fallback keeps
  only an `ast_compat_decl` payload and re-enters `host_decl_compat.c` inside
  the field-view owner.
...
- party-slot ability tag selection lives in
  ...
  default/constraint nodes. If the MIR-active renderer cannot find the ability
  declaration header or a required actual/default/constraint metadata row, C
  and LLVM fail closed instead of collapsing the tag back to the base ability
  name. Ability declarations also carry method rows in `MIRDeclHeader`;
  MIR-active party-slot method dispatch must decide ability method ownership
  from those `MIRDeclMethod` rows, not by reopening `ast_ability_method_*`
  scans.
...
- ABI layout facts live in `src/runtime/pgy_abi_spec.h`, with compile-time
  ...
  source-level slot builtins (`Read`/`Write`/`Release`), source-level slot
  method calls, expression-dispatch slot assignment/auto-read, and block
  auto-release cleanup also consume row-backed runtime names. C source-level
  pin block emitters consume row-backed PinRead/PinWrite and UnpinCleanup
  names; cleanup attributes are not allowed to synthesize `pgy_read_*`,
  `pgy_write_*`, `pgy_release_*`, or `pgy_unpin_cleanup_*` suffixes locally.
...
| Host field compatibility view | Class fields and domain shared fields reopened through separate backend switches, or C identifier lowering guessed between a lexical local and an implicit host field from stale SSA snapshots | `host_decl_compat.c` class/shared-field compatibility views, C/LLVM declaration-inventory hosted field views, and `transpiler_host_field_identifier.c` for C current-host identifier emission | Smoke requires C/LLVM constructor channel, class/domain constructor lowering, generic class specialization emission, LLVM domain-parts splitting, MIR SSA zone-field lookup, nominal/current-field, member/local type inference, overlay projection, projection-path helpers, declaration/register emitters to consume hosted field views or field lookup owner seams, C host-field identifier owner coverage, self-receiver gating, stale host-field snapshot rewriting, and zero `ast_compat_fields` exposure in codegen | Closed for backend compatibility and C current-host identifier lowering; direct codegen field-array access is confined to `host_decl_compat.c`, direct class/shared compatibility-view calls are confined to `host_decl_compat.c`, `transpiler_decl_field_view.c`, and `llvm_inventory_field_view.c`, direct class-field compatibility lookup is confined to `host_decl_compat.c`, hosted field views carry only a compat *drift count* (the `ast_compat_decl` non-MIR fallback was retired in the MIR-only decision, 2026-06-23) that fails closed when MIR field metadata is absent, and C field/local shadowing no longer treats MIR source-local field write facts as lexical locals. Backend declaration/projection emitter migration to MIR declaration-field metadata is complete; the only residual AST class-field reads are the design-gated semantic ones (declaration validation / graph collection / projection owner) noted in the declaration-field metadata row |
...
| Zone refresh metadata | Projection lowering reopened `AST_ZONE_REFRESH` to read object/source slot names and explicit target-to-source field maps | `MIRDeclZoneRefresh` rows populated for relation/effect/zone declarations by `mir_decl_header_refresh.c` and validated by `mir_decl_header_validate.c`; C relation/effect declaration sync, C relation/effect constructors, C zone declaration emission, C zone constructor projection-dirty initialization, C relation/effect/zone overlay/assignment projection invalidation, C relation/effect zone bind invalidation, and C world embedded effect sync consume them through `TranspilerHostedZoneRefreshView`; LLVM relation/effect/zone constructor projection-dirty initialization, LLVM relation/effect zone bind invalidation, LLVM zone action invalidation, LLVM world embedded effect sync, relation/effect/zone projection sync, projection slot counting, projection-state struct fields, projection value lowering, relation/effect/zone assignment projection invalidation, and current-zone subject projection sync consume them through `LLVMHostedZoneRefreshView` and MIR refresh metadata accessors | `mir-declaration-inventory-test-smoke` requires refresh metadata storage, accessors, relation/effect/zone capture, validation drift checks, MIR tests that preserve field-map and relation/effect rows, C relation/effect sync/constructor/overlay/bind/world-effect-sync consumers, C/LLVM zone refresh views, C/LLVM projection value consumers, C/LLVM projection sync/layout/constructor/assignment-invalidation consumers, LLVM relation/effect/zone constructor/bind/action/world-effect-sync/assignment consumption, LLVM current-zone subject projection-sync consumption, and rejects reopened refresh inventory in C relation/effect declaration/constructor/overlay/bind/world-effect-sync paths, C zone declaration/constructor/overlay-invalidation emission, retired LLVM AST-array declaration-parts/projection-target helpers, any `ast_compat_refreshes` payload in codegen, any AST refresh inventory/accessor read in codegen, and LLVM struct-field/constructor/bind/action/world-effect-sync/assignment-invalidation/projection-sync-call paths | Metadata capture plus C relation/effect sync/constructor/overlay/bind/world-effect-sync cutover, LLVM relation/effect/zone constructor/bind/action/world-effect-sync/assignment/projection-sync/struct-layout cutover, C and LLVM zone projection sync/struct-layout consumer cutover, C/LLVM assignment invalidation cutover, LLVM current-zone subject projection-sync cutover, C/LLVM hosted refresh view physical compatibility removal, and direct codegen AST refresh accessor retirement closed. Remaining refresh AST reads are compiler MIR-capture only |
| Zone state metadata | Zone lifecycle alias/state fields and query lookups were emitted by reopening `ast_zone_states(...)` from backend declaration emitters | `MIRDeclZoneState` rows populated by `mir_decl_header_zone_state.c` and validated by `mir_decl_header_validate.c`; rows carry owner, state name, layer slot, left/target slot, optional right slot, and relation/effect classification | `mir-declaration-inventory-test-smoke` requires zone-state storage/accessors/capture/validation, C hosted zone-state view, C zone struct emission through `TranspilerHostedZoneStateView`, C zone sync/frontier state iteration through `TranspilerHostedZoneStateView`, C embedded world frontier member counting through `TranspilerHostedZoneStateView`, C intent/projection state transition marking through `TranspilerHostedZoneStateView`, C `transpiler_zone_has_state`, LLVM `LLVMHostedZoneStateView`, LLVM `llvm_zone_has_state`, LLVM zone struct type/field registration through `LLVMHostedZoneStateView`, LLVM zone frontier previous/reset/change tracking through `LLVMHostedZoneStateView`, LLVM zone sync action/apply/detach/link relation clauses through `LLVMHostedZoneStateView`, LLVM embedded world frontier member counting through `LLVMHostedZoneStateView`, and LLVM intent/effect state transition marking through `LLVMHostedZoneStateView`; it rejects direct `ast_zone_states(node, &state_count)` in `transpiler_zone_struct_emit.c` and `transpiler_zone_decl_emit.c`, direct `ast_zone_states(zone_decl, &state_count)` in C/LLVM embedded world frontier member counting and C/LLVM intent/effect state transition consumers, AST-node-return zone-state query lookup names in C/LLVM codegen, AST zone-state accessors in the C zone frontier guard emitter, direct `ast_zone_states(stmt, &state_count)` and AST zone-state accessors in the LLVM zone frontier state helper and LLVM zone sync clause emitters, `ast_zone_states` in LLVM zone struct type/field registration, and any AST zone-state compatibility payload inside C/LLVM hosted zone-state view owners; `test-mir` preserves effect/relation state rows and rejects zone-state metadata drift | Closed for C zone struct `__state_*` field emission, C zone sync/frontier state iteration and previous-state/change tracking, C/LLVM embedded world frontier member counting, C/LLVM domain query state existence lookups, C/LLVM intent/effect state transition marking, LLVM zone struct state field count/name registration, LLVM zone frontier previous/reset/change tracking, LLVM zone sync action/apply/detach/link relation clause state matching, and C/LLVM hosted zone-state view physical compatibility removal. Remaining zone-state AST reads are compiler MIR-capture provenance only |
| Dedicated declaration IR | `MIRProgram` still carries AST-shaped declaration payloads and some backend consumers still use compatibility field views | `MIRDeclHeader` / `MIRDeclMethod` / `MIRDeclField` / `MIRDeclRoleInclude` / `MIRDeclZoneAuthority` / `MIRDeclZoneRefresh` / `MIRDeclZoneState` metadata model, consumed through compiler accessors | Partial gate exists for methods, fields, role includes, zone authority, refresh, and zone-state rows; full closure requires projection/declaration emitters to stop reopening compatibility views when MIR facts exist | Closed (MIR-only decision, 2026-06-23). Production codegen hard-fails without MIR (`driver_app.c:334`); the non-MIR fallback was load-bearing only for in-process codegen unit tests. **Field-shape fallback family closed**: `ast_compat_decl` (C+LLVM class/shared field hosted views) fully retired — accessors fail closed when MIR metadata is absent, the dead struct field is removed, the synthetic-AST isolation test is rewritten through the real parse->lower->emit pipeline, and `mir-declaration-inventory-test-smoke` forbids `ast_compat_decl` reintroduction. **Method-shape fallback family closed**: `ast_compat_methods` (C+LLVM hosted method views) retired — the compat-method accessors fail closed, the struct field/constructor parameter are removed, and the non-MIR consumers (`find_nominal_host_method_decl` etc., all guarded by `if (transpiler_active_has_mir) return` so dead in production) now no-op. **Slot-shape fallback family closed**: `ast_compat_slots` (C+LLVM zone-layer / world-zone / role / roster slot views) retired — accessors fail closed when MIR metadata is absent, the struct field/assigns are removed, and from_decl computes only the drift count. **Row closed (MIR-only)**: all three declaration fallback families (`ast_compat_decl` / `ast_compat_methods` / `ast_compat_slots`) are retired; declaration field/method/slot shape is owned solely by MIR metadata. `transpile` suite stays green (910 passed) and `mir-declaration-inventory-test-smoke` forbids each symbol's reintroduction. The non-MIR fallback remains reachable only when `ctx->mir == NULL`, which production never produces (`driver_app.c:334`). |
...
`MIRDeclField` facts. Hosted field views may retain `ast_compat_decl` for the
explicit non-MIR fallback, but they must not expose `ast_compat_fields` arrays
or let consumers index them. Direct
...
host-compat owner and the C/LLVM field-view owners. Direct
...
The include graph itself is declaration metadata: C/LLVM operator lookup and C
include-wrapper/vtable emission consume `MIRDeclRoleInclude` rows from the role
header in MIR-active paths. AST include statements remain a parser/lowering and
non-MIR compatibility payload only.
