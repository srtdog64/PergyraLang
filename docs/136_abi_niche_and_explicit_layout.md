...
- Slot-like MIR resource operations use `mir_abi_resource_runtime_fn(...)` over
  ...
  builtin call emission must not synthesize `pgy_read_*`, `pgy_write_*`, or
  `pgy_release_*` names from a type suffix. Pin declarations, C direct source
  emitters, and async device submit remain separate ABI projections until
