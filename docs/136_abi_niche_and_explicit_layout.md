...
- Slot-like MIR resource operations use `mir_abi_resource_runtime_fn(...)` over
  ...
  source-level slot builtins, block auto-release cleanup, and pin block cleanup
  attributes must not synthesize `pgy_read_*`, `pgy_write_*`, `pgy_release_*`,
  `pgy_pin_*`, `pgy_unpin_*`, or `pgy_unpin_cleanup_*` names from a type suffix.
