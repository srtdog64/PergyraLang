...
`object`는 intent를 시작하지 않는 피동 상태 대상이고, `tobject`는 예외 객체가 아니라 그보다 더 좁은 boundary projection/transfer 형식이다.
...
특히 `relation`, `effect`, `zone`의 직접 문법, `subject` profile surface, `inout self`, `impl Trait`, thread affinity 표기 예시는 현재 stable current surface를 직접 설명하지 않는다.
...
- `subject`는 subject와 병렬인 존재론적 종류가 아니라, simulation loop / mailbox / scheduler semantics가 붙은 execution model로 보는 것이 목표다
...
ability Attackable {
    ...
    func Attack(inout target: impl Damageable) -> Int
}

ability Tradeable {
    fields _inventory: Slot<Inventory>
    func Trade(inout other: impl Tradeable, items: Array<Item>)
}
