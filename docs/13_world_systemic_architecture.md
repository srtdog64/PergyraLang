...
`object`는 intent를 시작하지 않는 피동 상태 대상이고, `tobject`는 그보다 더 좁은 경계 전달/투영 형식이다.
중심은 `subject -> relation/effect/zone/world 문맥 -> object/tobject projection` 흐름이다.
```
...
### Object, TObject, and Entity

- `object`는 `subject`와 병렬인 새 존재론 계층이 아니다
- `subject`가 transfer / tobject / view / serialization 문맥으로 들어가면 수동적으로 다뤄지는 `object`처럼 해석될 수 있다
