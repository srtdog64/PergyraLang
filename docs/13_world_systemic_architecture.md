...
`relation`, `effect`, `zone`은 이제 `subject slot` / `object slot` / `shared` / `func` 수준의 최소 body surface까지 올라왔고, `zone`은 `relation slot` / `effect slot`, `world`는 `zone` slot으로 최소 조립이 가능하지만, 계층 간 구조적 semantics는 아직 얕다.
