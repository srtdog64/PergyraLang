...
- ◑ slice 5c: LSP publish에 BLUE 머지. `pgy_lsp_diagnostics.c`가 (clean parse/analyze일 때만) DIR/HIR/RIR→`air_synthesize`→`air_collect_erasure_squiggles`→BLUE 진단(severity 3, `squiggleClass:"blue"`, line>0만) append. **LSP-only 격리**(PGY_LSP 링크에 DIR/HIR/RIR/AIR 추가; 배치 컴파일러는 진단용 AIR 합성 안 함 → 빌드/컴파일 0). **검증 경계 정직**: LSP가 실 fixture(05_zone_intent)에서 크래시 없이 동작 + RETAIN-only라 올바르게 blue 없음(`none`) 확인. 정책+collector는 단위검증으로 erase→blue 보장. **남은 것: 실제 erasable-intent 소스로 e2e blue 시연** — pure-sync intent를 만드는 유효 소스 작성이 intent 문법/erasure 조건 때문에 미완(권한 있는 intent는 RETAIN, pure intent 작성 시 문법 오류). collector를 실 lowered AIR에서 boundary->ast NULL 여부 확인 포함 후속.
