...
**slice 5 진행**:
**★ 핵심 발견 (slice 5 "파보자"): `ERASE`는 유효 소스에서 도달 불가.** `air_boundary_compression_budget`상 **boundary는 절대 ERASE 안 됨**(ZONE=authority면 RETAIN/아니면 SUMMARIZE; WORLD/PARALLEL/IO/CHANNEL/EXECUTION=RETAIN). 그리고 모든 유효 intent step은 zone에 묶여 SUMMARIZE boundary를 만들거나(→intent SUMMARIZE) authority(→RETAIN)라, **ERASE intent를 쓰는 유효 소스가 없다**(zone-free pure intent는 INTENT_STEP_INVALID/air==NULL). → slice 5a가 ERASE에 키잉한 게 잘못된 버킷. docs/140 BLUE 정의 "erase **가능**한 의미"(erasable)는 정확히 **SUMMARIZE**(버킷-B POLICY: digest로 유지되나 opt-out 제거 가능)에 대응. RETAIN(버킷-A INHERENT)은 환원불가, ERASE는 이미 사라져 표시할 게 없음.

- ✅ slice 5a: BLUE 정책 `air_compression_squiggle_class(budget, cause)` (`src/compiler/air_erasure_squiggle.*`). **SUMMARIZE→BLUE**(erasable), ERASE/RETAIN/FORBID/UNKNOWN→NONE. 순수 함수(런타임 0, codegen 0). test_air 단위.
- ✅ slice 5b: `air_collect_erasure_squiggles(air, out, cap)` — intents+boundaries 순회, BLUE 노드의 line/col을 **`node->ast`에서 직접** + source_name/reason. 순수 read-only. test_air 단위테스트가 **실 소스(zone-bound intent)를 lower→located BLUE site** 검증.
- ✅ slice 5c: LSP가 clean program을 DIR/HIR/RIR→`air_synthesize`→collect→BLUE 진단(severity 3, `squiggleClass:"blue"`, code `PGY_AIR_MEANING_ERASABLE`, line>0만) append. **LSP-only 격리**(PGY_LSP 링크에 DIR/HIR/RIR/AIR; 배치 컴파일러는 진단용 AIR 합성 안 함 → 빌드/컴파일 0). **e2e 확인**: `Observe`(non-authority zone + 순수 step) 소스 → LSP가 `squiggleClass:"blue"` ×2(intent+boundary) + `MEANING_ERASABLE` publish. RETAIN-only fixture(05)는 올바르게 blue 없음.
- ⏳ 노이즈 관리: SUMMARIZE는 흔함(모든 zone step) → 생산자를 개발자-작성 annotation으로 한정 필요. 정책 함수는 faithful map만 제공.
