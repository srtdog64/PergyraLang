...
|---|---|---|
| `?` 전파 | 없음 | **Result<T,E>에 한해 full end-to-end** (parser/semantic/C/LLVM, fixture 3종: try_operator_result 등). let-초기화 한정 계약 |
...
진짜 갭은 두 개였다: (a) `?`의 **Option 변형 부재**(semantic이 명시 거절 —
self-host는 Result가 아니라 Option-heavy라 정확히 여기가 아팠다),
(b) **subset lag**: self-호스트 parser/checker(bounded subset)가 sugar를 못
읽어서 최대 코드베이스가 채택 불가.
...
C==LLVM), semantic 케이스 2종(수용 + `5?` 거절 메시지), 기존 Result fixture 3종
무회귀, test-semantic 2791/0, test-transpile 916/0.
...
## 5. 남은 진짜 갭 — subset lag (WO-U2)

self-호스트 parser(`parser/expr_*`)와 checker(`semantic/body_check_owner` 등)가
`?`/보간을 모른다 → selfcheck가 거절 → self_hosted 코드가 채택 불가 →
ratchet(sentinel/munge)이 높게 유지되는 **구조적 원인의 일부**. 순서:
subset에 두 sugar 교육 → json.pgy 계열 Option 의식을 `?`로 재작성(감사 §0의
실증 파일부터) → ratchet 하향.
