...
- **DRV-2 — artifact-body semantic source-to-C (landed)**:
  ...
  같은 binary의 source 모드는 typed `AstTreeArtifact`를 Pergyra MIR fact
  graph로 낮추고 검증한 뒤 `pgy.mir.v1`으로 projection하며, 그 JSON을
  Pergyra `mir_lower` owner가 다시 소비한 뒤 semantic verifier와 codegen에
  합류한다. `--mir-json <file>`은 같은 consumer 경계를 직접 실행한다.
  `--emit-mir-json-verified`와 `--canonicalize-mir-json`은 교집합
  4개 fixture에서 C oracle MIR와 canonical JSON이 같음을 잠근다. C oracle
  MIR은 이제 비교 증거이며 bounded source path의 producer가 아니다.
...
- **LSP-2d — buffered response emission plan (landed)**:
  ...
  plan으로 투영한다. 이 base rung은 live read-exact loop와 document store
  mutation을 소유하지 않고, textDocument feature result shape는 LSP-2g에서
  별도 owner로 붙는다.
- **LSP-2e — buffered session replay (landed)**:
  ...
  loop나 document-store mutation은 아니다. textDocument feature result
...
- **LSP-2h — buffered session-state projection (landed)**:
  ...
  아니다.
- **LSP-2 — transport 루프 (planned; G-STDIN/LSP-2a/LSP-2b/LSP-2c/LSP-2d/LSP-2e/LSP-2f/LSP-2g/LSP-2h/LSP-2i prerequisite landed)**: JSON-RPC Content-Length
  ...
  아니다). LSP-2a/2b/2c/2d/2e/2f/2g/2h/2i가 frame/stream/request-plan/response-plan/
  buffered-session/multi-document-state/feature-shape/session-state/hover-content 소비자를 열었으므로 남은 일은
...
| lsp | LSP-2i | landed | src/self_hosted/lsp/hover_content_owner.pgy | tests/self_hosted/parity/lsp_hover_content_parity.sh |
| lsp | LSP-2 | planned | - | - |
| lsp | LSP-3 | planned | - | - |
<!-- DRIVER-LSP-RUNG-END -->
