...
production self-host source 203개를 측정한다. locked minima:
`source_min=203`, `lexer_pass_min=203`, `parser_pass_min=203`,
`semantic_pass_min=203`, `codegen_pass_min=203`,
`lex_parse_pass_min=203`, `lex_parse_semantic_pass_min=203`,
`full_pipeline_pass_min=203`. 세 pipeline baseline manifest는 별도 복사본이 아니라
...
주의: `full_pipeline_pass_min=203`는 이제 lexer/parser/semantic/codegen stage가
