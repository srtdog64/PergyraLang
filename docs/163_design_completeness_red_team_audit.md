...
| Metric | Current |
|---|---:|
| production self-host sources | 203 |
| lexer pass | 203 |
| parser pass | 203 |
| semantic pass | 203 |
| codegen pass | 203 |
| lex+parse pass | 203 |
| lex+parse+semantic pass | 203 |
| full stage intersection | 203 |
...
The monotone ledger is now tightened at `203/203` for lexer, parser, semantic,
...
| Parser self-host | Strong | 203/203 in completeness; parity fixture surface broad |
| Semantic self-host | Medium | 203/203 source-stage checks pass, but deep semantic parity is still bounded by the current subset |
| Codegen self-host | Medium | fixed-point exists for subset; 203/203 source-stage checks pass via self-parser AST text |
...
1. Replace the codegen stage's AST text bridge with typed self-parser /
   self-semantic owned facts, then keep the 203/203 ledger green.
