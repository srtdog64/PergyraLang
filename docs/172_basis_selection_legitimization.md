...
**현황: 부분** — AxisOwnership.v(exactly-one-owner, no-silent-override)와
docs/42가 직교성을 커버. macro-expressibility 논증 자체는 미작성.
...
**현황: 미착수.** core calculus(UnifiedCore.v/WholeProgramCore.v)가 내부 일관성
(preservation/progress)을 잡았으니, 다음 자연 rung이 외부 참조 프레임 대비
완전성이다.
...
**현황: 자료 산재**(docs/119 계보, docs/170) — 삼각측량 *논증*으로 묶은 문서 없음.
**정직한 편향 경고**: 게임 어휘는 이산적·시뮬레이션 가능 세계에 편향. 금융/세무
/공장(BDFL 본업) 같은 연속·규제 도메인이 다른 명사를 요구하면 기저 수정 신호다
— M5의 dogfood에 **비게임 도메인 1개 필수**(공장 장비 SW가 이상적 2번째 점).
...
### M5 — 경험 반증 (dogfood 지표)
기저 적합성을 경험 가설로 취급: **탈출구(escape hatch) 지표** — 실코퍼스에서
도메인 의미가 주석/문자열/네이밍 컨벤션으로 새는 빈도 = 기저가 못 담은 의미의
실측. 던전크롤러(+비게임 1개)에서 측정, 임계 초과 시 기저 수정 논의가 열린다.
**현황: 계획만**(killer usecase 문서에 지표 정의 없음).
...
없다 — 그건 M2(완전성)+M3(수렴)+M5(반증)의 몫. 세 무브가 다 실패하면 기저를
고쳐야 하고, M4가 그 수정 비용을 유계로 만든다.
...
- **WO-BASIS-1** (M3, 최저비용): 수렴 삼각측량 문서 — 5개 독립 전통의 범주
  대응표 + 게임-편향 경고 명문화. 순수 저술.
- **WO-BASIS-2** (M1): 축별 macro-expressibility 논증 — "이 축을 라이브러리로
  내리면 무엇이 깨지나"(정적 의무 중심). AxisOwnership.v 옆에 산문+가능하면
  기계화.
- **WO-BASIS-3** (M2, 최대 갭·최대 비용): 참조 프레임 1개 선정(권고: bigraph
  유계 단편) → 인코딩 정리. core calculus 트랙의 다음 rung으로 자연 편입.
- **WO-BASIS-4** (M5): 탈출구 지표 정의 + 던전크롤러/공장-도메인 dogfood 측정
  계획. killer-usecase 문서에 계측 절 추가.
