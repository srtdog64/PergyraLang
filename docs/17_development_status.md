...
- 2026-04-16 기준 LLVM 내부 helper 정리:
  - host declaration/method lookup, implicit `self` 판정, pointer-self type classification을 공용 helper로 묶었다
...
- `handoff_projection_frontier`도 backend-compare에 올라가서 v1 handoff materialization 이후 source/target projection freshness를 C/LLVM stdout parity로 직접 잠근다.
- `handoff_world_state_frontier`도 backend-compare에 올라가서 active world-owned zone handoff 이후 projection-backed world state와 composed state freshness를 C/LLVM stdout parity로 직접 잠근다.
- `handoff_layer_state_frontier`, `world_embedded_action_frontier`, `world_embedded_action_pool_frontier`도 backend-compare에 올라가서 action-caused effect가 transfer target zone 및 embedded world zone의 single/pool layer-state와 active world-derived layer/state alias로 전파되는 경로를 C/LLVM stdout parity로 잠근다.
- `intent_authority_snapshot`도 backend-compare에 올라가서 `authorized by`가 MIR metadata에서 C/LLVM runtime authority snapshot까지 내려가는 경로를 stdout parity로 잠근다.
- 이 시점의 남은 propagation blocker는 richer flag/provenance 자체가 아니라, 이미 들어간 zone/world bounded frontier loop와 embedded/handoff projection/layer/state closure를 닫은 뒤 남은 authority/failure handoff와 더 넓은 world-zone propagation family까지 같은 source-of-truth로 일반화하는 일이다.
...
- 좁은 `binding slot` admission/runtime slice는 production self source -> admitted MIR -> general C에서 실행되며 native C/LLVM과 exact output parity 및 valid-ID wrong-kind negative를 통과한다. 이 증거는 compiler root 전체가 아니라 해당 입력 기능 slice만 `SUBSTITUTING`으로 분류한다.
...
- Added `authority_failure_abi` and `intent_authority_snapshot_abi` coverage to the ABI smoke matrix and to `test_abi_pipeline`.
- Added `authority_failure_surface` and `intent_authority_snapshot` to backend-compare and fixed the transpiler extern-inventory drift behind authority runtime exports.
...
- The baseline queryable state now includes `last_ok`, `last_zone`, `last_participant`, `last_code`, and `last_reason`.
- Intent `authorized by` validation now resolves participants to concrete zone subject slots before accepting authority, so same-type non-authority slots and ambiguous same-type mappings are rejected before backend/runtime lowering.
