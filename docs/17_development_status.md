...
- `DIR`, `RIR`, `MIR` 코드 계층은 각각 `--dir`, `--rir`, `--mir`로 declaration/resource/execution dump를 제공하고, backend runner는 `CompilerIRBundle`을 입력으로 받는다. 현재 실제 codegen 연산의 중심 자료구조는 여전히 `HIR`이지만, C backend에는 이제 simple top-level function CFG subset과 intent cleanup/rollback/invalidation CFG subset을 `MIR` 본문/exceptional block에서 직접 emit하는 첫 vertical slice가 들어가 있다.
