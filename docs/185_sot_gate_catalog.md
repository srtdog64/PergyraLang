...
| `make self-host-one-mir-generic-struct-value-flow-projection-test-smoke` | 5 min | LANDED, focused | the explicit generic template, self-MIR specialization class, four graph calls, Pair receipt, SSA use, or member path drifts; a three-routine rejection retries another interpretation; an emitter reopens MIR or reconstructs the specialized symbol; or C/LLVM exact output differs |
| `make self-host-one-mir-inferred-generic-nominal-projection-test-smoke` | 5 min | LANDED, focused | the inferred exact-two specialization class, generic template, two graph calls, Pair receipt, SSA use, or member path drifts; two-routine routing retries plain/Option; an emitter reopens MIR or reconstructs the specialized symbol; order/provenance metamorphics change the artifact; or C/LLVM exact `42` differs |
