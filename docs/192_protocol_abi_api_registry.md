...
They are still `BRIDGE` because the whole aggregate/runtime surface is not
migrated behind one complete compatibility corpus. The active C MIR resource
path now
reads the lowered `MIRInstruction.resource_runtime_fact`; C pin/block/builtin/
let-slot emitters route through `transpiler_slot_runtime_row_for_operation`,
