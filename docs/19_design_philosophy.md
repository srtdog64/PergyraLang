...
## ✦ 판정이 아니라 선언 — Rice 정리와 두 종류의 손실

이 언어가 *왜 분석기가 아니라 선언 primitive의 집합인지*에 대한 가장 깊은
이유. project_research_program_thesis("잃어버린 도메인 의미 회복")를 계산
이론 위에 다시 세운다.

### ✦.1 손실은 두 종류다

프로그래밍이 도메인 의미를 줄이는 "손실"은 단일하지 않다. 구분이 전부다:

- **Rice-손실 (있는데 못 정한다):** 정보가 프로그램 *행동* 안에 실재하지만,
  정적으로 정확히 판정 불가능. "이 프로그램이 런타임에 CLOCK을 행사하는가"
  — 행동은 존재하나 동적 디스패치/FFI/`unsafe` 때문에 호출 그래프가
  불완전 → **결정 불가능**.
- **Thesis-손실 (애초에 없다):** 누가/왜/어디까지/자격/세계/책임/전이 —
  대부분 프로그램 행동의 성질이 *아니다*. 설계자 머릿속에 있었고 **코드에
  인코딩된 적이 없어서** 사라진 빈자리. `authorized by`를 안 적었으면 어떤
  분석도 못 복원한다. 거기 *없으니까*.

### ✦.2 Rice 정리 — 분석으로 되찾는 길은 원리적으로 막혀 있다

> **프로그램이 "무엇을 하는지"(비자명한 의미론적 성질)에 대한 모든 질문은
> 일반적으로 결정 불가능하다.** (정지 문제의 일반화.)

핵심 구분: *의미론적(행동)* 성질은 결정 불가능, *구문적(텍스트)* 성질은
결정 가능. "소스에 `Now` 토큰이 있나"는 OK, "런타임에 시계를 읽나"는 불가능.
→ Rice-손실도 Thesis-손실도 **"분석으로 의미를 되찾는다"는 길을 막는다.**

### ✦.3 그래서 — 판정(decide)하지 말고 선언(declare)으로 우회한다

Pergyra의 답은 결정 불가능한 질문을 *풀려는 게 아니라* 빠져나가는 것:

| | 방향 | 성격 |
|---|---|---|
| **유도(inference)** | Rice와 정면 대결 | 일반적으로 결정 불가능 → best-effort + 런타임 backstop. 손실을 *인정*. |
| **선언(declaration)** | Rice를 *빠져나감* | 의미를 구문으로 끌어내림 → 결정 불가능한 의미론 질문이 **결정 가능한 구문 사실**로 변함. |

`with caps clock`이라 쓰는 순간 "이게 CLOCK을 선언했나"는 그냥 텍스트
...
이 통찰은 thesis 자체를 다시 쓴다:

- ❌ "잃어버린 도메인 의미를 (분석으로) **복원**한다" — Rice/Thesis-손실이
  막음. 원리적으로 불가능.
- ✅ "도메인 의미를 **선언할 어휘를 제공**해서 애초에 잃지 않게 한다 +
  못 선언한 잔차는 **런타임에 fail-close**한다." — 결정 가능 + 강제 가능.

slot / lifecycle / world / intent / authority / capability가 전부 *분석기*가
아니라 *선언 primitive*인 것은 우연이 아니다 — 언어는 무의식적으로 이미
Rice를 우회하는 쪽으로 설계돼 왔다. capability를 "유도+선언"으로 만든
2026-06-21 결정(docs/semantics/15)도 같은 본능의 명시적 발현이다:
유도는 Rice와 싸우고(런타임 backstop), 선언은 Rice를 빠져나간다.
