...
```
zone 내부 action → within 생략 가능 (자기 zone에서 유도)
...
within        → 선택. 장소 제한이 필요할 때만.
               zone 안 action은 lexical zone에서 자동 유도 (2026-04-06 구현)
authorized by → 선택. 승인이 필요할 때만.
causes        → 선택. 효과가 있을 때만.
```

### zone 안 action의 within lexical 유도 (2026-04-06 구현)
...
zone BattleZone
{
    action Attack(self)
        requires Combatable
        // within BattleZone ← lexical 유도. 컴파일러가 채움.
    {
        ...
    }
}
...
| 기능 | 상태 | 파일 |
|------|------|------|
| action within lexical 유도 | 구현 완료 | type_checker.c (zone 내부 action within 자동 주입) |
...
| within lexical 유도 | zone 내부만 | zone 외부는 명시 필수 (안전) |
