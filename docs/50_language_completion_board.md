...
| MIR -> LLVM | 진행 중 | routine body MIR 경로 고정, compiler MIR entry에서 원본 HIR direct dependency 제거, declaration/top-level inventory를 MIRProgram으로 이관, intent/class/subject MIR carrier 확장, intent step check/eval/meta MIR-only 강제, C declaration signature fallback(`int32_t`) 제거, `select`/view/move/BoxArray inner-type fallback 제거 | dedicated declaration IR 없이 AST-carried inventory를 쓰는 구조 debt 축소 |
