...
4. direct `AST_EVENT_INVOKE` 경로와 parser surface의 정합성을 확인한다.
...
- [ ] `invoke` 인자 개수/타입 검증
- [ ] parser surface와 direct event AST 경로 정합성 점검
...
### 2.3 World LLVM closure

대상:
- `World`의 LLVM cell

작업:
1. world struct/sync/init 흐름을 LLVM 쪽에서 정리한다.
2. world-related path가 `llvm_domain.c` 내부에서 어떤 전제에 기대는지 문서화한다.
3. C와 LLVM world smoke를 맞춘다.
4. 필요하면 `llvm_world.c`로 분리하되, 분리는 목적이 아니라 검증 수단으로 쓴다.

완료 기준:
- 대표 world 예제가 C/LLVM에서 같은 의미로 실행된다.
- "world는 C만 된다"는 평가가 더 이상 맞지 않는다.

체크리스트:
- [ ] world init/sync LLVM 경로 확인
- [ ] world state/roster/zone activation LLVM smoke 추가
- [ ] C/LLVM 출력 동치 테스트 추가
- [ ] 필요 시 file split
...
- 아직 남은 것: parser가 직접 만드는 invoke surface와 direct event AST 경로의 최종 정합성 점검, generated helper 기반 runtime 설명 정리
