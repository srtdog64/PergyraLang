...
- non-generic nominal class and known non-class scope type references now use DAG metadata materialization rather than the central recursive resolver. The smoke gate is tightened to `metadata_entries>=3300`, `metadata_hits>=4900`, `metadata_owned>=200`, `materializer_fallbacks<=1296`, and exact fallback-family accounting. Current fallback families are `named=1289`, `generic_named=7`, `compound=0`, `other=0`; named details are `builtin_shell=2`, `generic_class=0`, `alias=1281`, `non_class_symbol=0`, `missing_symbol=6`.
