...
- Already admitted return, let, and assignment verdicts legitimately share
  that public identity while retaining distinct Pergyra codes.
  `builtin_arg_type_mismatch` remains a forbidden co-admission, along with C
  mapping, native retry, message parsing, another semantic pass, invented
  location, text/fact changes, and a second renderer.
- The active rung may admit only this exact Pergyra code, prove all thirteen
  ...
  `tests/self_hosted/parity/public_call_arg_type_mismatch_json_diagnostic_receipt_owner.sh`.
- A fresh Pergyra-built DRV-2 passes the focused thirteen-context gate,
  rebaselined return/logical/let/assignment negatives, and the complete
  installed CLI aggregate through artifact, parser, tokens, AST, LLVM IR,
...
- Fresh production falsifier: `bad_assign_type.pgy` reaches the installed
  ...
  MIR JSON stops at the generic malformed-receipt error.
- Explicit native JSON fixes `PGY_SEM_TYPE_MISMATCH`, stage `semantic`, layer
  `type`, cause `semantic:assignability_check`, and fix `annotate-or-convert`.
- Already admitted return and let verdicts legitimately share that public
  identity while retaining distinct Pergyra codes. `call_arg_type_mismatch`
...
- A fresh Pergyra-built DRV-2 passes the focused assignment gate, rebaselined
  return/logical/let negatives, and the complete installed CLI aggregate
  through artifact, parser, tokens, AST, LLVM IR, native opt-in, REPL,
  formatter, and DeviceSlot boundaries.
- Diagnostic, SoT, Gate, protocol, build-source inventory, substitution
  velocity, hard-contract, and progress ratchets pass. The broad component
...
- Fresh production falsifier: the assign, condition, and operand compare
  ...
  public MIR JSON stops at the generic malformed-receipt error.
- Explicit native JSON fixes `PGY_SEM_BINOP_TYPE_MISMATCH`, stage `semantic`,
  layer `type`, cause `semantic:binop:operand_types`, and fix
  `align-operand-types-or-overload` for all three contexts.
- Logical operands legitimately share that public identity while retaining a
  different Pergyra code/fact shape. `binop_type_mismatch` remains a forbidden
  co-admission, along with C mapping, native retry, message parsing, a second
  semantic pass, invented location, text/fact changes, and a second renderer.
- The active rung may admit only this exact Pergyra code, prove all three
  private contexts, and reuse the existing MIR/artifact process and wire owners
  for public MIR/C/LLVM relay. Its focused falsifier is
  `tests/self_hosted/parity/public_compare_type_mismatch_json_diagnostic_receipt_owner.sh`.
- A fresh Pergyra-built DRV-2 passes the focused compare gate, rebaselined
  logical/unary gates, and the complete installed CLI aggregate through
...
- Exact base is `ca122be5396a4862a8b330e7a9d2e9710920a775` and implementation
  `c01ac369716e0a8486c4c762a4ca5606f12b528a` is on `origin/main`. Protected
  unrelated untracked paths remain outside inspection, edit, and staging.
...
- Diagnostic, SoT, Gate, protocol, build-source inventory, substitution
  velocity, hard-contract, and progress ratchets pass. The broad component
  inventory was left to exact CI under the 60-second local budget.
- Exact-head CI run `33527177441` is green 30/30 with backend shards 20/20.
  `build-linux` passed the structural component contract in 24m58s. Full
  self-host passed in 35m12s and records exactly one 173244-line `gen2 == gen3`
  fixed point, receipt-bound driver adoption, Pergyra-built DRV-2 installation,
  and focused condition-receipt marker.
- SoT remains `88/183`, `CLOSED=55 BRIDGE=32 ACTIVE=1`, with 9 blockers.
...
- Exact base is `645d6e42014cc7959416bc5c5aecbe4df3c4d46d` and implementation
  `1d76bd1cfdac736df706ef114813c9d272b4e288` is on `origin/main`. Protected
  unrelated untracked paths are listed in the collaboration ledger and remain
  outside inspection, edit, and staging.
...
- Exact run `33455765599` is green 30/30. Full self-host records
  `gen2 == gen3 (173074 lines)`, installs the receipt-bound Pergyra DRV-2, and
  passes the existing source-inspection profile gate followed by the new MIR
  profile gate. Full self-host took 33.70 minutes and build-linux 25.48 minutes;
  both bootstrap jobs, Rocq, sanitizers, Linux/Windows/macOS, backend toolchain,
  and all 20 comparison shards passed.
- Exact objective and local evidence are in
  `docs/agent_work_directives/optimization_neutral_mir_read_outputs_2026-09-01.md`.
  The publication falsifier is closed. Resume by selecting one fresh production
  executable falsifier; do not infer a machine-manifest or native-only IR
  successor from this boundary or its timing.
...
- Removed bypass: C value-result copy-in no longer spells `pgy_as` locally;
  LLVM copy-in/out no longer spell `%pgy.array.string` or `align 8`; the C
  callable signature no longer reads the global ArrayString carrier directly.
- Fact owner: the admitted `DirectMirScalarProgramArrayStringAbiFact` plus one
...
- The release reconstruction target is now explicitly marked an approved
  product goal: optimized, stripped Pergyra releases must be no easier to
  reconstruct than a same-toolchain C++ baseline. Its acceptance gate remains
...
- First repair exact-head evidence: run `33213561874` at `9d1fded5` completed
  RED 28/30. Windows, macOS, TSan, sanitizers, Rocq, codegen fixed point,
  backend toolchain, and backend 20/20 were GREEN. `build-linux` reached its
  final preparation step and found the generated language-word implementation
  inventory stale. `self-host-bootstrap-linux` ran 35 minutes, built the
  driver, passed the nested-intent C/LLVM gate, then exposed a second closure
  residue in `driver_source_c_execution_action_gate.sh`: the child-owned
  `intent:` target still had to equal an empty step-header reconstruction.
- Current repair: the language-word inventory is regenerated by its owner.
  `CodegenIntentSourceFactsFromAdmittedSemanticOrDie` preserves a producer
  diagnostic instead of replacing it with a generic crossed-fact message.
...
- Observed local gates: a fresh Pergyra-built DRV-2 passes the complete
  ...
  pair passes placed-nested-intent direct/public/native execution parity. The
...
- Objective card: production entrypoint `pgy debug SOURCE`; priority is one
  ...
  missing/invalid behavior, and old C path deletion. The parser build owns
  source/cursor observations, `src/self_hosted/debug/session_owner.pgy` owns the
  admitted session and transitions, and `src/compiler/debugger.c` is the last
  public C adapter. Native parse/semantic retry, dual reads, post-admission
  source reload, and C AST walking are forbidden.
- Executable replacement: `debugger.c` shrank from the native debugger to one
  argument adapter. `self_host_debug_driver.c` owns installed child execution;
  it canonicalizes the source identity, resolves the sibling, authorizes I/O,
  executes once, and reports failure without retry. The Pergyra session creates
  one `ParserProgramBuild`, derives the artifact from that build, admits typed
  semantic and body facts, binds parser-cursor locations to the exact artifact,
  then owns command, breakpoint, traversal cursor, and exit state. This is
  Pergyra-native `SUBSTITUTING`, not UI-only reachability.
- Observed local gates: the current Pergyra-built installed driver completed;
  the native compiler linked with `LLVM_ENABLED=0`; focused public debug passed
  real `n/n/q` line-3 provenance, exactly-once shim, missing-driver and invalid
  source nonzero/empty stdout, no native timing, one parser build, and static C
  residue deletion. Tooling conformance, build-source inventory, C
  `-Wall -Wextra -Werror -fsyntax-only`, SoT authority edge, and the full
...
- Published closure: implementation `d78a4040`, initial checkpoint `4b18fb9f`,
  ...
  `origin/main`. Exact-head run `33105228322` completed GREEN 30/30. The repair
  emits `<errno.h>` with self-host host-I/O C so `errno`/`EINTR` cannot depend
  on an incidental transitive include. Protected untracked
...
- Observed local evidence after rebuilding `bin/pgy-self-driver.exe` and
  ...
  LLVM installed leg.
- Local CI repair evidence: `read-stdin-builtin-test-smoke` passes its C and no-EOF
...
- Remote closure: run `33105228322` passed the repaired codegen bootstrap in
  8m18s, full self-host in 34m05s, `build-linux` in 28m22s, Windows in 8m40s,
  sanitizers in 11m35s, and backend shards 20/20. The long overall wall time
  includes substantial hosted-runner queueing; it is not all compiler/test
  execution time.
- Grade and next falsifier: this is production `SUBSTITUTING` for the real
  ...
  published progress values stay unchanged. No executable successor is
  admitted. The attached identity review remains an audit lens: do not open a
  query/cache, epoch-performance, multi-document, or semantic-index track until
  one fresh production bypass, complete Pergyra owner, last consumer, and
  executable falsifier are named together.
