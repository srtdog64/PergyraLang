...
- 현재 runtime은 executable intent function + basic conflict scheduler + minimal trace/rollback engine이다. `exclusive` 충돌 차단, `concurrent` 병행 허용, higher-`priority` nested override, reverse-order `compensate` rollback, `IntentLastTrace()` / `IntentLastFailure()`, `using:` 기반 live zone-instance sync까지는 구현되어 있다. 남은 건 richer trace id/history API, cross-world transfer, 그리고 fuller actor-to-zone-slot materialization이다.
