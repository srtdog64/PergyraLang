...
payloads for binding-name recovery. The next cut target is the remaining
expression/shape tail: destructure emission shape/initializer handling and
selected validation/diagnostic body facts are still read from source payloads
rather than dedicated MIR records.
...
identity. Destructure binding-name/index recovery is MIR-owned for C SSA local
type/view facts. The self-hosted checker proves the same manifest. The
remaining destructure emission shape/initializer and selected
...
pointer identity. The remaining ACTIVE tail is the narrower destructure
...
destructure emission/initializer plus validation/diagnostic tail, then continue
