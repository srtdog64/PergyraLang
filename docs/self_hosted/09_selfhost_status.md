...
- Parser (src/self_hosted/parser/): compiles on C and LLVM and compares
  ...
  cursor/token reads, written type-name parsing, expression parsing, and compact
  AST text formatting are owned by separate parser modules; statement and
  declaration parsing remain in the entry module. The current examples scale
  probe is
