...
tool parity. The emitted artifact carries the deduplicated proof-gate list,
...
contract in `hir/typed_ast_arena_owner.pgy`. The contract proves the
...
and compares C/LLVM-built in-process driver emission against a C-built driver
oracle across the committed parity fixtures named by
...
in their stage owners. The former cross-process `AST text -> standalone
codegen` oracle is not a legal driver boundary: it discarded parser-owned
expression and match-pattern graphs, then forced the consumer to recover
semantic facts from text. Standalone codegen retains its own bootstrap and
parity gates, while driver parity preserves the typed artifact in-process.
