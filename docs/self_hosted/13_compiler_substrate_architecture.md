...
plus arena value rows for the initializer, and `Assign` consumes arena atom/value
rows for target and RHS; other compound payloads remain bridge-owned.
...
name/type/initializer and `Assign` target/RHS rows already come from the arena.
Parameter mode is part of this bridge contract: native and
