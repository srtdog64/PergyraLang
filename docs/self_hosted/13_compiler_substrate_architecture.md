...
- `ast_text_row_fact_owner.pgy` owns the name/type/mode rows derived from
  inventory payloads: function, return, role, nominal, enum, field, and
  parameter row facts are populated once during inventory construction and then
  consumed from `CodegenAstTextNode` fields.
