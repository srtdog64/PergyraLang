...
current `program_emit.pgy` consumes that typed node inventory first and then
projects the legacy parallel `indents`/`texts` arrays for the emitters that
have not migrated yet. The legacy projection is a measured bridge, not a final
semantic source of truth. The
