...
current `program_emit.pgy` consumes typed nodes for program-level declaration
routing, `Main` counting, event rejection, owner skipping, and method/function
dispatch. It still projects the legacy parallel `indents`/`texts` arrays for
collector, function, and statement emitters that have not migrated yet. The
legacy projection is a measured bridge, not a final semantic source of truth. The
