...
fields. Single-payload statement emission uses the same arena atom rows for
`Log`, value `Return`, `ArrayPop`, `Exit`, `While`, `If`, `Match`, match cases,
and bare-call payloads. `Let` emission also consumes arena atom/type-name rows
for the local name and declared type; other compound payloads remain
bridge-owned until they are represented as row-shaped arena facts.
...
The current `input/` bridge has three explicit fact owners:
...
- `ast_text_statement_owner.pgy` owns statement-row facts for `Let`, `Assign`,
  ...
  `Break`, `Continue`, `For`, `While`, `If`, `Else`/`else if` routing, and bare
  call statements.
...
rather than a tagged AST semantic record. Single-payload statements now read
arena atom rows in emission; compound statement payloads still read statement
owner facts until their multi-field payloads move into typed rows. `Let`
name/type rows already come from the arena, but initializers still come from the
statement owner. Parameter mode is part of this bridge contract: native and
