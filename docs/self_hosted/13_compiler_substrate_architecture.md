...
rather than a tagged AST semantic record. Parameter mode is part of this bridge
contract: native and
...
| self-host typed AST-text bridge | `input/ast_text_inventory_owner.pgy` for raw `pgy --ast` lines, `CodegenAstTextNode`, indentation, parent edge, coarse kind, function/return/role/nominal/enum-name/field/parameter payload fields, marker predicates, blank filtering, `[export]` normalization, declaration payload accessors, parameter mode preservation, cursor expectations, and `CodegenTypedAstBridgeReady`; `input/ast_text_statement_owner.pgy` for statement-row facts; `typed_ast_node_skeleton.pgy` for the typed AST arena payload contract that will replace the bridge | self-hosted C emission today; parser/codegen cutover later | `program_emit`, declaration collectors, function signature emission, and statement body emission consume typed bridge nodes for program-level routing, prepasses, marker rows, function/return/role/nominal/enum-name/field/parameter payloads, enum-variant payloads, statement facts, function body-marker reads, and remaining expression payload reads; `GenerateC` now consumes `CodegenTypedAstBridgeReady(nodes, count)` before emission; `inout` signatures/calls consume recorded `pm` facts; no emission participant may re-split AST text or consume parallel `indents`/`texts` arrays |
