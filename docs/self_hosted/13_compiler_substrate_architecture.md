...
types, parameter mode/name/type rows, field name/type rows, and the first
statement-level rows for `Let` and `Assign`; emission participants consume
those accessors instead of carrying local AST-text parsers. The
