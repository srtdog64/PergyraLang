...
| Target capability envelope | `target_capability_owner.pgy`, `TargetCapabilityZone` | `self-host-compiler-world-contract-test-smoke`, real-source selfcheck | CPU/C/LLVM/self-hosted projections now use an exact projection/fact/fallback schema before emission. Projection rows, target facts, and fallback reasons are named owner facts, and `CompilerTargetCapabilityEnvelopeReady()` consumes those facts instead of comparing row indexes to string literals. The self-host C codegen run boundary consumes that envelope before `GenerateC`, so hidden CPU fallback cannot enter that hard rung by omission. Native C/LLVM target-specific consumers still need to read the same envelope before this surface can leave `ACTIVE`; AIR-overlapping target fact names remain target-envelope facts until the import/idempotence contract is checked and the target owner can consume `air_evidence_owner.pgy` directly. |
...
| Artifact Zone evidence | `src/self_hosted/compiler/artifact_zone_owner.pgy`, `ArtifactZone` | The comparable artifact vocabulary now covers diagnostics, LSP diagnostics, LSP transport frame/stream, LSP request dispatch, LSP response emission, LSP session replay, LSP document-store state, LSP session-state, LSP hover-content, AST text, AIR JSON, MIR JSON, ABI/layout, runtime materialization classification, emitted C, emitted LLVM, emitted self-hosted output, and run output. Each artifact kind has a named owner fact (`CompilerDiagnosticsArtifactKind()`, `CompilerLspDiagnosticsArtifactKind()`, `CompilerLspTransportFrameArtifactKind()`, `CompilerLspTransportStreamArtifactKind()`, `CompilerLspRequestDispatchArtifactKind()`, `CompilerLspResponseEmissionArtifactKind()`, `CompilerLspSessionReplayArtifactKind()`, `CompilerLspDocumentStoreArtifactKind()`, `CompilerLspSessionStateArtifactKind()`, `CompilerLspHoverContentArtifactKind()`, `CompilerAstTextArtifactKind()`, `CompilerAirJsonArtifactKind()`, `CompilerMirJsonArtifactKind()`, `CompilerAbiLayoutArtifactKind()`, `CompilerRuntimeMaterializationArtifactKind()`, `CompilerEmittedCArtifactKind()`, `CompilerEmittedLlvmArtifactKind()`, `CompilerEmittedSelfHostedArtifactKind()`, `CompilerRunOutputArtifactKind()`), and `CompilerArtifactZoneReady()` consumes those facts instead of comparing row indexes to string literals. A duplicated, reordered, or re-literalized artifact kind cannot satisfy the owner by name presence alone. `backend_output_comparator` records `run_output` through `CompilerRunOutputArtifactKind()`, emitted C through `CompilerEmittedCArtifactKind()`, diagnostics through `CompilerDiagnosticsArtifactKind()`, LSP payloads and squiggle-policy snapshots through `CompilerLspDiagnosticsArtifactKind()`, buffered LSP document state through `CompilerLspDocumentStoreArtifactKind()`, and generated self-hosted artifacts through `CompilerEmittedSelfHostedArtifactKind()` instead of positional artifact indexes. The tri-compare harness now treats the Pergyra comparator report as the verdict owner instead of keeping a shell `cmp`/`diff` veto, and the shared C-vs-LLVM helper, DRV-0 AST/C artifact parity, parser AST fixture/live/backend-output parity, parser scale-probe AST-text measurements, semantic diagnostic-verdict parity, diagnostic-catalog report parity, linter diagnostics parity, MIR JSON reconstructed run-output parity, LSP diagnostics payload/squiggle parity, LSP transport/request/response/session/document-store/session-state/hover-content parity, codegen parity run-output checks, lexer token-output checks, lexer scale-probe run-output measurements, codegen bootstrap emitted-C fixpoint, codegen bootstrap fuzz-generator manifest/`f*.pgy` corpus checks, fuzz backend generator emitted-self-hosted output checks, AIR graph validator clean JSON parity, AIR graph validator live fixture drift checks, AIR graph consumer clean JSON parity, production C/header size report parity, runtime-boundary report parity, stable-subset-section report parity, stdlib-dispatch-inventory report parity, and general self-host report clean JSON parity invoke the Pergyra comparator on explicit artifact paths, including committed-expected vs live-oracle drift and generated self-host output. It remains active until all parity artifacts are written and compared from this owner. |
| Test harness substrate | `src/self_hosted/compiler/test_harness_owner.pgy`, `TestHarnessZone` | Fixture and result row vocabulary is now Pergyra-owned. Source, diagnostic, AIR JSON, MIR JSON, ABI layout, stdout, exit, projection, C/LLVM/self-hosted projection names, and comparable fixture paths are named owner facts, and `CompilerTestHarnessReady()` consumes those facts instead of comparing row/projection indexes to string literals. `backend_output_comparator` records C/LLVM/self-hosted projection rows, comparable artifact paths, and finding caps by consuming this owner. It accepts artifact paths and projection row indexes through `Args()`, and `backend_output_tri_compare_parity.sh`, `llvm_leg_helpers.sh`, `codegen_parity.sh`, `lexer_parity.sh`, plus AIR graph validator clean JSON parity now route verdicts through that Pergyra owner. It remains active until the shell parity scripts are projections of these records instead of the primary harness owner. |
