...
| DRV-0 artifact gate | `src/self_hosted/compiler/driver_rung0_owner.pgy` plus focused AST/C artifact parity gate | The in-process assembly owner now composes source path -> self-parser AST text -> self-codegen emitted C after consuming path, stage-artifact, and target-capability facts. It exposes separate `CompileSourceToAst`, `CompileAstToC`, and `CompileSourceToC` artifact seams plus `--emit-ast` / `--emit-c` runner modes so the future gate can compare owner-owned outputs directly. It remains active until that gate compares the assembled AST and C artifacts against `pgy --ast` and the current codegen oracle; without that gate, the owner is scaffold evidence, not a landed hard driver rung. |
