...
TypedAst delta, 2026-07-07: `ArrayPush` statement emission now consumes typed
arena atom/value rows for the receiver and pushed expression. `ArraySet` and
`For` remain in the statement owner because they need additional multi-field
typed rows.
...
old targeted `CodegenAstText*` payload accessors in `stmt_emit.pgy`. Compound
statement payloads remain bridge-owned until their multi-field payloads are
projected as typed arena rows.
