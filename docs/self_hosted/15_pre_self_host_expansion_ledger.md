...
| Self-host typed AST-text bridge | `src/self_hosted/codegen/input/ast_text_inventory_owner.pgy`, `src/self_hosted/codegen/input/ast_text_statement_owner.pgy`, plus `pgy --ast` parameter-mode preservation | component contract, real-source selfcheck, parser parity, codegen parity | raw `pgy --ast` line splitting, `CodegenAstTextNode` inventory, indentation, parent/kind rows, marker-node predicates, blank-line filtering, `[export]` normalization, program-level typed routing, declaration collector prepasses, function/return/enum/nominal/role/parameter/field payload accessors, `Let`/`Assign`/simple statement fact accessors, `ArraySet`/`ArrayPush` collection mutation statement fact accessors, `For`/`While`/`If`/`Else` control-flow statement fact accessors, function signature/header facts, statement body reads, `inout`/`own`/`ref` parameter-mode facts, and cursor expectation checks are consumed from input fact owners during the transitional text bridge |
...
|---|---|---|
| Mixed AST-like tree owner | Pergyra record/class/tagged-node owner plus traversal parity | The AST-text bridge now has typed line nodes with parent edges and coarse kind rows. `program_emit` and `function_emit` route declaration categories through owner-owned kind predicates, declaration collectors consume typed nodes for global env/prototype/struct/enum prepasses, runtime/header usage facts are derived from the node inventory through `ast_usage_owner.pgy` rather than whole-AST rescans, and marker checks plus function/return/enum/nominal/role/parameter/field payload reads are centralized behind `ast_text_inventory_owner.pgy` accessors. `stmt_emit` now consumes owner-owned `Let`, `Assign`, `Log`, `Return`, `Defer`, `ArrayPop`, `ArraySet`, `ArrayPush`, `Exit`, `Break`, `Continue`, `For`, `While`, `If`, and `Else`/`else if` statement facts from `ast_text_statement_owner.pgy` instead of splitting those AST lines locally, and parameter mode facts survive into codegen. This blocker remains active because `CodegenAstTextNode.text` is still a line-text payload inside the bridge owner; it closes only when owned typed/tagged AST data replaces line-text semantics. |
