...
|---|---|---|
| Mixed AST-like tree owner | Pergyra record/class/tagged-node owner plus traversal parity | The AST-text bridge now has typed line nodes and `program_emit` routes program-level declarations through them, but function/statement emitters still consume legacy projected text lines; this blocker closes only when owned typed/tagged AST data replaces text-line consumption. |
