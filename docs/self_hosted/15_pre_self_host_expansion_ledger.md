...
| Self-host typed AST-text bridge | `src/self_hosted/codegen/input/ast_text_inventory_owner.pgy` | component contract, real-source selfcheck, codegen parity | raw `pgy --ast` line splitting, `CodegenAstTextNode` inventory, indentation, blank-line filtering, `[export]` normalization, program-level typed routing, declaration collector prepasses, legacy projection, and cursor expectation checks are consumed from one owner during the transitional text bridge |
...
|---|---|---|
| Mixed AST-like tree owner | Pergyra record/class/tagged-node owner plus traversal parity | The AST-text bridge now has typed line nodes, `program_emit` routes program-level declarations through them, and declaration collectors consume them for global env/prototype/struct/enum prepasses, but function/statement emitters still consume legacy projected text lines; this blocker closes only when owned typed/tagged AST data replaces text-line consumption. |
