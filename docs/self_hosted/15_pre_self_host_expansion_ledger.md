...
| Remaining runtime helper spelling | named runtime ABI owners for math, file/argv, and `Option`/`Result` helper names | Collection and string/text call names are owned; the remaining direct helper spellings still let call-site drift reappear. |
