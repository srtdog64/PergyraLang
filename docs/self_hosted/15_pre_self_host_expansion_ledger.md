...
relational, additive, multiplicative, and index topology; migrated codegen recurses
over those handles and cannot invoke the legacy Boolean/operator scanners.
...
lack them. C/LLVM-built drivers are byte-identical across the 20-source/11-MIR
producer-first gate, including arithmetic precedence in initializer,
assignment, condition, return, and array-index positions. Index codegen consumes
the receiver/index child handles and collection runtime ABI owner without the
legacy index scanner. The row remains `BRIDGE` because
call/wrapper/unary internals, log/collection/auxiliary lanes, and initial
compact-tree arena construction remain text-backed.

| Blocker | Required owner | Why it matters |
|---|---|---|
| Mixed AST-like tree owner | Pergyra record/class/tagged-node owner plus traversal parity | Typed arena rows own parent/kind/name/type/value/aux/mode facts. Semantic signature, local, assignment, statement, iteration, and body owners consume those rows once. Codegen has no direct atom/type/value/aux/parameter payload view; it receives semantic owner rows and retains only structural/provenance arena traversal. Parser/HIR directly owns logical, equality, relational, additive, multiplicative, and index graph topology for `if`, `while`, `let`, assignment, and value return. DRV-2 carries those handles as instruction-owned `expr0_graph` MIR rows, and source/MIR hard consumers fail closed instead of reparsing expression text. Index codegen consumes receiver/index handles plus the collection runtime ABI owner without the legacy scanner. Call/wrapper/unary internals, log/collection/auxiliary lanes, and initial compact-tree arena construction remain text-backed, so the blocker remains active. |
