...
|---|---|---|
| Mixed AST-like tree owner | Pergyra record/class/tagged-node owner plus traversal parity | Typed arena rows own parent/kind/name/type/value/aux/mode facts. Semantic signature, local, assignment, statement, iteration, and body owners consume those rows once. Codegen has no direct atom/type/value/aux/parameter payload view; it receives semantic owner rows and retains only structural/provenance arena traversal. Semantic expression surfaces now own normalized top-level operator rows and the `Log`/role-operator consumer uses them, but value/auxiliary and recursive child-expression lowering still parse compact text. The blocker remains active until those consumers move to the same typed row/tree authority. |
