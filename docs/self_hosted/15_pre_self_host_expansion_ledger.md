...
direct-call, and call-argument topology; migrated codegen recurses over those
handles and cannot invoke the legacy Boolean/operator/call-argument scanners.
Unary and call-root rows have one edge. Call-argument rows carry the prior call
spine plus one argument; malformed shape fails verification.
...
construction. The row remains `BRIDGE` because qualified/member calls,
