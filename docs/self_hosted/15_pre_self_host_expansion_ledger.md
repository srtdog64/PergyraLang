...
Typed-expression delta, 2026-07-13 (condition graph): normalized condition rows
now carry stable expression node handles and child edges. `if`/`while` codegen
recurses over those handles and is gate-forbidden from invoking the legacy
boolean scanner or splitting payload text. A grouped logical fixture protects
precedence, and C/LLVM-built tools produce byte-identical C and equal output.
The parser precedence walk now produces these graph rows directly and carries
them in `AstTreeArtifact`; DRV-2 rejects a text-created artifact that lacks the
typed graph. The row remains `BRIDGE` because indexed/wrapper/auxiliary lanes,
non-condition recursive consumers, and initial compact-tree arena construction
remain text-backed.

| Blocker | Required owner | Why it matters |
|---|---|---|
| Mixed AST-like tree owner | Pergyra record/class/tagged-node owner plus traversal parity | Typed arena rows own parent/kind/name/type/value/aux/mode facts. Semantic signature, local, assignment, statement, iteration, and body owners consume those rows once. Codegen has no direct atom/type/value/aux/parameter payload view; it receives semantic owner rows and retains only structural/provenance arena traversal. Semantic expression surfaces own normalized top-level operator rows; `Log`, scalar/String returns, and ordinary scalar/String local/assignment consumers use them. The parser precedence walk directly owns `if`/`while` logical/equality graph production, DRV-2 carries those handles as instruction-owned `expr0_graph` MIR rows, and both source and MIR hard consumers fail closed instead of reparsing expression text. Indexed collection elements, Option/Result/struct wrapper internals, auxiliary lanes, non-condition recursive expression lowering, and initial compact-tree arena construction remain text-backed, so the blocker remains active. |
