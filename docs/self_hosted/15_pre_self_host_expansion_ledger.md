...
| JSON read/emit primitives | `src/self_hosted/lib/json.pgy` | component contract and real-source selfcheck | shared string/number/span reads plus JSON string escaping/literal emission for fact-shaped tools |
...
| Stable JSON parse/emit owner | schema-aware JSON reader/writer with diagnostics | Read primitives and string-literal emission are shared; schema validation, object/array iteration, and object/array writer ownership still need one owner. |
