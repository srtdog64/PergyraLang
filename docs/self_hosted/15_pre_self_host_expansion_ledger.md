...
| Direct MIR dual-backend projection (literal-Log subset) | `src/self_hosted/compiler/direct_mir_backend_projection_owner.pgy`, machine-admitted `pgy.mir.v1`, `CompilerTargetProjectionFact` | `self-host-one-mir-dual-backend-projection-test-smoke`, component contract | One Pergyra-produced MIR artifact directly projects to C and LLVM from one backend-neutral owner. The READY scope is exactly one `Main() -> Void`, one reachable block, and one graph-owned ASCII literal `Log`; missing/mutated facts and invalid targets fail before output. Parser, semantic, source-compile, and canonicalizer bridges are negative-gated. General expressions, declarations, CFG, ABI/runtime facts, AIR-certified projection/spawn/parallel/region plans, and default-driver promotion remain ACTIVE surfaces and are not implied by this row. |
