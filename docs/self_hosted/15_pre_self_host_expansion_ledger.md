...
| Direct MIR dual-backend projection (literal/local-add subset) | `direct_mir_scalar_graph_admission_owner.pgy`, `direct_mir_backend_projection_owner.pgy`, `routine_instruction_use_fact_owner.pgy`, machine-admitted `pgy.mir.v1`, `CompilerTargetProjectionFact` | `self-host-one-mir-dual-backend-projection-test-smoke`, component contract | One Pergyra-produced MIR artifact directly projects to C and LLVM through shared backend-neutral admission. The READY scope is one `Main() -> Void`, one reachable block, and either one graph-owned ASCII literal `Log` or exact `x:Int=21` plus `Log(ToString(x+x))`. Local/result/use, add edges, and direct-call target are typed facts; missing/mutated facts and invalid targets fail before output. Parser, semantic, raw `expr0`, source-compile, canonicalizer, and backend-specific MIR readers are negative-gated. General scalar blocks, declarations, CFG, complete ABI/runtime facts, AIR-certified projection/spawn/parallel/region plans, and default-driver promotion remain ACTIVE and are not implied by this row. |
