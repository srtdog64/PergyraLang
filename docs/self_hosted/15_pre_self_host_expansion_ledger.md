...
| Integrated driver bootstrap | `src/self_hosted/compiler/driver_rung2_owner.pgy`, `src/self_hosted/compiler/driver_bootstrap_main.pgy` | `self-host-driver-bootstrap-test-smoke` | the bounded source->shared AST artifact->semantic facts->MIR facts->C pipeline is built by the Pergyra-built codegen, matches the oracle on a real source, and reaches byte-identical `gen2 == gen3`; whole-compiler and released-default substitution remain open |
