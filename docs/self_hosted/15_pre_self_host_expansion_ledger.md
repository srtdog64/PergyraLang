...
`program_emit.pgy` is ratcheted against calling the raw-node bridge, and
`ast_usage_owner.pgy` is ratcheted against direct `nodes[i].payload` /
`nodes[i].aux_payload` scans. Builtin-name detection remains string-based over
