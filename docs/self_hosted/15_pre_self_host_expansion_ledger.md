...
| Self-host C symbol spelling | `src/self_hosted/compiler/symbol_table_owner.pgy` | component contract, real-source selfcheck, codegen parity | function/method/operator/enum names, namespace-qualified call spellings, and `inout` temporary parameter spellings are consumed from the compiler-world row owner inside the current self-host C subset |
