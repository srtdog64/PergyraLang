...
state is tracked by `CompilerLspDocumentStoreArtifactKind()`. Content-Length
transport, request-dispatch, response-emission, session-replay, and
document-store parity must not reuse or alias the LSP diagnostics artifact.
