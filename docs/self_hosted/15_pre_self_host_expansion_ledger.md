...
| Self-host typed AST-text bridge | `src/self_hosted/codegen/input/ast_text_inventory_owner.pgy` plus `pgy --ast` parameter-mode preservation | component contract, real-source selfcheck, parser parity, codegen parity | raw `pgy --ast` line splitting, `CodegenAstTextNode` inventory, indentation, marker-node predicates, blank-line filtering, `[export]` normalization, program-level typed routing, declaration collector prepasses, function/return/enum/nominal/role/parameter/field payload accessors, `Let`/`Assign`/simple statement fact accessors, `ArraySet`/`ArrayPush` collection mutation statement fact accessors, function signature/header facts, statement body reads, `inout`/`own`/`ref` parameter-mode facts, and cursor expectation checks are consumed from one owner during the transitional text bridge |
