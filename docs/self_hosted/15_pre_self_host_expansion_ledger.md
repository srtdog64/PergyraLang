...
| Stable JSON parse/emit owner | schema-aware JSON reader/writer with diagnostics | Read primitives plus string/field/object/array emission are shared, production size checkers and `stable_subset_section_checker` consume the object/array writer, and `air_graph_json_validator` consumes schema and numeric summary fields through `lib/json.pgy`. This blocker remains active because object/array iteration and a structured writer for every self-hosted report schema are not yet single-owner. |
