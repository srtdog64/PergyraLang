...
| Integrated driver bootstrap | `src/self_hosted/compiler/driver_pipeline_owner.pgy`, `src/self_hosted/compiler/driver_bootstrap_main.pgy` | `self-host-driver-bootstrap-test-smoke` | the source->shared AST artifact->semantic entrypoint verdict->C pipeline consumed by DRV-0/1 is built by the Pergyra-built codegen, matches the oracle on a real source, and reaches byte-identical `gen2 == gen3`; broader semantic/MIR inclusion remains open |
