...
| Cross-backend ABI/layout row projection | `src/self_hosted/compiler/abi_layout_row_owner.pgy` plus current C-subset `abi_layout_owner.pgy` | The compiler-world owner now carries concrete C ABI row projections for the supported self-host subset (`Int`, `Bool`, `Float`, `String`, `Array<Int>`, `Array<String>`, `Array<CodegenAstTextNode>`, `Result<Int>`, `Option<Int>`, `Option<String>`) plus materialization policy. The self-host C ABI owner consumes those rows first and only consults `TypeEnvZone` for user structs. This remains active until native C/LLVM and self-hosted consumers read the same concrete row table. |
