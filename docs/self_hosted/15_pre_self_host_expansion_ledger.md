...
| Self-host AST-text line inventory | `src/self_hosted/codegen/input/ast_text_inventory_owner.pgy` | component contract, real-source selfcheck, codegen parity | raw `pgy --ast` line splitting, indentation, blank-line filtering, `[export]` normalization, and cursor expectation checks are consumed from one owner during the transitional text bridge |
