...
construction. Simple `self.field` and `v.Method(arg)` emission consumes
receiver/member handles, method signature rows, and the explicit receiver
argument without reopening member, qualified-call, field-access, or
parenthesis scanners. The row remains `BRIDGE` because nested or
namespace-qualified member calls,
