...
| Stable JSON parse/emit owner | schema-aware JSON reader/writer with diagnostics | Read primitives plus string/field/object/array emission are shared, production size checkers, `stable_subset_section_checker`, and `module_manifest_resolver` consume the object/array writer, and AIR/module validators consume schema or top-level field checks through `lib/json.pgy`. This blocker remains active because object/array iteration and a structured writer for every self-hosted report schema are not yet single-owner. |
