...
| Target capability envelope | `target_capability_owner.pgy`, `TargetCapabilityZone` | `self-host-compiler-world-contract-test-smoke`, real-source selfcheck | CPU/C/LLVM/self-hosted projections now use an exact projection/fact/fallback schema before emission, so hidden CPU fallback and unsupported target lowering cannot pass by name presence alone |
