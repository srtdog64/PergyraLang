...
| Target capability envelope (self-host C subset) | `target_capability_owner.pgy`, `target_projection_fact_owner.pgy`, `target_capability_manifest.pgy`, `TargetCapabilityZone` | `self-host-compiler-world-contract-test-smoke`, `self-host-target-capability-envelope-parity-test-smoke`, DRV-2 target-projection negative, real-source selfcheck | CPU/C/LLVM/self-hosted projection rows, target facts, and fallback reasons are named owner facts. `CompilerTargetCapabilityEnvelopeReady()` consumes those facts instead of comparing row indexes to string literals, and the runnable manifest emits a stable target-capability artifact with a missing-fact fail-closed case across C/LLVM-built self-host tools. The active DRV-2 path derives `CompilerTargetProjectionFact`, carries it into `CompilerEmissionArtifact`, and requires the final C emitter to validate the exact `cpu-c` projection; C-built and LLVM-built drivers reject an empty projection fact. This is projection carriage, not physical target closure: native fingerprint, concrete size/align/endian, AIR binding, and non-C projection values remain bridge work. |
| Direct MIR dual-backend projection (literal/typed scalar-block subset) | `direct_mir_scalar_graph_admission_owner.pgy`, `direct_mir_backend_projection_owner.pgy`, `routine_instruction_use_fact_owner.pgy`, `ssa_identity_owner.pgy`, machine-admitted `MirDocumentFactIndex` / `pgy.mir.v1`, typed formatted-print runtime ABI fact, `CompilerTargetProjectionFact` | `self-host-one-mir-dual-backend-projection-test-smoke`, component contract | One Pergyra-produced MIR artifact directly projects to C and LLVM through shared backend-neutral admission. The READY scope is one `Main() -> Void`, one reachable block, and either one graph-owned ASCII literal `Log` or N typed `Int` definitions followed by N graph-owned `Log(ToString(binary))` statements using add/multiply. Local/result/use/SSA identity, instruction order, graph edges, direct-call target, formatted-print symbol/call shape, line format, and C `Int` spelling are owner facts. Admission reuses the machine-carried document index and validates the exact graph/node schema in the same sequence pass. Missing/mutated facts and invalid targets fail before output. Parser, semantic, raw `expr0`, document reindexing, source-compile, canonicalizer, hardcoded runtime/ABI spelling, and backend-specific MIR readers are negative-gated. Declarations, CFG, MIR-bound AIR certification, verified projection/spawn/parallel/region plans, and default-driver promotion remain ACTIVE and are not implied by this row. |
...
| Self-host C symbol spelling | `src/self_hosted/compiler/symbol_table_owner.pgy`, `src/self_hosted/codegen/type_facts/type_env.pgy`, semantic graph emitters | component contract, real-source selfcheck, codegen parity | function/method/operator/enum names, namespace-qualified call spellings, struct field spellings for declarations, literals, and member access, source-to-C binding spellings, `inout` temporary parameter spellings, foreach loop temporary spellings, and try/match emission temporary spellings are consumed from owner rows inside the current self-host C subset; declarations append `cbind` rows and semantic graph emitters consume them directly, with `c_reserved_binding` proving non-identical source/C local spelling. The former expression binding rewrite owner is deleted and gated against return. |
...
| Self-host shared AST artifact | `src/self_hosted/parser/ast_text_artifact_parse_owner.pgy`, `src/self_hosted/hir/ast_text_scan_owner.pgy`, `ast_text_inventory_owner.pgy`, `ast_text_row_fact_owner.pgy`, `ast_text_arena_projection_owner.pgy`, and `typed_ast_arena_owner.pgy`; semantic/codegen views consume the completed artifact | component contract, parser/codegen driver bootstrap, expression-surface closed-owner gate, parser parity, codegen parity, compiler-world contract | parser constructs one `AstTreeArtifact` with compact provenance, shared arena, expression graph, and node count; semantic analysis and codegen consume that artifact without rebuilding expression topology from payload text. Missing or malformed graph ownership fails closed, and the deleted compact-text bridge owners are negative-ratcheted. |
...
| Self-host C Option/Result runtime symbols | `src/self_hosted/codegen/runtime_abi/option_result_runtime_owner.pgy` | component contract, real-source selfcheck, codegen parity | `Option<Int>` / `Option<Long>` / `Option<String>` / `Option<Float>` / `Option<Double>` / `Result<Int>` helper call names and scalar option typedefs are consumed from one owner inside the current self-host C subset. `Option<Int>` and `Option<Long>` now retain distinct `int32_t`/`int64_t` payload carriers rather than sharing one widened option lane. |
...
| DRV-0 artifact gate | `src/self_hosted/compiler/driver_rung0_owner.pgy`, `src/self_hosted/compiler/driver_rung0_main.pgy` | `self-host-driver-rung0-parity-test-smoke` | source path -> parser-owned typed artifact -> semantic facts -> emitted C is assembled in one Pergyra owner boundary. AST text remains compared with the self-parser, while emitted C is compared across C/LLVM-built in-process drivers. The retired text-only standalone-codegen oracle may not discard parser-owned expression or match-pattern graphs. Fixture identity remains owned by `codegen/fixture_manifest_owner.pgy`. |
| Integrated driver bootstrap | `src/self_hosted/compiler/driver_rung2_owner.pgy`, `src/self_hosted/compiler/driver_bootstrap_main.pgy` | blocking `self-host-driver-bootstrap-test-smoke`; explicit `self-host-driver-bootstrap-full-test-smoke` | the bounded source->shared AST artifact->semantic facts->MIR facts->C pipeline is built by the Pergyra-built codegen. Blocking CI compares the Pergyra-built integrated seed with the native-built same driver on a real source, then requires both builds to produce byte-identical verified MIR for one bounded sample and consume the common fact to byte-identical C. The explicit full gate builds full-input `gen2` from the native-built same driver's verified MIR artifact and extends it to byte-identical `gen2 == gen3`; bounded DRV-2 fixtures own self-host MIR-producer parity and whole-compiler substitution remains open |
| Artifact Zone evidence | `src/self_hosted/compiler/artifact_zone_owner.pgy`, `ArtifactZone` | `self-host-component-contract-test-smoke`, `artifact-zone-plan-identity-test-smoke`, parity artifact gates | Comparable diagnostics, LSP, AST text, AIR/MIR JSON, ABI/layout, runtime-call ABI, materialization, emitted C/LLVM/self-hosted, run-output, and run-group plan artifacts now route equality verdicts through `backend_output_comparator` with explicit artifact kinds. Native C/LLVM `CompilerResult` rows also carry the verified projection plan revision/digest and runners fail closed when absent. Consumer parity scripts must not recompute artifact equality in shell; the comparator's own parity rung is limited to expected-JSON bootstrap comparison plus mismatch and missing-input fixtures. |
...
arena. Temporary `CodegenAstTextNode` rows are construction inputs only and do
not cross the artifact boundary. Function signatures and local
declaration/name/type/scope rows now consume artifact-native semantic fact
...
and auxiliary lanes. The first live consumer is `Log`: statement emission
looks up the atom row by `SyntaxNodeId`, and semantic-shape emission consumes
the stored additive index and operator kind instead of calling
`FindTopLevelPlus`. The role fixture proves `+` dispatch and `-` non-dispatch
under C-built and LLVM-built self-host tools.
The blocker remains open because value/auxiliary consumers and recursively
split child expressions still use compact text.

Typed-expression delta, 2026-07-13 (second consumer set): scalar and String
returns consume atom-lane rows; ordinary scalar/String local initializers and
assignments consume value-lane rows. C/LLVM parity covers integer and String
returns, Int/Float initializers, Long assignment, role dispatch from `return`,
and `-` non-dispatch. Indexed collection elements, Option/Result/struct wrapper
...
Typed-expression delta, 2026-07-13 (statement expression graphs): normalized
rows now carry stable expression node handles and child edges for `if`, `while`,
`let`, assignment, and value return. Parser precedence owns logical, equality,
relational, additive, multiplicative, index, logical-not, numeric-negate,
member-access, direct-call, and call-argument topology; migrated codegen
recurses over those handles and cannot invoke the legacy
Boolean/operator/call-argument scanners. Unary and call-root rows have one
edge. Member-access rows carry receiver/member edges. Call-argument rows carry
...
instructions that lack them. The inventory contains 20 source and 61 MIR
fixtures. The previous 41-row frontier is joined by 18 declaration, arithmetic,
call, branch, reassignment, and loop rows that passed focused canonical-MIR and
runtime parity under C-built and LLVM-built self drivers. `break_after_stmt`
then became row 60 after the MIR-to-tree consumer used CFG arm-successor facts
to close its canonical fixed point. `nested_if_in_loop` then became row 61
after a typed control-flow fact owner separated direct backedge reachability
from ordinary fallthrough and the producer checked that plan against the final
CFG. The complete 61-row matrix was not rerun in these slices.
`array_destructure` remains excluded by a static ratchet until typed-binding
ownership is complete.
Index codegen consumes
the receiver/index child handles, graph-derived receiver type, and collection
runtime ABI owner without the legacy index scanner or member-text type lookup.
Logical-not and numeric-negate emit from the operand
handle without reparsing the unary root. Direct identifier calls consume the
call spine for argument order, parameter mode, runtime ABI alias, and struct
construction. Simple `self.field`, recursively nested `v.inner.field`, and
`v.Method(arg)` emission consume receiver/member handles, field/type rows,
method signature rows, and the explicit receiver argument without reopening
...
before consuming the method signature. Payload-free enum expected values
consume the parser-owned leaf/member graph and exact enum row; changing only
the member child while retaining root provenance fails closed. Nested member-
array indexing remains correct when only member-node provenance text is stale,
proving child handles and type rows are the last consumer. The row remains
`BRIDGE` because object-init internals, remaining literal-only argument
bridges, borrow/receive/spawn unary forms, expression result-type
classification, and initial compact-tree arena construction remain text-backed.
...
the graph consumer. Its hard emitter cannot call the former ToString prefix,
substring, or semantic-shape paths. The first parity run exposed that rich
...
|---|---|---|
| Mixed AST-like tree owner | Pergyra record/class/tagged-node owner plus traversal parity | Typed arena rows own parent/kind/name/type/value/aux/mode facts. Semantic signature, local, assignment, statement, iteration, and body owners consume those rows once. Codegen has no direct atom/type/value/aux/parameter payload view; it receives semantic owner rows and retains only structural/provenance arena traversal. Parser/HIR directly owns numeric, Bool, and String literal identity plus logical, equality, relational, additive, multiplicative, index, logical-not, numeric-negate, await, member-access, direct-call, call-argument, pipe-call, postfix-try, array-literal, struct-literal, and collection-value graph topology for `if`, `while`, `let`, assignment, value return, Log, match scrutinee, `ArrayPush`, and `ArraySet` lanes. Unary and call-root rows carry one edge; member-access rows carry receiver/member edges; call-argument rows carry the prior call spine plus one argument and fail malformed-shape verification. DRV-2 carries those handles as instruction-owned `expr0_graph` MIR rows, and source/MIR hard consumers fail closed instead of reparsing expression text. Index reads consume receiver/index handles plus the collection runtime ABI owner; indexed assignment and mutators render Int, String, and struct values through one expected-type graph consumer; logical-not, numeric-negate, and await emit from their operand handle; direct identifier calls consume graph-owned argument order, parameter modes, runtime aliases, and struct construction. Indexed assignment also carries its target/index tree in the auxiliary lane as `expr1_graph`; hard emission consumes receiver/index handles and rejects a missing target graph. Initializer and assignment typing derive binding/member/index/call scalar types from parser graph handles and are ratcheted against legacy projection; the unreferenced projection owner has been deleted. Array-literal initializer typing and codegen share one semantic-owned array-spine view and ordered element handles; the initializer cannot trim brackets or split element text. Payload-free enum expected values consume member handles and exact enum rows; graph-only member drift fails even when root provenance is unchanged. Match, initializer, and assignment typing reject graph-only member drift without recovering source text. Simple and recursively nested field reads consume receiver/member handles and field/type rows; simple and nested-receiver instance method calls consume receiver/member handles, recursive receiver type facts, and signature rows without legacy member scanners. Log emission consumes its parser-owned argument subtree without prefix/substring/semantic-shape recovery. Namespace-qualified call classification, object-init internals, remaining literal-only argument bridges, borrow/receive/spawn unary forms, and initial compact-tree arena construction remain text-backed, so the blocker remains active. |
...
| Target capability envelope (native/global consumers) | `target_capability_owner.pgy`, `TargetCapabilityZone` | The compiler-owned native projection planner is now the single consumer of the adapter envelope; C/LLVM receive only its fingerprinted plan row. AIR-overlapping target fact names remain target-envelope facts until the import/idempotence contract is checked and the target owner can consume `air_evidence_owner.pgy` directly. |
...
TestHarness delta, 2026-07-10: `CompilerTestHarnessReady()` now consumes
core harness rows, projections, and comparable artifact paths through named
membership (`CompilerHarnessRowKnown`, `CompilerHarnessProjectionKnown`, and
`CompilerHarnessComparableArtifactPathKnown`) instead of representative
...
TestHarness delta, 2026-07-10: compatibility-corpus and
compatibility-evolution path suites moved to
`test_harness_compatibility_paths_owner.pgy` and now consume named path membership
(`CompilerHarnessCompatibilityCorpusPathKnown` and
`CompilerHarnessCompatibilityEvolutionPathKnown`) plus out-of-range boundary
checks instead of `PathCount() == N` and representative `PathAt(0..N)`
readiness. Ordered path rows remain the stable manifest artifact shape only.
...
TestHarness delta, 2026-07-10: SEA execution-lane parity paths moved to
`test_harness_execution_lane_paths_owner.pgy` and now consume named path
membership (`CompilerHarnessExecutionLanePathKnown`) plus an out-of-range
boundary check instead of `PathCount() == 5` and representative
`PathAt(0..4)` readiness. Ordered path rows remain the stable manifest artifact
shape only.
...
`hir/ast_text_arena_projection_owner.pgy` instead of direct raw `nodes[i].indent`
...
it cannot reconstruct a missing initializer from `AstArena`. Initializer and
assignment expression typing now have DRV-2 hard verdict owners. Assignment
target/base/index/RHS rows are owned by
`semantic/ast_assignment_fact_owner.pgy`; both earlier codegen assignment
owners are retired and ratcheted against return.
...
artifact-bound function owner/name, parameter node/name/type/mode, and return
...
range start/end, foreach collection, body scope, function, and lexical scope
rows once. `semantic_statement_codegen_view_owner.pgy` is the fail-closed
projection consumed by `stmt_emit.pgy`; `ast_text_for_stmt_owner.pgy` is
deleted and the component contract rejects its return.

TypedAst delta, superseded 2026-07-10: single-payload statement facts moved out
of codegen. `semantic/ast_statement_fact_owner.pgy` owns `Log`, bare/value
`Return`, `ArrayPop`, `Exit`, `While`, `If`, `Match`, match case, and bare-call
node/function/scope/payload rows. `semantic_statement_codegen_view_owner.pgy`
...
TypedAst delta, 2026-07-09: `ast_kind_usage_owner.pgy` now owns the remaining
runtime/header statement-shape usage rows (`Log`, `Exit`, array literal,
`ArrayPush`, and `ArraySet`) derived from typed arena kind rows.
`ast_usage_owner.pgy` consumes `CodegenKindUsageFactsFromArena(...)` and is
forbidden by the component contract from calling `CodegenAstArenaKindPresent`
...
`driver_pipeline_owner.pgy` and `driver_bootstrap_main.pgy` initially raised
...
parser/codegen driver at a 14,659-line `gen2 == gen3` fixed point. This does not
close semantic/MIR inclusion or released-driver substitution.

Shared semantic artifact delta, 2026-07-10: HIR node-kind tags now have one
owner consumed by inventory and codegen views. The integrated driver passes the
parser-owned artifact through an arena-backed semantic entrypoint verdict before
emission, and the refreshed fixed point is byte-identical `gen2 == gen3` at
16,881 generated-C lines. The four new HIR/semantic owners passed focused
lexer/parser/semantic/codegen checks 4/4, raising all M2 minima to 225. Broader
semantic rules and MIR lowering remain outside the executable.
...
Artifact-native signature delta, 2026-07-10: function signatures moved from a
codegen-owned arena scan into `SemanticAstArtifactAnalysis`. The same rows feed
function definitions, prototypes, role-operator lookup, and the C type
environment. Focused completeness passed the two new/replacement owners 2/2
through all four stages and measured 226 production sources. Semantic parity
remains 110/110, codegen parity remains 68/68, and the integrated driver fixed
point is `gen2 == gen3` at 17,304 generated-C lines. Local/body semantics and
MIR remain outside this executable and keep the blocker ACTIVE.

Artifact-native local-binding delta, 2026-07-10: `Let` declaration owner,
enclosing function, lexical scope, name, and optional declared type moved into
`SemanticAstArtifactAnalysis`. The codegen-local compatibility owner was
deleted; emission consumes the semantic rows and fails closed when this bounded
codegen slice has no declared type. Focused completeness passed all four changed
owners through all four stages and measured 228 production sources. Semantic
parity remains 110/110, codegen parity remains 68/68, and the integrated driver
...
The production inventory is 240, with no alias restored: the real semantic
...
verdict. `SemanticAstBodyVerdict` selects the first failing artifact node across
...
`source_pattern`, filter env/stage, and `proof_gate` row fields, emits
...
env/value rows without reconstructing the impact decision in shell. The
planner now also exposes the same groups through `--run-groups` as
...
TestHarness delta, 2026-07-10: `driver_rung0_owner.pgy` no longer owns a
three-sample driver fixture list. It consumes `CodegenParityFixtureManifestRows`
from `codegen/fixture_manifest_owner.pgy`, projects those base names into source
paths, and ratchets the driver fixture frontier to the 69 committed codegen
parity fixtures. DRV-0 and DRV-1 shell runners reject manifest count drift.
