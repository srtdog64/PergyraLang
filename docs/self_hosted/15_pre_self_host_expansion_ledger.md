...
|---|---|---|
| Mixed AST-like tree owner | Pergyra record/class/tagged-node owner plus traversal parity | Typed arena rows own parent/kind/name/type/value/aux/mode facts. Semantic signature, local, assignment, statement, iteration, and body owners consume those rows once; codegen receives fail-closed views and no longer owns `For` payload reconstruction. Expression usage still derives from transitional arena text, so the blocker remains active until typed expression rows replace that bridge. |
