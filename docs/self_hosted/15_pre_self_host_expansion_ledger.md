...
| Stable JSON parse/emit owner | schema-aware JSON reader/writer with diagnostics | Read primitives plus string/field/object/array emission are shared, all current self-hosted report schemas consume the object/array writer, AIR/module validators consume schema or top-level field checks through `lib/json.pgy`, `module_manifest_resolver` consumes top-level array-object row bounds, and `mir_lower` declaration/routine header/program assembly/routine CFG/source-local/statement-array/match-pattern lowering consumes MIR row/object/string/array facts through `json_fact_read.pgy` and `routine_inventory_owner.pgy` instead of local field-key scans. This blocker remains active because the owner is still a bounded schema scanner rather than a complete JSON DOM/fact table. |
