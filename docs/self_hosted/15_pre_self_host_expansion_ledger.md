...
| Self-host shared AST artifact | `src/self_hosted/hir/ast_text_scan_owner.pgy`, `ast_text_inventory_owner.pgy`, `ast_text_row_fact_owner.pgy`, `ast_text_arena_projection_owner.pgy`, and `typed_ast_arena_owner.pgy`; codegen-only views remain in `src/self_hosted/codegen/input/ast_arena_codegen_view_owner.pgy` | component contract, parser/codegen driver bootstrap, parser parity, codegen parity, compiler-world contract | parser constructs one `AstTreeArtifact` with compact provenance, shared arena, and node count; codegen consumes the same fact without rebuilding it; temporary `CodegenAstTextNode` rows are construction-only; expression facts remain compact-text backed and semantic artifact consumption is still required |
