...
DRV-2 carries required roots through branch, definition, return, and Log
`expr0_graph` rows and rejects text-created artifacts or MIR instructions that
lack them. C/LLVM-built drivers are byte-identical across the 20-source/13-MIR
producer-first gate, including arithmetic precedence in initializer,
assignment, condition, return, array-index, and unary positions. Index codegen consumes
...
member, qualified-call, field-access, or parenthesis scanners. The row remains
`BRIDGE` because nested-receiver or namespace-qualified member calls,
object-init internals, literal-only argument bridges,
borrow/receive/spawn/await unary forms, collection/auxiliary lanes, and initial
...
|---|---|---|
| Mixed AST-like tree owner | Pergyra record/class/tagged-node owner plus traversal parity | Typed arena rows own parent/kind/name/type/value/aux/mode facts. Semantic signature, local, assignment, statement, iteration, and body owners consume those rows once. Codegen has no direct atom/type/value/aux/parameter payload view; it receives semantic owner rows and retains only structural/provenance arena traversal. Parser/HIR directly owns logical, equality, relational, additive, multiplicative, index, logical-not, numeric-negate, member-access, direct-call, call-argument, pipe-call, and postfix-try graph topology for `if`, `while`, `let`, assignment, value return, and Log argument lanes. Unary and call-root rows carry one edge; member-access rows carry receiver/member edges; call-argument rows carry the prior call spine plus one argument and fail malformed-shape verification. DRV-2 carries those handles as instruction-owned `expr0_graph` MIR rows, and source/MIR hard consumers fail closed instead of reparsing expression text. Index codegen consumes receiver/index handles plus the collection runtime ABI owner without the legacy scanner; logical-not and numeric-negate emit from their operand handle; direct identifier calls consume graph-owned argument order, parameter modes, runtime aliases, and struct construction. Simple and recursively nested field reads consume receiver/member handles and field/type rows; instance method calls consume receiver/member handles and signature rows without legacy member scanners. Log emission consumes its parser-owned argument subtree without prefix/substring/semantic-shape recovery. Nested-receiver or namespace-qualified member calls, object-init internals, literal-only argument bridges, borrow/receive/spawn/await unary forms, collection/auxiliary lanes, expression result-type classification, and initial compact-tree arena construction remain text-backed, so the blocker remains active. |
