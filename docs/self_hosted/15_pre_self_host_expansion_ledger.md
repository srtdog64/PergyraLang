...
|---|---|---|
| Mixed AST-like tree owner | Pergyra record/class/tagged-node owner plus traversal parity | The AST-text bridge now has typed line nodes, but codegen emitters still consume legacy projected text lines; this blocker closes only when owned typed/tagged AST data replaces text-line consumption. |
