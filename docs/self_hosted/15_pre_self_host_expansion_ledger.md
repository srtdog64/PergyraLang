...
| Stable JSON parse/emit owner | schema-aware JSON reader/writer with diagnostics | Read primitives plus string/field/object/array emission are shared, production size checkers, `stable_subset_section_checker`, `backend_output_comparator`, `doc_link_checker`, `examples_inventory_checker`, `stdlib_dispatch_inventory_checker`, AIR graph checkers, and `module_manifest_resolver` consume the object/array writer, AIR/module validators consume schema or top-level field checks through `lib/json.pgy`, and `module_manifest_resolver` now counts module objects/fields through bounded JSON array-object traversal. This blocker remains active because full object/array iteration and every remaining self-hosted report schema are not yet single-owner. |
