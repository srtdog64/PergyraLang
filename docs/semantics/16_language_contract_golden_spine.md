...
| Logical operators | logical operators produce Bool and require Bool operands. C-style Int logical values are not the stable language contract. | `src/self_hosted/parity/semantic_parity.sh`; `src/self_hosted/semantic/expected/bad_logical_*.diag` |
...
| Proof spine | The proof pack is connected through named proof nodes. Runtime safety, axis ownership, intent core, unified machine, certificate pipeline, and verification methodology can be cited as connected groups only through the spine; even a complete spine is not whole-language verification. | `make proof-spine-test-smoke`; `docs/semantics/proofs/ProofSpine.v`; `docs/semantics/proofs/ProofSpine.md` |
| Machine-neutral compute | C and LLVM are first validation projections, not the language ontology. Future dataflow, actor, tensor, capability, reconfigurable, and event-driven backends must consume the same AIR/MIR/ABI owner facts or fail closed. | `docs/semantics/18_machine_neutral_compute.md`; `make language-contract-golden-test-smoke` |
