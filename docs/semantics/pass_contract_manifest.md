...
dir_zone_runtime_topology | src/compiler/dir_collect_domain.c | tests/domain_runtime_topology_smoke.sh | gate-backed-partial | DIR_domain_graph_id,source_program_SyntaxNodeId,owner_and_directive_SyntaxNodeId,stable_projection_layer_endpoint_slot_identity | MIR_domain_topology_carrier,target_neutral_zone_frontier_graph,C_LLVM_graph_parity | backend_AST_zone_frontier_graph,name_only_slot_join,wrong_source_DIR_binding | MIR domain lowering requires DIR-owned topology facts; MIR lowering received DIR facts from a different source program; MIR domain topology row has incomplete identity or shape | unowned_ast_rescan,backend_ast_semantic_read,compat_success_without_fact
