...
mir_decl_bootstrap_parity | src/codegen/llvm_decl.c | tests/mir_declaration_inventory_smoke.sh,tests/mir_decl_field_kind_vocabulary_smoke.sh,tests/self_hosted/parity/driver_rung2_effect_declaration_parity_owner.sh | gate-backed | MIR_routine_signature, MIR_decl_headers, nominal_kind, nominal_field_kind, ordered_intent_bindings, hosted_field_views, ability_ref_metadata | C_LLVM_signature_parity, declaration_inventory_parity, field_kind_registry_projection, exact_domain_slot_reconstruction, generic_default_actuals | AST_signature_reconstruction, AST_binding_order_reconstruction, field_name_or_type_as_field_kind | silent i32 fallback is not allowed; MIR-only path missing metadata; nominal declaration participant shape is invalid | backend_ast_semantic_read, consumer_local_field_kind_table, compat_success_without_fact
...
dir_zone_runtime_topology | src/compiler/dir_collect_domain.c | tests/domain_runtime_topology_smoke.sh,tests/self_hosted/parity/domain_topology_admission_owner.sh | gate-backed-partial | DIR_domain_graph_id,source_program_SyntaxNodeId,owner_and_directive_SyntaxNodeId,stable_projection_layer_endpoint_slot_identity | MIR_domain_topology_carrier,pgy_mir_v1_relation_and_domain_topology,selfhost_typed_domain_topology_admission,target_neutral_zone_frontier_graph,C_LLVM_graph_parity | backend_AST_zone_frontier_graph,name_only_slot_join,wrong_source_DIR_binding,selfhost_source_or_AST_topology_recovery | MIR domain lowering requires DIR-owned topology facts; MIR lowering received DIR facts from a different source program; MIR domain topology row has incomplete identity or shape; MIR domain topology facts are missing or invalid | unowned_ast_rescan,backend_ast_semantic_read,compat_success_without_fact
