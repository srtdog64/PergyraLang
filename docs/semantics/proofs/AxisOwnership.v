...
(* model's ap_axis), and SS10/SS11 are mechanized. What is left is per-call-site:*)
(* extract the provider/kind actually passed at each air_evidence_*.c call site  *)
(* and check each is WellAttributed (provider = owning axis), so the refinement  *)
(* constrains every real write and not just the entry-point shape. A full        *)
(* operational semantics of the verifier graph (beyond single appends) remains   *)
(* future work.                                                                  *)
(* ========================================== *)
