...
selfhost.expression_surface | semantic | SyntaxNodeId | SFExpressionSurface | SOSemanticExpressionSurface | src/self_hosted/semantic/ast_expression_surface_fact_owner.pgy | SemanticAstExpressionSurfaceFacts | src/self_hosted/codegen/input/ast_expression_usage_owner.pgy,src/self_hosted/codegen/input/ast_usage_owner.pgy,src/self_hosted/codegen/input/semantic_expression_codegen_view_owner.pgy,src/self_hosted/codegen/emission/expr_semantic_shape_emit_owner.pgy,src/self_hosted/codegen/emission/expr_semantic_graph_emit_owner.pgy | TypedAstArenaAtomText,TypedAstArenaValueText,TypedAstArenaAuxValueText,ContainsCallOutsideStrings,CodegenExpressionUsageFactsFromArena,CodegenAstArenaExpressionPartsAt,FindTopLevelPlus,RewriteBool,RewriteIndexing,RewriteInoutCallArgs,ExprSequenceItemAt | tests/self_hosted_component_contract_smoke.sh#compiler-stage contracts ok | BRIDGE | logical_equality_relational_arithmetic_index_logical_not_numeric_negate_direct_identifier_call_simple_and_nested_member_field_method_namespace_qualified_call_log_argument_pipe_call_postfix_try_for_range_identifier_foreach_and_payload_free_enum_argument_graphs_are_parser_owned_but_object_init_special_unary_array_struct_literal_argument_nonidentifier_foreach_type_classification_and_initial_compact_tree_arena_construction_remain_text_backed
...
closes array-literal initializer body ownership only; other expression shapes
remain under `selfhost.expression_surface`. The try row closes postfix-try
