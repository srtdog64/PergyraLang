...
```
semantic.builtin_effect_policy | semantic | BuiltinFixedEffectId | SFBuiltinEffectPolicy | SOBuiltinEffectPolicy | src/semantic/builtin_effect_registry.def | PGY_BUILTIN_EFFECT_POLICY_ROWS | src/semantic/type_checker_helpers_effects.c,src/self_hosted/semantic/builtin_effect_projection_owner.pgy,src/self_hosted/semantic/ast_capability_call_graph_owner.pgy | capability_bits_as_effects,missing_fixed_effect_as_pure,renderer_only_effect_admission,scalar_callee_mask_as_complete_effects | tests/concept_semantics/effect_admission.sh#owned refusal,tests/builtin_effect_registry_smoke.sh#fixed builtin effect registry smoke: ok | ACTIVE | Fixed intrinsic effects share one declaration and invocation graph, including scalar/string/math, task cancellation/observation, quantum measurement, non-callback collection and HashMap dispatch. Effect rows do not assert signature or backend availability. IsCancelled independently reaches installed public C/LLVM through the task runtime-call ABI owner and its focused execution/negative gate; that backend fact is not inferred from this effect row. Operand-dependent resource and callback builtin effects remain in their existing native owners and public call facts carry explicit unknowns. Known call effects are lower bounds rather than whole-language purity proofs.
...
Current status counts: `CLOSED=64 BRIDGE=28 ACTIVE=3`.
