...
semantic.builtin_capability_policy | semantic | BuiltinCapabilityPolicyId | SFBuiltinCapabilityPolicy | SOBuiltinCapabilityPolicy | src/semantic/builtin_capability_registry.def | PGY_BUILTIN_CAPABILITY_POLICY_ROWS | src/semantic/capability_analyze.c,src/semantic/type_checker_program.c,src/self_hosted/semantic/builtin_capability_projection_owner.pgy,src/self_hosted/semantic/ast_capability_fact_owner.pgy | native_builtin_capability_switch,selfhost_builtin_capability_switch,manifest_builtin_name_rescan,missing_builtin_capability_registry_success | tests/builtin_capability_registry_smoke.sh#builtin capability registry smoke: ok | CLOSED | none
...
Current status counts: `CLOSED=49 BRIDGE=36 ACTIVE=1`.
...
must preserve all prefix `call_target_syntax_ids`, `binding_kinds`, and
`binding_ordinals`, append Unknown identities only for newly parsed nodes, and
