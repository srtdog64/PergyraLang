...
resource.region_allocation_plan | resource | AllocationSiteId | SFRegionAllocationPlan | SORegionPlan | src/compiler/verified_region_plan.c | pgy_verified_region_plan_build_rows | src/compiler/driver_app.c,src/codegen/transpiler_context.c,src/codegen/llvm_expr_scalar_core.c,src/self_hosted/compiler/region_plan_owner.pgy | backend_ast_region_safety_guess,allocation_site_pointer_identity,missing_region_plan_success | tests/abi_ownership_shape_smoke.sh#allocation_site_id,tests/region_plan_unit.c#pgy_verified_region_plan_lookup( | BRIDGE | driver-owned stable AllocationSiteId rows are published and C/LLVM/self-host consumers validate them; the escape classifier is still a bounded AST-owned prototype using textual Print/PrintLn identity, and full semantic/HIR/MIR callee/retention facts remain open
...
Current status counts: `CLOSED=22 BRIDGE=20 ACTIVE=0`.
