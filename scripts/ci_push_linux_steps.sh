...
if [[ "$PGY_CI_PUSH_LINUX_RUN_CORE" == "1" ]]; then
    ...
    run 'bash tests/self_hosted/parity/builtin_name_shadow_owner.sh'
    run 'bash tests/self_hosted/parity/text_conversion_argument_owner.sh'
    run 'bash tests/self_hosted/parity/default_route_scalar_value_owner.sh'
    ...
    run 'bash tests/self_hosted/parity/containment_embedding_copy_owner.sh'
    ...
    run 'make generic-falsification-test-smoke'
