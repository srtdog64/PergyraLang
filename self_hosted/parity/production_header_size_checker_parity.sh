...
# (self_hosted/tools/production_header_size_checker/main.pgy).
...
PERGYRA_TOOL_SOURCE="$ROOT_DIR/self_hosted/tools/production_header_size_checker/main.pgy"
PERGYRA_TOOL_BUILD_DIR="${PGY_SELFHOST_BUILD_DIR:-$ROOT_DIR/.tmp/self_hosted/production_header_size_checker}"
PERGYRA_TOOL="$PERGYRA_TOOL_BUILD_DIR/main.pgy"
EXPECTED_JSON_FILE="$ROOT_DIR/self_hosted/tools/production_header_size_checker/expected/clean.json"
MANIFEST_FILE="$ROOT_DIR/self_hosted/tools/production_header_size_checker/fixture/headers_manifest.txt"
MANIFEST_REL="self_hosted/tools/production_header_size_checker/fixture/headers_manifest.txt"
...
{
    cat "$MANIFEST_FILE"
    echo "src/runtime/pgy_runtime_synthetic_drift.h"
} > "$NEG_ROOT/$MANIFEST_REL"
# Also mirror the live header tree so the manifest entries resolve.
while IFS= read -r path; do
    [[ -n "$path" ]] || continue
