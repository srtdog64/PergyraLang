                                    ...
                                    const char *callee_name, LLVMValueRef *out)
{
    ...
    for (size_t i = 0; i < sizeof(builtins) / sizeof(builtins[0]); i++) {
        const LLVMIntentObservabilityBuiltin *builtin = &builtins[i];
        LLVMFuncEntry *fn;
        if (strcmp(callee_name, builtin->name) != 0
            || node->data.call.arg_count != builtin->arg_count)
            continue;

        ctx->uses_intent_observability = true;
        fn = llvm_lookup_function(ctx, builtin->runtime_name);
        if (fn == NULL)
            return false;
