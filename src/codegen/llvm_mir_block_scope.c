                                    ...
                                    bool required)
{
    ...
    if (!llvm_mir_base_name_from_versioned(versioned_name, base_name,
        ...
        if (required)
            llvm_set_mir_inventory_missing(ctx,
                "LLVM MIR use '%s' has no valid SSA identity", versioned_name);
        return;
    }
    ...
    if (entry == NULL || entry->alloca == NULL || entry->type == NULL) {
        ...
        if (required)
            llvm_set_mir_inventory_missing(ctx,
                "LLVM MIR use '%s' is missing exact SSA storage", versioned_name);
        return;
    }
                                ...
                                size_t var_count)
{
    ...
    for (size_t i = 0; i < mir_block->ssa_entry_value_count; i++) {
        llvm_mir_bind_versioned_local_scope(ctx, vars, var_count,
            mir_block->ssa_entry_values[i], NULL, false);
    }
    for (size_t i = 0; i < mir_block->live_in_name_count; i++) {
        llvm_mir_bind_versioned_local_scope(ctx, vars, var_count,
            mir_block->live_in_names[i], NULL, false);
    }
}
                              ...
                              size_t var_count)
{
    ...
    for (size_t i = 0; i < mir_block->instruction_count; i++) {
        ...
        if (inst->kind == MIR_INST_PHI && inst->result_name != NULL)
            llvm_mir_bind_versioned_local_scope(ctx, vars, var_count,
                inst->result_name, inst->arg1, true);
    }
}

void
llvm_mir_seed_instruction_use_scope(const MIRInstruction *inst,
                                    LLVMGenCtx *ctx,
                                    LLVMMirVar *vars, size_t var_count)
{
    if (inst == NULL || inst->kind == MIR_INST_PHI)
        return;
    for (size_t i = 0; i < inst->use_count; i++)
        llvm_mir_bind_versioned_local_scope(ctx, vars, var_count,
            inst->uses[i], NULL, true);
}
