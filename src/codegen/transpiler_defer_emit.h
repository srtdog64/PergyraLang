 ...
 * file-scope cleanup helper cannot capture locals like method `self`.
 */

void
transpiler_defer_scope_push(TranspilerCtx *ctx)
{
    if (ctx == NULL || ctx->defer_scope_depth >= TRANSPILE_MAX_SCOPE_DEPTH)
        return;
...
transpiler_register_defer(ASTNode *body, TranspilerCtx *ctx)
{
    ...
    if (ctx == NULL || body == NULL || ctx->defer_scope_depth <= 0)
        return;
