 ...
 * C backend Rc/Weak/Box builtin call lowering.
 */
...
emit_builtin_rc(ASTNode *call, BuiltinKind kind, TranspilerCtx *ctx)
{
    ...
    char inner_buf[128];
    ASTNode *arg = ast_call_arg_count(call) > 0
    ...
    case BUILTIN_RC_NEW:
        ...
        }
        {
            ...
            if (expected_inner != NULL)
                ...
                arg_type = transpiler_expr_infer_type_name(
                    ctx, arg);
            if (expected_inner == NULL
                && arg_type != NULL
    ...
    case BUILTIN_BOX:
        ...
        if (arg->type == AST_NUMBER) inner = "Int";
            ...
            if (class_decl != NULL && class_decl->type == AST_CLASS_DECL)
                ...
                const char *arg_type =
                    transpiler_expr_infer_type_name(ctx, arg);
                if (arg_type != NULL && strcmp(arg_type, "Unknown") != 0)
                    inner = arg_type;
            }
        } else if (arg->type == AST_IDENTIFIER) {
            const char *arg_type = lookup_typed_var(ctx,
