...
/* Check if pattern is a tagged union variant destructor:
 * Circle(r), Rect(w, h), None. */
static bool
...
static void
emit_builtin_match_binding(ASTNode *pattern_node,
                           const char *kind,
                           const char *binding,
...
static void
emit_enum_match_bindings(ASTNode *pattern_node,
                         const char *kind,
                         int tmp_id,
                         TranspilerCtx *ctx)
{
...
emit_match_stmt(ASTNode *node, TranspilerCtx *ctx)
{
    ...
    for (size_t i = 0; i < ast_match_case_count(node); i++) {
        ...
        if (binding != NULL && kind != NULL) {
            emit_builtin_match_binding(pattern_node, kind, binding,
                subject_type, subject_is_option, tmp_id, ctx);
        }
        emit_enum_match_bindings(pattern_node, kind, tmp_id, ctx);
