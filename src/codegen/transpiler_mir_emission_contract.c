...
static bool transpiler_validate_mir_emission_block_shape(
                                                 ...
                                                 size_t reason_cap)
{
    ...
    if (!transpiler_mir_routine_signature_supported((TranspilerCtx *)ctx,
            routine,
            func_decl)) {
        if (reason != NULL && reason_cap > 0)
                                                       ...
                                                       size_t reason_cap)
{
    ...
    if (transpiler_mir_routine_kind(routine) != MIR_SCOPE_INTENT
        ...
        if (reason != NULL && reason_cap > 0)
            transpiler_mir_reasonf(reason, reason_cap,
                "intent %s has no MIR cleanup section (kind=%d, has_cleanup_block=%d)",
