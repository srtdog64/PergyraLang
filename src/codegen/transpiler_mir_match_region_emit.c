...
transpiler_mir_find_incoming_match_branch(const MIRRoutine *routine,
...
static bool
transpiler_mir_block_reaches(const MIRRoutine *routine,
                             size_t from_id,
                             size_t target_id,
                             bool *visited)
{
    ...
    if (block->has_succ_true
        && transpiler_mir_block_reaches(routine, block->succ_true,
                                        target_id, visited)) {
        return true;
    }
    if (block->has_succ_false
        && transpiler_mir_block_reaches(routine, block->succ_false,
                                        target_id, visited)) {
        return true;
    }
    return false;
...
bool
transpiler_mir_case_true_region_contains(const MIRRoutine *routine,
                                         const MIRBasicBlock *case_block,
                                         size_t target_id)
{
    bool *visited;
    bool reaches_true;
    bool reaches_false = false;
    ...
    if (visited == NULL)
        return false;
    reaches_true = transpiler_mir_block_reaches(routine,
        case_block->succ_true, target_id, visited);
    memset(visited, 0, routine->block_count * sizeof(bool));
    reaches_false = transpiler_mir_block_reaches(routine,
        case_block->succ_false, target_id, visited);
    free(visited);
