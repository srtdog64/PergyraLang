...
pergyra_type_to_c(const char *name)
{
    ...
    /* Tuple type: "(T0, T1, ...)" → "PgyTuple_T0_T1_..._t" */
    if (name[0] == '(') {
        static char buf[256];
        size_t off = 0;
        ...
        while (i < n && name[i] != ')') {
            ...
            while (i < n && (name[i] == ' ' || name[i] == '\t'))
                i++;
            /* read one element up to ',' or ')' (depth-tracking) */
            char elem[96];
            size_t eo = 0;
            int depth = 0;
