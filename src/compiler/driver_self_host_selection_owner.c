...
bool
driver_self_host_tokens_request_supported(const DriverFlags *flags)
{
    return flags != NULL
        && flags->source_path != NULL
        && flags->output_path == NULL && flags->dump_tokens
        && !flags->emit_c_only && !flags->emit_llvm_ir && !flags->do_run
        && !flags->dump_ast && !flags->dump_capability_manifest
        ...
        && flags->diag_format == DIAG_FORMAT_TEXT && flags->runtime_mode == RUNTIME_DEFAULT
