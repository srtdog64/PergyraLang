                                            ...
                                            const DriverFlags *flags)
{
    ...
    if (!driver_self_host_mir_diagnostic_request_supported(flags)) {
        fprintf(stderr,
                "pgy: --mir options are outside the installed self-host driver contract\n");
        return 1;
    }
    source_path = driver_self_host_source_identity_path_dup(flags->source_path);
    if (source_path == NULL) {
        fprintf(stderr,
                "pgy: could not canonicalize self-host MIR diagnostic source identity\n");
        return 1;
    }
    ...
    if (payload_length == 0) {
        fprintf(stderr,
                "pgy: self-host driver reported success without a MIR diagnostic payload\n");
        free(payload);
        free(binary);
        free(source_path);
        return 1;
