...
/* =================================================================
 * Slot Memory (Canonical Checked ABI)
 *
 * Slot<T> always carries the occupied flag. Raw/value-only storage must use a
 * distinct explicit ABI owner; it must not reuse the PgySlot_* type names.
 * ================================================================= */

/* Canonical Slot<T> runtime shape. */
...
static inline void \
__attribute__((unused)) \
pgy_pin_read_init_##SuffixName(PgyPinnedSlotView_##SuffixName* out, \
                               PgySlot_##SuffixName* s) \
{ \
    if (out == NULL) \
        PGY_RUNTIME_PANIC(PGY_RUNTIME_PANIC_CLASS_INTERNAL_INVARIANT, \
                          "null slot pin read out"); \
    *out = pgy_pin_read_##SuffixName(s); \
} \
\
static inline void \
__attribute__((unused)) \
pgy_pin_write_init_##SuffixName(PgyPinnedSlotView_##SuffixName* out, \
                                PgySlot_##SuffixName* s) \
{ \
    if (out == NULL) \
        PGY_RUNTIME_PANIC(PGY_RUNTIME_PANIC_CLASS_INTERNAL_INVARIANT, \
                          "null slot pin write out"); \
