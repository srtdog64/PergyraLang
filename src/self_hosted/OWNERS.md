...
- `src/self_hosted/codegen/runtime_abi/spawn_runtime_owner.pgy` -- bounded self-host C spawn/await runtime ABI facts for one- or two-argument Int-call async work through one invocation descriptor, including `Future<Int>` handle materialization; unsupported spawn shapes fail closed.
