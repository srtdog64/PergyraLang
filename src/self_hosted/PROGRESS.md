...
byte-equal output vs `pgy --ast` on **112 of 121** committed
...
parser complete is byte-equal. Eight files are accepted by the C oracle
...
- Self-host parser exits before producing byte-equal AST:
  ...
  `six_item_alignment_demo`, `structured_comments`, `transaction_saga`,
...
3. **Parser bootstrap** -- *expanding* (2026-06-22). `src/self_hosted/parser/`
   ...
   generated by both C and LLVM, and **112 of 121** `examples/*.pgy` files at
