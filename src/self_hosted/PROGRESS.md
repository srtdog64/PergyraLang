...
file/argv/process helper and target-library symbol spelling, supported
...
runtime, released/native compiler driver replacement, and LSP
substitution are still 0%; the compiler driver now has a DRV-0 in-process
assembly artifact rung (docs/150), but that is not counted as released driver
replacement. The MIR-lowering substitution has now *started* (see below).
...
runtime, released/native compiler driver replacement, and LSP remain 0%.
The compiler driver has a DRV-0 assembly artifact rung, but not a shipped driver
substitution.
...
| `src/compiler/` |   43304 |           0 | 0%       | released/native compiler driver replacement remains 0%; DRV-0 in-process parser→codegen assembly artifact exists under `src/self_hosted/compiler/` and is tracked by docs/150, but is not counted as shipped driver substitution |
...
4. **Semantic subset** -- *rung-2 active* (2026-06-23). The current rung
  ...
  owner, stage-artifact envelope owner, hard-rung
