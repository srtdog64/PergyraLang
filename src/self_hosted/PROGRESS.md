...
**Compiler-internal substitution: ~4.07% LOC-scale** (10,362 Pergyra LOC vs 254,742
...
responsibility-owned modules (`text_owner`, `type_env`, `expr_scan`,
`expr_rewrite`, `stmt_emit`, `function_emit`, `program_emit`) rather than one
...
(`make self-host-mir-json-parity-test-smoke`, 72 fixtures plus 2 clean-reject
...
committed MIR-lower/codegen fixture inventory is currently **72 PASS / 0 gap plus 2 clean
...
`binary_search`. `main.pgy` is now only the entrypoint; character/codepoint
classification, token keyword/line rendering, and the scan loop live in
`char_owner.pgy`, `token_owner.pgy`, and `scan_owner.pgy`. The broader lexer
scale result remains the committed scale probe measurement below.
...
| **Total**       | **254742** |  **10362**  | **~4.07% LOC-scale** | lexer/parser/semantic + codegen rung-0..16; no HIR/MIR/runtime/compiler/LSP substitution yet |

Notes:

- *Coverage %* is a rough functional estimate, not a LOC-equivalence
  number. The lexer is 670 LOC and is judged by byte-equal fixture coverage,
...
6. **C-emit codegen subset** -- *rung-0..16 active* (2026-06-23). A Pergyra
   ...
   `Array<Int>` `ArraySort`/`ArrayMap`/`ArrayFilter`, `Result<Int>`
   ...
   on 55 committed fixtures, with the emitter built through both backends. Next
