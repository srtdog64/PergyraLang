...
**Compiler-internal substitution: ~5.82% direct owner-file LOC-scale** (14,469 Pergyra LOC vs 248,794
...
**first codegen rungs** (`src/self_hosted/codegen/`, 3,647 LOC; rung-0 string Log,
...
evidence, `symbol_facts/` for emitted-symbol spelling, and `emission/` for
C-emission action participants. That keeps
...
| `src/codegen/`  |  107123 |        3647 | rung-0..20 | **C-emit rung-0..20 (2026-06-24).** Pergyra emitter consumes `pgy --ast` text and emits standalone C for the supported scalar/string/array/result/option/struct/defer/file/argv/random/float subset. `ast_input_owner.pgy` owns AST path selection, `codegen_run_owner.pgy` owns CLI-to-output orchestration, `type_facts/` owns type routing, `symbol_facts/symbol_mangle_owner.pgy` owns function/method/operator/enum emitted-symbol spelling, and `emission/` contains action participants. **63 fixtures run-stdout equal** to the C/LLVM oracle on tools built through both backends. Gate: `parity/codegen_parity.sh` (`make self-host-codegen-parity-test-smoke`). Out-of-subset input is an observable `Exit(1)`. |
...
| **Total**       | **248794** |  **14469**  | **~5.82% direct owner-file LOC-scale** | lexer/parser/semantic + codegen rung-0..20; MIR JSON lowering and compiler-world contracts are tracked separately above |
...
4. **Semantic subset** -- *rung-2 active* (2026-06-23). The current rung
  ...
  real-source selfcheck now feeds 74 accepted self-host owner/source files
  ...
  owner, codegen symbol-mangle owner, semantic run/program/body/call/expression owner files, and audit-tool
