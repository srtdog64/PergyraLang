...
## Active self-host context - 2026-08-26 nested intent direct-MIR LLVM route

- Executable checkpoint `a9e0784115c98eda4a52db0b28f9a9a46b26eeb6`
  is published on `main`. Preserve the unrelated user-owned
  `pgy-80135c2c/` directory and do not use it as project evidence.
- The preceding intent-policy C-codegen migration is closed. Its exact policy
  view owns canonical routine syntax/name, admitted MIR mode/priority carriers,
  and the semantic expression occurrence; the direct semantic entrypoint emits
  the same DIR-derived receipt. Neither policy emitter can reopen
  `TypedAstArena` or `AstTreeArtifact`, and missing/duplicate/stale carrier,
  missing/drifted graph, and absent-row nonzero-root cases fail closed.
- Final code checkpoint `a9e07841` completed remote run `32870231909` at 29/29
  green in 29m24, including full self-host, codegen bootstrap, all platforms,
  sanitizers, Rocq, and all 20 backend shards. The repaired likeness remains at
  sentinel `24/24` and Result/Option `4267/4267`; no ratchet was loosened.
- That closure covers the C observability AST-last-consumer seam, not the wider
  `selfhost.intent_declaration_rows` family. Overall remains 78%, strict beta
  83%, and hard SoT `CLOSED=49 BRIDGE=36 ACTIVE=1`.
- Active objective card: make installed direct-MIR LLVM execute the nested
  function/method/intent priority fixture through one exclusive mixed-callable
  route. Admitted declaration/routine identity, routine kind/owner/source
  syntax ID, the referenced-enum owner only for actual enum references, and
  `MirIntentRoutineCarrierProjection` / `MirIntentModeProjection` /
  `MirIntentPriorityProjection` own the required facts. The last consumer is
  the new exclusive route in `DirectMirMultiRoutineProjection` before scalar
  admission. Priority `0` fallback, source/AST reconstruction, widening the
  function-only signature fact, fixture branching, native retry, and a general
  query/cache track are forbidden.
- The falsifier is observed RED: the admitted MIR for
  `tests/self_hosted/parity/fixture/intent_priority_nested_observability.pgy`
  publishes no artifact and first fails at `scalar-program-route` stage
  `referenced-enum`. The enum owner incorrectly scans method/intent rows through
  function-only signature readiness even though this fixture has no enums.
  Canonical-empty enum handling then reaches a second scalar-only rejection at
  `callable-route-envelope` stage `signature`; an enum-only repair is not the
  executable delta.
- The prepared lease must seal admitted facts for `Main -> Outer`,
  `Outer -> Inner`, `Inner -> Capture`, subject/zone fields, literal Outer
  priority `1`, and Inner's runtime parameter `requested`. Completion requires
  installed LLVM emit/link/run parity with no scalar-route receipt plus
  no-artifact negatives for graph drift, missing priority, and syntax/name
  crosswire. No implementation of this new route is claimed yet.
...
- Executable checkpoint `37624de9` removes the final native fallback for exact
  ...
  source-read sibling request, `DriverRung2CliRequestFromArgsOrDie` assigns the
  typed `DriverCliSourceAstStdout` identity, and the read executor consumes it
  through import-composed `ParseRootProgram`. Unsupported option mixtures and
  a missing sibling fail without entering `driver_run_pipeline`.
- `tests/self_hosted/parity/public_ast_installed_self_host_owner.sh` passes in
  2.2 seconds. It proves direct-installed/public byte equality, equality with
  the committed arithmetic fixture and explicit native oracle, imported-source
  composition through `Intent: ImportedFrontendPipeline`, missing sibling
  no-fallback behavior, unsupported-option rejection, and installed argv arity
  failure.
- `tests/self_hosted/parity/parser_parity.sh` passes all 189 manifest rows for C
  and LLVM in 324.8 seconds. Every live native reference now spells
  `--native-pipeline --ast`; public `--ast` is the installed candidate and must
  not be reused as its own oracle. Imported intent composition, generic default
  contracts, and language-word registry parser gates also pass in 1.9, 128.7,
  and 121.8 seconds respectively.
- The token substitution remains green: its focused regression gate passes in
  3.0 seconds. The installed CLI mode gate passes in 2.4 seconds, and the full
  component inventory passes in 129.8 seconds after the shared source-read
  selector and parser-oracle ratchets.
- The current Pergyra-built installed driver is `bin/pgy-self-driver.exe`,
  5,072,633 bytes, SHA-256
  `A123FBBD00F10CEC9DE72F8EFD5D3C2F8BAC08F5A715964DBB031957FE418465`.
  Its canonical rebuild completed in 134.8 seconds. Public AST dumping is
  bounded `SUBSTITUTING`; source compilation's parser/semantic path and other
  debug/dump modes remain open.
- Same-mistake rule: after a public source-read selector becomes the installed
  candidate, that public spelling is no longer an independent oracle. Token
  and AST native references must use `--native-pipeline`; the direct sibling or
  public selector owns the installed voice, and the gate compares the two
...
- The current-source Pergyra-built driver is 4,613,637 bytes with SHA-256
  ...
  It closes `src/self_hosted/codegen/fixture/array_max.pgy` through the
  installed direct C and LLVM routes. The 9,706-byte self-produced MIR has
  SHA-256
  `F3200219061C8257830EF18A2A0E201B4D2C4460E2384C05A204AD7144CE10F3`.
  Its 1,022-byte C artifact
  (`0A0C028BDDD8AB273EAE22395CD7D8296F3DECCABCECD91A8A07537BA959458F`)
  and 3,936-byte LLVM artifact
  (`32F5B7296DC777837FAFC6F4007B2B38EBA29E18BB1502FC75FD901D7E6AFD98`)
  both host-compile and execute exact `9`.
- One target-neutral `GraphPlan` and one Array program receipt now own three
  mutually exclusive modes: dynamic push, initialized static set, and
  initialized read-only range maximum. The former loop-mutation owner identity
  was promoted to program ownership without a compatibility alias; the five
  old files and their symbols are negative-ratcheted.
- The read-only mode reuses the existing collection definition, literal spine,
  canonical four-field ABI, range blocks, ValueId/LocalRef identities, phi and
  log operations. Its receipt owns only the additional range syntax binding,
  comparison/update roles, exact predecessor/value bindings, and maximum
  readiness. It does not duplicate generic range facts or reconstruct the
  graph per operation.
- C and LLVM both use current length, initialize `best` from element zero,
  compare each dynamic `xs[i]`, update the same phi-owned best value, and log
  the final value. Neither backend reopens MIR, treats capacity as length,
  precomputes `9`, retries another Array mode, or escapes to native collection
  semantics.
- The focused gate proves baseline `9`, first/last maximum and all-negative
  variants, display/phi-order artifact equality, and 23 independent no-artifact
  negatives across Array identity, ABI, source types, range, comparison,
  update, phi, log, extra use, and duplicate operation facts. The prior sum/set
  and dynamic-push paths remain executable regressions.
- Observed evidence after the final owner split: composed source check about
  6.0 seconds; current-source driver build 128.7 seconds; the final three-mode
  max/sum/push regression 102.3 seconds; component/removed-path ratchet 245.8
  seconds; cumulative scalar-CFG integration 388.539 seconds. The final
  cumulative process tree was sampled every 500 ms and peaked at 0.027 GiB
  working set and 0.010 GiB private memory. This small sampled peak can miss
  short-lived children and is integration-test evidence, not an ordinary build
  or compiler-wide memory claim. Neither pressure threshold fired.
- Classification is bounded `SUBSTITUTING` for this exact nonescaping local
  initialized read-only range/if/phi maximum shape in installed C/LLVM, beside
  the two previously closed Array modes. It is not a generic reduction, alias,
  parameter, returned collection, arbitrary call graph, or whole-compiler
  replacement. Full CI, public matrices, current gen2==gen3 fixpoint, and proof
  suites did not run. LLVM emitted only tolerated target-triple warnings.
- The independent observation that AI often preserves language surface more
  readily than compiler architecture remains anecdotal evidence. This rung's
  local failures were architectural: obsolete owner identity, duplicate range
  facts, a forged absence sentinel, and an SSA-name/SSA-ID mismatch. Named
  owners and negative gates, not generated code volume, closed them.
- The sole next executable falsifier is
  `src/self_hosted/codegen/fixture/array_reverse.pgy`. The current driver
  produces a 7,260-byte MIR with SHA-256
  ...
  current receipt owns both the source `nums` and fresh result `reversed`
  collection identities across the `ArrayReverse` call.
- Next objective card: extend the one scalar-CFG plan with an explicit
  fresh-value Array transform receipt. Priority is source/result collection
  identity and non-aliasing semantics, owner-directed `ArrayReverse` call and
  result facts, fallback exclusion, then exact `3`, `2`, `1` parity. The
  canonical Array ABI, literal spine, expression graph, LocalRef, runtime ABI,
  and local inventory owners supply facts; `CompileAdmittedDirectMirForTarget`
  is the last legitimate consumer. Forbidden fallbacks are fixture/text or
  block-count routing, backend MIR reads, precomputed output, silently aliasing
...
- Checkpoint `e3423436995409438f1044570d5af4fb10b760d2` closed
  `array_sum.pgy` in the same Array program receipt. Its direct C and LLVM
  artifacts execute exact `60`, `99`, then `3`; 35 negatives and the old-route
  ratchets forbid a second static-set planner, backend graph reads, runtime
  escape, and dynamic-mode retry.
- The cumulative integration took 348.268 seconds and sampled 0.367 GiB working
  set and 0.323 GiB private memory. This evidence remains historical; the top
  card alone names the active continuation.

### Previous 2026-08-04 bounded `Array<Int>` loop push

- The current-source Pergyra-built driver is 4,566,764 bytes with SHA-256
  `F167FE76B3009CCDE5A1CD937B9655CD7A5C828E478505EADF62746CB7C4A73E`.
  It closes `src/self_hosted/codegen/fixture/array_push.pgy` through the direct
  C and LLVM routes. The 13,824-byte self-produced MIR has SHA-256
  `AE1427524AA69F54A180C99BB6F5D3678C5C95353F7BBC74963197787AE82220`.
  Its 1,306-byte C artifact
  (`382B455E0FF3F3C59E956EBDABCEB12E7EF82CA3AA63DABDACD42F0A08EC4A97`)
  and 3,906-byte LLVM artifact
  (`DC78108CF6CB5953D2840D17AAD5615D7ECF4FCB09FE9E265BDEFF2CE882BB5B`)
  both host-compile and execute exact `30` then `5`.
- One target-neutral scalar-CFG `GraphPlan` now owns the empty local
  `Array<Int>`, canonical four-field ABI, producer and consumer loop induction,
  dynamic `i * i` pushes, current length, indexed accumulation, and final
  length log. C and LLVM consume this receipt; neither backend reopens MIR,
  discovers the collection, or calls a native/runtime push fallback.
- Generic CFG responsibilities were extracted into responsibility-named
  owners for instruction graphs and positions, while induction, array guard
  dominance, collection expressions, and plan sealing. Existing String-array
  owners delegate to them. This is one import-composed compiler graph, not a
  second compiler or an independent helper subsystem.
- Admission requires exact source-local identity and ABI, one bounded
  zero-start/unit-step producer, store-before-length advance, one
  zero-start/unit-step consumer, current-length guards, accumulator phi
  recurrence, exact CFG topology, and a bound from 1 through 46,340. The bound
  keeps `i * i` in signed 32-bit range and fixes static capacity without
  pretending capacity is current length.
- Display-only and phi-order mutations are artifact-identical. Graph changes
  to multiplication/addition and the producer bound change execution in both
  targets. Twenty-three receiver, use, graph, route, bound, edge, backedge,
  step, duplicate-operation, length, read, local-type, and ABI falsifiers fail
  before artifact publication with no block-count or Option retry.
- Observed gates: composed source check 8.4 seconds; final driver build 126.4
  seconds; focused parity 29.6 seconds; indexed and mutation String regressions
  green; component/removed-path ratchet 239 seconds; cumulative scalar-CFG
  integration 326.321 seconds. The cumulative run was sampled once and peaked
  at 0.489 GiB working set and 0.896 GiB private memory. Neither the 2.4 GiB
  attention threshold nor the 3 GiB hard stop fired.
- Classification is bounded `SUBSTITUTING` for this exact nonescaping local
  `Array<Int>` loop-mutation shape in the installed direct C/LLVM slice. It is
  not generic dynamic growth, reallocation, arbitrary push/control flow,
  aliases, parameters, returned arrays, or whole-compiler replacement. Full
  CI, public matrices, current gen2==gen3 fixpoint, and proof suites did not
  run. LLVM host compilation emitted only the already tolerated target-triple
  override warnings.
- The sole next executable falsifier is
  `src/self_hosted/codegen/fixture/array_sum.pgy`. Its 12,383-byte MIR has
  SHA-256
...
- Next objective card: reuse the canonical static `Array<Int>` definition and
  the newly shared while/guard/index owners for exact `60`, then admit the
  existing in-bounds `ArraySet(xs, 1, 99)` and execute exact `99` and `3`.
  Priority is collection identity and current-length ownership, reuse of the
  one scalar-CFG plan, old-route exclusion, then parity. Existing Array ABI,
  collection expression, loop, guard, operation, ValueId, and LocalRef owners
  are the fact owners; `CompileAdmittedDirectMirForTarget` is the last
  legitimate consumer. Forbidden fallbacks are a fixture route, a second
  Array compiler, backend graph reads, precomputed outputs, capacity-as-length,
  native/runtime escape, or weakening the just-closed dynamic-push claim.

### Older historical checkpoints
...
private/working set, below the unchanged 3,072 MB cap. It emitted
54,205,046 bytes with SHA-256
`3d6aa33595592f8af2c78a68c6d5fc9e5a242c15e55b9e5a8deb4fe60209083b`,
byte-identical to the current C-oracle artifact. No partial output existed
before verified JSON writing.

The formal full-bootstrap runner now makes the Pergyra-built seed produce
`driver_source.mir.json`. The native driver emits a separate oracle artifact;
the artifact owner compares them before gen2 consumption. Gen2 and gen3 then
consume only the one Pergyra-produced MIR. Static ratchets reject restoring
the oracle-owned start, and the script remains below its 300-line owner cap.
This closes complete-source Pergyra MIR production plus the v63 gen2/gen3 C
...
| `src/compiler/` |   43304 |        9389 | complete-source Pergyra MIR producer + gen2/gen3 fixed point; released 0% | `driver_pipeline_owner.pgy` owns the shared source->AST spine. The Pergyra-built gen2 produces verified complete-source MIR byte-identical to separate C-oracle evidence; the Pergyra seed/gen2 then consume that artifact and reach byte-identical gen2/gen3 C. The default native-driver promotion remains open. |
...
4. **Semantic subset** -- *rung-2 active* (2026-06-23). The current rung
  ...
  math-runtime, host-I/O-runtime, Option/Result-runtime, and string-runtime owners, semantic run/program/body/call/expression owner files, and audit-tool
   ...
   binaries across 114 fixtures. The same gate now validates the 17-code
...
7. **Bootstrap loop** -- v63 completes the explicit integrated
   semantic/MIR fixed point. v64 makes the Pergyra-built gen2 driver produce
   the verified complete-source MIR, requires byte parity against the separate
   C-oracle artifact, and feeds only the Pergyra artifact to gen2/gen3. The
   resulting gen2/gen3 C is byte-identical. Released-driver replacement remains
   open.
...
integrated complete-source Pergyra MIR producer plus gen2/gen3 C fixed point.
The remaining terminus is released/default promotion; AIR consumer expansion
remains a separate owned track.
