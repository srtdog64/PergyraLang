...
byte-equal output vs `pgy --ast` on **99 of 121** committed
...
parser complete is byte-equal. Twenty-one files are accepted by the C oracle
...
- Self-host parser exits before producing byte-equal AST:
  ...
  `intent_contract_derivation_minimal`, `intent_contract_pair_minimal`,
  `parallel`, `party_system_demo`,
...
3. **Parser bootstrap** -- *expanding* (2026-06-22). `src/self_hosted/parser/`
   ...
   generated by both C and LLVM, and **99 of 121** `examples/*.pgy` files at
