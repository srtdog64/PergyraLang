...
**first codegen rungs** (`src/self_hosted/codegen/`, 1657 LOC; rung-0 string Log,
...
fixtures. Breadth: the same codegen also compiles the lexer (587 lines) and parser (3338 lines); each codegen-built binary matches its oracle-built counterpart on a sample source -- three real self-host components self-built. Wider survey: the codegen compiles 21 of 22 committed self-host components/tools to valid C (only the semantic checker remains, needing ArrayPop), each verified run-equivalent to the oracle-built binary on a sample. This includes namespace-imported audit tools (`TextScan::` qualified calls, flattened to `NS_Func` -- import/namespace + DirWalk support added). The bootstrap gate verifies codegen + lexer + parser + 6 audit tools all match oracle-built. The earlier 18/22 ceiling was a `pgy --ast` bug (for-each `for x in lines` rendered as `For: x in (null)..(null)`, dropping the collection); this session FIXED it in src/parser/ast_print.c (emit the iterable) + the self-host parser, regenerated 5 parser fixtures, and added for-each lowering + bare-void-return + word-boundary builtin matching to the codegen. Parser parity (188 sources) stays byte-equal. 21/22 now compile; the bootstrap gate verifies codegen + lexer + parser + 13 audit tools. Gated by `parity/codegen_bootstrap.sh`
