...
MIR facts (args, arrays, Bool/string/Float builtins, string concat/equality,
recursion, `continue`, and file read/write), gated by
`parity/mir_json_parity.sh` (`make self-host-mir-json-parity-test-smoke`, 30
fixtures). The gate now requires the MIR JSON fact surface and checks the `for`
