...
from `ArrayPush` or other statement text. The real-source semantic selfcheck now accepts 125
...
`parser_parity.sh` now consumes a **186-row** source/fixture manifest emitted
...
4. **Semantic subset** -- *rung-2 active* (2026-06-23). The current rung
  ...
  real-source selfcheck now feeds 125 accepted self-host owner/source files
  ...
  entrypoints, the lexer run/fixture-manifest owners, the compiler path manifest
