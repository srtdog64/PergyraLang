...
byte-equal output vs `pgy --ast` on **98 of 121** committed
...
parser complete is byte-equal. Twenty-two files are accepted by the C oracle
but currently exit in the self-host parser, so the remaining work is grammar
breadth rather than canonical AST ordering drift. One file fails under
`pgy --ast` itself and is a C-skip (`secure_slots`). The scale probe is a
...
- Self-host parser exits before producing byte-equal AST:
  `action_contract_inheritance_minimal`, `authority_contract_pair_minimal`,
  `battle_sim`, `calendar_manage_event_compressed`,
  `calendar_manage_event_explicit`, `generic_ability_requires_minimal`,
  `infer_return`, `intent_contract_derivation_minimal`,
  `intent_contract_pair_minimal`, `parallel`, `party_system_demo`,
  `role_ability_demo`, `six_item_alignment_demo`, `structured_comments`,
  `surface_compression_maximal`, `transaction_saga`,
  `transfer_contract_pair_minimal`, `transfer_move_minimal`,
  `transfer_move_typed_minimal`, `vessel_action_design`,
  `world_roster_city`, `zone_context_minimal`.
...
3. **Parser bootstrap** -- *expanding* (2026-06-22). `src/self_hosted/parser/`
   ...
   generated by both C and LLVM, and **98 of 121** `examples/*.pgy` files at
   scale with zero completed-output byte drift. It now covers the domain declaration surface (`subject`, `object`,
   ...
   grammar breadth plus the scale-probe exit list, not C-only backend evidence.
4. **Semantic subset** -- *rung-2 active* (2026-06-16). The current rung
   ...
   oracle on C and LLVM binaries across 61 fixtures. Next expansion should add
