...
| `src/lsp/`      |    1037 |           0 | 0%       | released/native LSP replacement remains 0%; LSP-0 diagnostic `publishDiagnostics` payload projection and LSP-1 squiggle policy exist under `src/self_hosted/lsp/` and are tracked by docs/150. `ReadStdin(n)` substrate has landed for future Content-Length framing, but no transport/session replacement has landed |
