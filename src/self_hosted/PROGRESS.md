...
(`make self-host-mir-json-parity-test-smoke`, 70 fixtures plus 2 clean-reject
...
committed MIR-lower/codegen fixture inventory is currently **70 PASS / 0 gap plus 2 clean
...
6. **C-emit codegen subset** -- *rung-0..15 active* (2026-06-17). A Pergyra
   ...
   `Ok`/`Err`/`IsOk`/`IsErr`/`Unwrap`/`UnwrapOr`, block-local `defer`,
   `Exit(n)`,
   ...
   on 53 committed fixtures, with the emitter built through both backends. Next
