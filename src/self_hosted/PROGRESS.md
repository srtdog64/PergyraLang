...
The latest broad parity refresh was `make self-host-preparation-parity-test-smoke`
on 2026-07-09: it completed green with 195 real sources accepted by both
selfcheck backends, M2 completeness at 195/195 through
lexer/parser/semantic/codegen/full pipeline, codegen bootstrap `gen2 == gen3` at
8560 generated-C lines, DRV-0/DRV-1 driver parity, LSP parity, backend
tri-compare, and MIR JSON rung-0b parity over 86 fixtures. This is a broad
parity/preparation evidence refresh, not a claim that the heavier all-in-one
`self-host-preparation-test-smoke` wrapper was rerun in the same slice.
...
195 production self-host source files across lexer, parser, semantic, codegen,
and full-pipeline identity. The real-source semantic selfcheck uses the broad
195-source C/LLVM gate from the latest parity preparation refresh over the current accepted semantic subset,
...
`gen2 == gen3` byte-identical (last observed 8560 generated-C lines) -- and the
...
4. **Semantic subset** -- *rung-2 active* (2026-06-23). The current rung
  ...
  real-source selfcheck now feeds 195 accepted self-host owner/source files
...
6. **C-emit codegen subset** -- *rung-0..20 active* (2026-06-24). A Pergyra
   ...
   The M2 completeness ledger also now checks all 195 production self-host
