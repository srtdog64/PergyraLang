...
**Status:** *rung-1 parity* (2026-06-18). First Pergyra-origin tool that
...
compiler. This tool is the first that does. It reimplements a bounded Pergyra
lexer slice in Pergyra itself, emitting output byte-equal to `pgy --tokens`
for the committed parity sources.
...
- **source_owner**: `examples/hello.pgy`, `examples/array_literal.pgy`,
  `examples/break_continue.pgy`, `examples/basic.pgy`, `examples/heap.pgy`,
  and `examples/binary_search.pgy`
...
- The Pergyra origin's stdout byte-equals the committed fixture for each
  source pair (`hello_tokens.txt`, `array_literal_tokens.txt`,
  `break_continue_tokens.txt`, `basic_tokens.txt`, `heap_tokens.txt`, and
  `binary_search_tokens.txt`), each generated by `pgy --tokens`.
- The generated C and LLVM lexer binaries produce byte-equal output for the
  same committed source pairs.
