...
`program_check_owner.pgy`. `semantic_run_owner.pgy` owns CLI argument validation
and deterministic diagnostic verdict emission; `main.pgy` only wires `Args()`
into that owner. The run owner emits a deterministic diagnostic verdict for
