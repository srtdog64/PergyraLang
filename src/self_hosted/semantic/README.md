...
real-source gate: it compiles this checker through C and LLVM and runs it on 56
accepted self-host owner/source files, including lexer/parser/mir_lower/codegen/
compiler-world, the compiler path manifest owner, semantic run/program/body/
call/expression owner files, the deterministic backend fuzz generator, and
audit-tool slices that are inside the current semantic subset.
