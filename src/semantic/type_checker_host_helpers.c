...
semantic_host_decl_for_type(SemanticContext *ctx, const Type *type)
{
     ...
     * The constructor (e.g. `Box`) carries the original declared name; if the
     * first pass on `type->name` fails, retry the same lookup chain using
    ...
    if (constructor == NULL || constructor == type
        || constructor->name == NULL || constructor->name[0] == '\0'
        || strcmp(constructor->name, type->name) == 0)
        return NULL;
    base_name = constructor->name;

    decl = semantic_find_class_decl_by_name(ctx, base_name);
    if (decl != NULL)
        return decl;
    decl = semantic_find_party_decl_by_name(ctx, base_name);
    if (decl != NULL)
        return decl;
    decl = semantic_find_roster_decl_by_name(ctx, base_name);
    if (decl != NULL)
        return decl;
    decl = semantic_find_world_decl_by_name(ctx, base_name);
    if (decl != NULL)
        return decl;
    decl = semantic_find_zone_decl_by_name(ctx, base_name);
    if (decl != NULL)
        return decl;
    decl = semantic_find_relation_decl_by_name(ctx, base_name);
    if (decl != NULL)
        return decl;
    return semantic_find_effect_decl_by_name(ctx, base_name);
}
