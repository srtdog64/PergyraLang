...
static bool
test_air_parsed_action_self_authority_uses_intent_rir_evidence(void)
{
    const char *source =
        "ability Disciplined {\n"
        "    func Ready() -> Bool;\n"
        ...
        "    }\n"
        "}\n"
        "role RunnerDiscipline for Runner {\n"
        "    impl ability Disciplined {\n"
        "        func Ready() -> Bool { return true; }\n"
        "    }\n"
        "}\n"
        "zone RunnerZone {\n"
        "    subject slot runner: Runner\n"
        "    authority runner requires Disciplined\n"
        "}\n"
        "intent Execute(zone: RunnerZone, runner_alias: Runner) {\n"
        "    step Plan {\n"
        "        using: zone;\n"
        "        who: runner_alias;\n"
        "        on: runner_alias.Plan();\n"
        "        expect: true;\n"
        "    }\n"
        "    success: true;\n"
        "    failure: false;\n"
    ...
    AIRProgram *air = lower_air_from_source(source);
    bool found = false;
    bool found_intent_boundary_evidence = false;
    bool found_intent_authority_evidence = false;
    ...
    if (air != NULL) {
        for (size_t i = 0; i < air->boundary_count; i++) {
            ...
            if (boundary->kind == AIR_BOUNDARY_ZONE
                && boundary->source_name != NULL
                && strcmp(boundary->source_name, "RunnerZone") == 0
                && boundary->source_from_action
                && boundary->authority_from_action
                && boundary->authority_name_count == 1
                && strcmp(boundary->authority_names[0], "runner_alias") == 0
                && boundary->has_rir_boundary_evidence
                && boundary->has_rir_authority_evidence
                && boundary->rir_authority_evidence_name != NULL
                && strcmp(boundary->rir_authority_evidence_name,
                          "runner_alias") == 0) {
                found = true;
                break;
            }
        ...
        for (size_t i = 0; i < air->evidence_count; i++) {
            const AIREvidenceNode *evidence = &air->evidence_nodes[i];
            if (evidence->kind == AIR_EVIDENCE_RIR_BOUNDARY
                && evidence->provider_name != NULL
                && strcmp(evidence->provider_name, "Execute") == 0) {
                found_intent_boundary_evidence = true;
            }
            if (evidence->kind == AIR_EVIDENCE_RIR_AUTHORITY
                && evidence->provider_name != NULL
                && strcmp(evidence->provider_name, "Execute") == 0
                && evidence->subject_name != NULL
                && strcmp(evidence->subject_name, "runner_alias") == 0) {
                found_intent_authority_evidence = true;
            }
        }
    }
    ...
    bool ok = air != NULL
        && air->drift_count == 0
        && found
        && found_intent_boundary_evidence
        && found_intent_authority_evidence;
    air_destroy(air);
    return ok;
}
