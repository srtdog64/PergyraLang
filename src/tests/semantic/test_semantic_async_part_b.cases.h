    ...
    TEST("TrySend rejects borrowed Slice channel payloads");
    {
        SemanticContext *ctx = semantic_context_create();
        scope_enter(&ctx->scope, SCOPE_GLOBAL);
        ...
        EXPECT(type_equals(t, TYPE_BOOL));
        EXPECT(ctx_has_diagnostic_substring(ctx,
            "TrySend does not support borrowed Slice transport yet"));

        semantic_context_destroy(ctx);
        ast_destroy(call);
    }

    TEST("SendTimeoutStatus rejects borrowed Slice channel payloads");
    {
        SemanticContext *ctx = semantic_context_create();
        scope_enter(&ctx->scope, SCOPE_GLOBAL);
        ...
        Type *t = type_check_expression(call, ctx);
        EXPECT(ctx->has_error);
        EXPECT(type_is_constructed_named(t, "Option")
            && type_equals(type_get_constructed_arg(t, 0), TYPE_BOOL));
        EXPECT(ctx_has_diagnostic_substring(ctx,
            "SendTimeoutStatus does not support borrowed Slice transport yet"));

        semantic_context_destroy(ctx);
        ast_destroy(call);
    }

    TEST("SendTimeout rejects borrowed Slice channel payloads");
    {
        SemanticContext *ctx = semantic_context_create();
        scope_enter(&ctx->scope, SCOPE_GLOBAL);
        ...
        EXPECT(type_equals(t, TYPE_BOOL));
        EXPECT(ctx_has_diagnostic_substring(ctx,
            "SendTimeout does not support borrowed Slice transport yet"));
    ...
    TEST("channel recv rejects borrowed Slice payload");
    {
        SemanticContext *ctx = semantic_context_create();
        scope_enter(&ctx->scope, SCOPE_GLOBAL);
        ...
        Type *t = type_check_expression(recv, ctx);
        EXPECT(ctx->has_error);
        EXPECT(t == TYPE_UNKNOWN);
        EXPECT(ctx_has_diagnostic_substring(ctx,
