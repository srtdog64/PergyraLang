...
excess; `04_channel_parallel` `A_inh=6`, `phys_Sync_excess=15` (including the
bounded `pthread_cond_timedwait` cancellation quantum added by `6d29fef9` and
the `pthread_once` default runtime-context initialization reached by lane
context capture in `b4846491`);
