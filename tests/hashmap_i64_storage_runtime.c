...
grow_failure_is_closed(size_t allocation_offset)
{
    ...
    for (int64_t i = 0; i < 12; i++)
        pgy_map_set_i64_int(&map, i * INT64_C(4294967296) + 7, (int32_t)i + 100);
    keys = map.keys;
    values = map.values;
    ...
    pgy_map_set_i64_int(&map, INT64_C(0x4000000000000007), 77);
    fail_at = 0;
    if (map.keys != keys || map.values != values || map.occupied != occupied
        ...
        || pgy_map_has_i64_int(&map, INT64_C(0x4000000000000007))) {
        drop_i64_map(&map);
        return false;
    }
    for (int64_t i = 0; i < 12; i++) {
        if (pgy_map_get_i64_int(&map, i * INT64_C(4294967296) + 7)
            != (int32_t)i + 100) {
            drop_i64_map(&map);
            return false;
        }
...
collision_delete_grow_extremes(void)
{
    ...
    pgy_map_set_i64_int(&map, INT64_MIN, 17);
    pgy_map_set_i64_int(&map, INT64_MAX, 29);
    for (int64_t i = 0; i < 256; i += 2)
        pgy_map_remove_i64_int(&map, i * INT64_C(4294967296) + 7);
    for (int64_t i = 1; i < 512; i += 2) {
        if (pgy_map_get_i64_int(&map, i * INT64_C(4294967296) + 7)
            != (int32_t)i + 1) {
            drop_i64_map(&map);
            return false;
    ...
    for (int64_t i = 0; i < 256; i += 2)
        pgy_map_set_i64_int(&map, i * INT64_C(4294967296) + 7, (int32_t)i + 1000);
    if (pgy_map_get_i64_int(&map, INT64_MIN) != 17
        || pgy_map_get_i64_int(&map, INT64_MAX) != 29
        || map.count != 514) {
        drop_i64_map(&map);
        return false;
    }
...
existing_update_never_grows(void)
{
    ...
    if (!pgy_map_int_is_initialized(&map))
        return false;
    for (int64_t i = 0; i < 12; i++)
        pgy_map_set_i64_int(&map, i * INT64_C(4294967296) + 7, (int32_t)i);
    for (int64_t i = 0; i < 8; i++)
        pgy_map_remove_i64_int(&map, i * INT64_C(4294967296) + 7);
    calls_before = fault_call;
    fail_at = fault_call + 1;
    pgy_map_set_i64_int(&map, INT64_C(11) * INT64_C(4294967296) + 7, 9001);
    fail_at = 0;
    if (fault_call != calls_before
        || pgy_map_get_i64_int(&map,
            INT64_C(11) * INT64_C(4294967296) + 7) != 9001) {
        drop_i64_map(&map);
        return false;
    }
    drop_i64_map(&map);
...
main(int argc, char **argv)
{
    ...
    if (pgy_hashmap_hash_i64(7) ==
            pgy_hashmap_hash_i64(INT64_C(4294967296) + 7)
