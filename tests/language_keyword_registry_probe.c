...
main(void)
{
    ...
    if (lexer_keyword_registry_count() != 146
        || PGY_LANGUAGE_WORD_COUNT != 146) {
        fprintf(stderr, "registry/word-id count is not 146\n");
        return 1;
    }
    ...
    if (reserved_count != 70 || contextual_count != 73 || soft_count != 3)
        return 9;
