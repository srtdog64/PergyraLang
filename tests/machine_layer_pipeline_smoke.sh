...
for path in \
    "$ROOT_DIR/src/codegen/transpiler_mir_resource_op_core.c" \
    "$ROOT_DIR/src/codegen/transpiler_slot_builtin_emit.c" \
    "$ROOT_DIR/src/codegen/llvm_expr_slot_device_calls.c"; do
    grep -Fq -- "mir_machine_layer_fact_matches_runtime_operation" "$path" ||
        fail "runtime-operation identity gate missing in $path"
done
for path in \
    "$ROOT_DIR/src/codegen/transpiler_mir_resource_op_core.c" \
