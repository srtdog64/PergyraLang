...
grep -Fq "pgy_codegen_ast_number_is_nonzero_i32_literal" "$ROOT_DIR/src/codegen/transpiler_expr_core_emit.h"
...
grep -B10 -A2 -F "entry = find_free_entry_locked(manager);" \
    "$ROOT_DIR/src/runtime/slot_manager.c" | \
...
grep -Fq "!mir_instruction_uses_source_local_decl_emit(inst)" "$ROOT_DIR/src/codegen/transpiler_mir_pending_uses.h"
...
grep -Fq "pergyra_ast_type_to_c_copy" "$ROOT_DIR/src/codegen/transpiler_func_forward_helpers.h"
...
grep -Fq "transpiler_contextual_option_inner_type_copy(ctx" "$ROOT_DIR/src/codegen/transpiler_call_result_option_builtin_emit.h"
grep -Fq "transpiler_contextual_option_inner_type_copy(ctx" "$ROOT_DIR/src/codegen/transpiler_expr_type_infer.h"
...
! grep -Fq "lookup_future_inner_type(" "$ROOT_DIR/src/codegen/transpiler_func_forward_helpers.h"
grep -Fq "channel_inner_type_name_copy" "$ROOT_DIR/src/codegen/transpiler_type_mapping_helpers.h"
...
grep -Fq "slot_inner_type_name_copy(type_name, inner_buf" "$ROOT_DIR/src/codegen/transpiler_func_class_flow_emit.h"
...
! grep -Fq "static const char *bindings_buf" "$ROOT_DIR/src/codegen/transpiler_match_emit.h"
...
! grep -A24 -F "strcmp(init->data.call.callee->data.identifier.name, \"SetNew\")" "$ROOT_DIR/src/codegen/transpiler_let_emit.h" | grep -Fq "collection_runtime_suffix(inner)"
...
grep -Fq "inst->source_ast_type == AST_MATCH_CASE" "$ROOT_DIR/src/compiler/mir_source_shape.c"
grep -Fq "inst->source_ast_type == AST_BLOCK" "$ROOT_DIR/src/compiler/mir_source_shape.c"
...
grep -Fq "source_terminator_has_value" "$ROOT_DIR/src/compiler/mir_fact_terminator_validate.c"
...
grep -Fq "mir_instruction_branch_requires_source_emit(inst)" "$ROOT_DIR/src/codegen/llvm_mir_block_emit.c"
...
grep -Fq "mir_instruction_uses_source_statement_emit(inst)" "$ROOT_DIR/src/codegen/transpiler_mir_block_emit_helpers.h"
grep -Fq "transpiler_mir_def_uses_source_statement_emit(" "$ROOT_DIR/src/codegen/transpiler_mir_assignment_emit.h"
grep -Fq "mir_instruction_source_matches_ast_type(inst, expected_type)" "$ROOT_DIR/src/codegen/transpiler_mir_block_emit_helpers.h"
grep -Fq "mir_instruction_source_matches_ast_node(resource_inst, stmt)" "$ROOT_DIR/src/codegen/transpiler_mir_stmt_emit.h"
...
grep -Fq "mir_instruction_source_branch_payload_matches_shape(inst)" "$ROOT_DIR/src/codegen/transpiler_mir_cfg_control_emit.h"
...
grep -Fq "mir_instruction_branch_requires_source_emit(inst)" "$ROOT_DIR/src/codegen/llvm_mir_block_emit.c"
grep -Fq "mir_instruction_branch_requires_source_emit(inst)" "$ROOT_DIR/src/codegen/transpiler_mir_emission_contract.h"
grep -Fq "mir_instruction_source_branch_payload_matches_shape" "$ROOT_DIR/src/codegen/transpiler_mir_emission_contract.h"
...
grep -Fq "mir_instruction_is_with_slot_claim(inst)" "$ROOT_DIR/src/codegen/transpiler_mir_resource_op_emit.h"
...
grep -Fq "mir_instruction_source_is_local_decl(inst)" "$ROOT_DIR/src/codegen/transpiler_mir_resource_hook_emit.h"
...
grep -Fq "mir_instruction_source_is_local_destructure(inst)" "$ROOT_DIR/src/codegen/transpiler_mir_emission_mapping_contract.h"
grep -Fq "mir_instruction_source_is_assignment(inst)" "$ROOT_DIR/src/codegen/transpiler_mir_emission_mapping_contract.h"
...
grep -Fq "strcmp(inner_buf, \"Unknown\") != 0" "$ROOT_DIR/src/codegen/transpiler_expr_stdlib_builtin.h"
...
grep -Fq "transpiler_infer_lambda_param_c_type_copy" "$ROOT_DIR/src/codegen/transpiler.c"
grep -Fq "pergyra_ast_type_to_c_copy(node->data.lambda_expr.return_type" "$ROOT_DIR/src/codegen/transpiler.c"
grep -Fq "pergyra_ast_type_to_c_copy(param->data.let_decl.type" "$ROOT_DIR/src/codegen/transpiler.c"
...
grep -Fq "transpiler_require_type_name_c_type_copy(" "$ROOT_DIR/src/codegen/transpiler_async_parallel_emit.h"
grep -Fq "transpiler_require_ast_c_type_copy(" "$ROOT_DIR/src/codegen/transpiler_domain_role_methods_emit.h"
grep -Fq "transpiler_require_ast_c_type_copy(" "$ROOT_DIR/src/codegen/transpiler_class_decl_emit.h"
grep -Fq "transpiler_require_ast_c_type_copy(" "$ROOT_DIR/src/codegen/transpiler_domain_nominal_emit.h"
grep -Fq "transpiler_require_ast_c_type_copy(" "$ROOT_DIR/src/codegen/transpiler_enum_decl_emit.h"
grep -Fq "transpiler_require_ast_c_type_copy(" "$ROOT_DIR/src/codegen/transpiler_generic_class_specialization_emit.h"
grep -Fq "transpiler_require_ast_c_type_copy(" "$ROOT_DIR/src/codegen/transpiler_relation_effect_emit.h"
...
grep -Fq "transpiler_infer_slot_inner_type_name" \
    "$ROOT_DIR/src/codegen/transpiler_expr_type_infer.h" || {
