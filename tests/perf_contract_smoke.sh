...
grep -Fq "compiler_runtime_dir_has_newer_source" "$ROOT_DIR/src/compiler/compiler_runtime_cache.c"
grep -Fq "compiler_runtime_dir_has_newer_source(PGY_RUNTIME_DIR, cache_mtime)" "$ROOT_DIR/src/compiler/compiler_runtime_cache.c"
...
grep -Fq "type_ann = !mir_active && mir_instruction_uses_source_local_decl_emit(inst)" "$ROOT_DIR/src/codegen/llvm_mir_source_def_copy.c"
grep -Fq "llvm_mir_source_local_type_fact(ctx, base_name)" "$ROOT_DIR/src/codegen/llvm_mir_source_def_copy.c"
...
! grep -Fq "source_ast;" "$ROOT_DIR/src/compiler/mir_program.h" \
    "$ROOT_DIR/src/compiler/mir_types.h"
! grep -Fq "source_terminator_condition" "$ROOT_DIR/src/compiler/mir_types.h"
! grep -Fq "source_terminator_value" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "source_terminator_kind" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "source_terminator_has_value" "$ROOT_DIR/src/compiler/mir_types.h"
! grep -Fq "block->source_ast" "$ROOT_DIR/src/compiler/mir.c"
grep -Fq "MIRStatementInventory" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "used_non_cfg_body_fallback" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "non_cfg_body_fallback_count" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "has_non_cfg_body_fallback_inventory" "$ROOT_DIR/src/compiler/mir_program.h"
grep -Fq "non_cfg_body_fallback_total" "$ROOT_DIR/src/compiler/mir_program.h"
...
grep -Fq "transpiler_slot_runtime_row_for_source_operation(" "$ROOT_DIR/src/codegen/transpiler_slot_builtin_emit.c"
...
grep -Fq "generic_args_to_c_suffix_copy" "$ROOT_DIR/src/codegen/transpiler_type_name_utils.c"
grep -Fq "generic_args_to_c_suffix_write" "$ROOT_DIR/src/codegen/transpiler_type_name_utils.c"
grep -Fq "bool generic_args_to_c_suffix_copy" "$ROOT_DIR/src/codegen/codegen_type_mapping.h"
...
! grep -Fq "source_statements;" "$ROOT_DIR/src/compiler/mir_types.h"
! grep -Fq "source_statement_count;" "$ROOT_DIR/src/compiler/mir_types.h"
...
grep -Fq "requires_channel_receive_statement_emit" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "requires_select_receive_statement_emit" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "is_select_case_body" "$ROOT_DIR/src/compiler/hir.h"
grep -Fq "is_select_case_body" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "requires_source_branch_emit" "$ROOT_DIR/src/compiler/mir_types.h"
...
grep -Fq "mir_instruction_has_source_payload" "$ROOT_DIR/src/compiler/mir_source_provenance.c"
...
grep -Fq "mir_instruction_source_terminator_matches" "$ROOT_DIR/src/compiler/mir_source_provenance.c"
...
grep -Fq "mir_instruction_capture_source_provenance" "$ROOT_DIR/src/compiler/mir_source_provenance.c"
...
grep -Fq "inventory_uses_intent_observability_surface" "$ROOT_DIR/src/compiler/mir_program.h"
...
grep -Fq "has_source_statement_index" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "has_surface_usage_facts" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "uses_thread_pool_surface" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "uses_intent_observability_surface" "$ROOT_DIR/src/compiler/mir_types.h"
...
grep -Fq "MIRRoutineInventory" "$ROOT_DIR/src/compiler/mir_types.h"
...
grep -Fq "inst->resource_owner_slot_anchor" "$ROOT_DIR/src/codegen/transpiler_mir_resource_hook_emit.c"
grep -Fq "const MIRTypeLayout *source_layout = inst->type_layout" "$ROOT_DIR/src/codegen/transpiler_mir_resource_hook_emit.c"
! grep -Fq "transpiler_mir_def_type_annotation" "$ROOT_DIR/src/codegen/transpiler_mir_resource_hook_emit.c"
...
grep -Fq "pgy_intent_observability_abi_row_for_carried_identity" "$ROOT_DIR/src/codegen/transpiler_intent_observability_builtin_emit.c"
...
grep -Fq "transpiler_option_suffix_from_type_name(opt_type" "$ROOT_DIR/src/codegen/transpiler_call_result_option_builtin_emit.c"
! grep -Fq "slot_inner_type_name_copy(opt_type" "$ROOT_DIR/src/codegen/transpiler_call_result_option_builtin_emit.c"
grep -Fq "transpiler_contextual_option_inner_type_copy(ctx" "$ROOT_DIR/src/codegen/transpiler_expr_type_infer.c"
...
grep -Fq "bool llvm_require_secure_token_var(LLVMGenCtx *ctx" "$ROOT_DIR/src/codegen/llvm_internal_emit_api.h"
! grep -Fq "LLVMVarEntry *llvm_lookup_secure_token_var" "$ROOT_DIR/src/codegen/llvm_internal_api.h"
! grep -Fq "LLVMVarEntry *llvm_require_secure_token_var" "$ROOT_DIR/src/codegen/llvm_internal_emit_api.h"
...
grep -Fq "llvm_slot_runtime_row_for_operation(" "$ROOT_DIR/src/codegen/llvm_expr_identifier_slot_helpers.c"
...
grep -Fq "llvm_slot_runtime_row_for_operation(" "$ROOT_DIR/src/codegen/llvm_expr_assignment_member_projection.c"
...
grep -Fq "requires concrete Array<T> receiver metadata" "$ROOT_DIR/src/codegen/llvm_expr_assignment_member_projection.c"
...
grep -Fq "llvm_slot_runtime_row_for_operation(" "$ROOT_DIR/src/codegen/llvm_expr_slot_device_calls.c"
...
grep -Fq "llvm_slot_runtime_row_for_operation(" "$ROOT_DIR/src/codegen/llvm_stmt_with.c"
...
grep -Fq "llvm_slot_runtime_row_for_operation(" "$ROOT_DIR/src/codegen/llvm_stmt_let_resources.c"
...
grep -Fq "llvm_slot_runtime_row_for_operation(" "$ROOT_DIR/src/codegen/llvm_stmt_block.c"
...
grep -Fq "llvm_slot_runtime_row_for_operation(" "$ROOT_DIR/src/codegen/llvm_mir_pin_region.c"
...
grep -Fq "instruction_capacity" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "block_capacity" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "routine_capacity" "$ROOT_DIR/src/compiler/mir_program.h"
grep -Fq "predecessor_capacity" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "decl_header_capacity" "$ROOT_DIR/src/compiler/mir_program.h"
grep -Fq "value_summary_capacity" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "use_capacity" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "renamed_local_capacity" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "ssa_entry_value_capacity" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "ssa_exit_value_capacity" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "live_in_name_capacity" "$ROOT_DIR/src/compiler/mir_types.h"
grep -Fq "live_out_name_capacity" "$ROOT_DIR/src/compiler/mir_types.h"
