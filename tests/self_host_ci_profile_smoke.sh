...
require_job_timeout "platform-full-linux-self-host-parity" 90 "$PLATFORM_WORKFLOW"
require_job_timeout "platform-full-macos-c-only" 100 "$PLATFORM_WORKFLOW"
...
if [[ "$(grep -Ec "^[[:space:]]*run '" "$PUSH_LINUX_STEPS")" != "44" ]]; then
    echo "[self-host-ci-profile] Linux push shard inventory must retain 43 prebuilt gates plus the build-mode compiler step" >&2
    exit 1
fi
