...
if [[ "$(grep -Ec "^[[:space:]]*run '" "$PUSH_LINUX_STEPS")" != "33" ]]; then
    echo "[self-host-ci-profile] Linux push shard inventory must retain 32 prebuilt gates plus the build-mode compiler step" >&2
    exit 1
fi
