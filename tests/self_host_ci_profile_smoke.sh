...
if [[ "$(grep -Ec "^[[:space:]]*run '" "$PUSH_LINUX_STEPS")" != "56" ]]; then
    echo "[self-host-ci-profile] Linux push shard inventory must retain 55 prebuilt gates plus the build-mode compiler step" >&2
    exit 1
fi
