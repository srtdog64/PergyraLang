...
if [[ "$(grep -Ec "^[[:space:]]*run '" "$PUSH_LINUX_STEPS")" != "41" ]]; then
    echo "[self-host-ci-profile] Linux push shard inventory must retain 40 prebuilt gates plus the build-mode compiler step" >&2
    exit 1
fi
