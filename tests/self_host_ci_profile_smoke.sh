...
if [[ "$(grep -Ec "^[[:space:]]*run '" "$PUSH_LINUX_STEPS")" != "36" ]]; then
    echo "[self-host-ci-profile] Linux push shard inventory must retain 35 prebuilt gates plus the build-mode compiler step" >&2
    exit 1
fi
