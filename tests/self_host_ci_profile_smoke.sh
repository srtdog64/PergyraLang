...
if [[ "$(grep -Ec "^[[:space:]]*run '" "$PUSH_LINUX_STEPS")" != "60" ]]; then
    echo "[self-host-ci-profile] Linux push shard inventory must retain 59 prebuilt gates plus the build-mode compiler step" >&2
    exit 1
fi
