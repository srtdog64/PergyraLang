...
if [[ "${#harness_paths[@]}" -ne 5 ]]; then
    echo "[self-host-parity:air-id-uniqueness] TestHarness manifest expected 5 paths, got ${#harness_paths[@]}" >&2
    exit 1
fi
...
if ! (cd "$ROOT_DIR" && "$PGY" "$PERGYRA_TOOL_ARG" --backend=c \
    ...
    echo "[self-host-parity:air-id-uniqueness] C backend compile failed" >&2
    cat "$CLEAN_COMPILE_LOG" >&2
    exit 1
fi
if ! pgy_require_runnable_binary_here "self-host-parity:air-id-uniqueness" "$CLEAN_BIN"; then
    exit 1
fi
