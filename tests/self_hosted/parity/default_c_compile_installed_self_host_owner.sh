...
(cd "$ROOT_DIR" && unset PGY_SELF_DRIVER_BIN &&
    "$PGY" "$MEMBER_SOURCE" --backend=c \
        -o ".tmp/self_hosted/default_c_compile_installed/member-program$suffix") \
...
cmp -s "$WORK_DIR/member.expected" "$WORK_DIR/member-program.out" ||
    fail "installed self-host C artifact lost inferred generic member flow"
