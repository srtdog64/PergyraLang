...
grep -Fq 'pgy.selfhost.direct-mir-scalar-cfg-graph-plan.v78' "$PLAN" || fail "Bool inequality did not advance the GraphPlan schema"
...
grep -Fq '" = add i64 "' "$LLVM_OWNER" || fail "LLVM wrap addition is missing"
grep -Fq '" = sub i64 "' "$LLVM_OWNER" || fail "LLVM wrap subtraction is missing"
grep -Fq '" = sub i64 0, "' "$LLVM_OWNER" || fail "LLVM wrap negation is missing"
grep -Fq '%negated = sub i64 0, %value' "$LLVM_MATH" || fail "LLVM wrap Abs is missing"
...
for backend in c llvm; do
    ...
    if [[ "$backend" == c ]]; then
        ...
        grep -Fq 'add i64' "$artifact" || fail "LLVM artifact omitted wrap addition"
        ! grep -Fq 'add nsw i64' "$artifact" || fail "LLVM artifact marked wrap addition nsw"
        ! grep -Fq 'sub nsw i64' "$artifact" || fail "LLVM artifact marked wrap subtraction nsw"
...
for mutation in mixed-type bool-mixed-type; do
