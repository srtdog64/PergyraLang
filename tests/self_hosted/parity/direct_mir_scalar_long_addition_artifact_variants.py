...
if backend == "c":
    ...
    def llvm_call(left, right):
        return (
            "  %pgy.long.addition = call i64 @pgy.scalar.routine.1("
            f"i64 {left}, i64 {right})\n"
            "  call i32 (ptr, ...) @printf(ptr getelementptr inbounds "
            "([6 x i8], ptr @.pgy.scalar.cfg.int.format, i64 0, i64 0), "
            "i64 %pgy.long.addition)"
        )
    values = {
        "ordinary": llvm_call("40", "2"),
        "overflow": llvm_call("9223372036854775807", "1"),
    }
else:
