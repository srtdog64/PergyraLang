...
if backend == "c":
    pattern = r'printf\("%s\\n", "long-subtraction-ready"\);'
    values = {
        "ordinary":
            'printf("%lld\\n", pgy_scalar_routine_1(7LL, 5LL));',
        "overflow":
            'printf("%lld\\n", pgy_scalar_routine_1('
            '(-9223372036854775807LL - 1LL), 1LL));',
    }
elif backend == "llvm":
    pattern = (
    ...
    def llvm_call(left, right):
        return (
            "  %pgy.long.subtraction = call i64 @pgy.scalar.routine.1("
            f"i64 {left}, i64 {right})\n"
            "  call i32 (ptr, ...) @printf(ptr getelementptr inbounds "
            "([6 x i8], ptr @.pgy.scalar.cfg.int.format, i64 0, i64 0), "
            "i64 %pgy.long.subtraction)"
        )

    values = {
        "ordinary": llvm_call("7", "5"),
        "overflow": llvm_call("-9223372036854775808", "1"),
    }
else:
