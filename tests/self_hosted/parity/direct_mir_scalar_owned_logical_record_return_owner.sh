...
WORK_REL=".tmp/self_hosted/direct_mir_scalar_owned_logical_record_return"
WORK_DIR="$ROOT_DIR/$WORK_REL"
SOURCE_REL="tests/self_hosted/fixtures/direct_mir_owned_logical_record_return.pgy"
MIR_REL="$WORK_REL/program.mir.json"; MIR="$ROOT_DIR/$MIR_REL"
POLICY="$ROOT_DIR/src/self_hosted/compiler/direct_mir_scalar_program_owned_logical_record_return_policy_owner.pgy"
...
LLVM_CALL_OWNER="$ROOT_DIR/src/self_hosted/compiler/direct_mir_scalar_program_llvm_direct_call_expression_owner.pgy"
...
for backend in c llvm; do
    ...
    if [[ "$backend" == c ]]; then
        ...
        grep -Eq 'pgy_scalar_routine_1\(pgy_local_[0-9]+\)' "$artifact" ||
        ...
        grep -Eq 'call %pgy\.scalar\.logical\.record\.value\.[0-9]+ @pgy\.scalar\.routine\.1\(%pgy\.scalar\.logical\.record\.value\.[0-9]+ ' "$artifact" ||
