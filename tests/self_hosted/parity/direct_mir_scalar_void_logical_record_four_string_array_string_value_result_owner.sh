#!/usr/bin/env bash
# Exact Void record/four-String value plus ArrayString copyout C/LLVM boundary.
set -euo pipefail
...
LABEL="self-host-direct-mir-scalar-void-record-four-string-array-copyout"
DRIVER="$(pgy_select_optional_exe_binary "${PGY_SELF_DRIVER_BIN:-$ROOT_DIR/bin/pgy-self-driver}")"
CC="${PGY_SELFHOST_CC:-gcc}"; CLANG="${PGY_SELFHOST_CLANG:-clang}"
WORK_REL=".tmp/self_hosted/direct_mir_scalar_void_record_four_string_array_copyout"
WORK_DIR="$ROOT_DIR/$WORK_REL"
SOURCE_REL="tests/self_hosted/fixtures/direct_mir_void_logical_record_four_string_array_string_value_result.pgy"
...
MUTATIONS="$ROOT_DIR/tests/self_hosted/parity/direct_mir_scalar_void_logical_record_four_string_array_string_value_result_mutations.py"
...
grep -Fq 'let ordinal: Int = 2;' "$POLICY" || fail "policy omitted String-tail verification"
...
grep -Fq '"name":"AppendTypedBinding"' "$MIR" || fail "producer omitted callable"
grep -Fq '"type":"Array<String>","carriage":"value-result"' "$MIR" || fail "producer omitted ArrayString copyout"
[[ "$(grep -Fo '"type":"String","carriage":"value"' "$MIR" | wc -l)" -ge 4 ]] || fail "producer omitted four String values"
printf 'void-record-four-string-array-copyout-ready\n1\n' >"$WORK_DIR/expected.run"
...
for backend in c llvm; do
    ...
    if [[ "$backend" == c ]]; then
        ...
        [[ "$(grep -Fo 'const char* pgy_param_' <<<"$signature" | wc -l)" -eq 4 ]] || fail "C omitted four String values"
        grep -Fq 'pgy_as pgy_param_0 = *pgy_param_0_mutref;' "$artifact" || fail "C omitted copy-in"
        grep -Fq '*pgy_param_0_mutref = pgy_param_0;' "$artifact" || fail "C omitted copyout"
        ...
        grep -Eq 'define internal void @pgy\.scalar\.routine\.[0-9]+\(ptr %pgy\.param\.0\.mutref, %pgy\.scalar\.logical\.record\.value\.[0-9]+ %pgy\.param\.1, ptr %pgy\.param\.2, ptr %pgy\.param\.3, ptr %pgy\.param\.4, ptr %pgy\.param\.5\)' "$artifact" || fail "LLVM omitted exact signature"
        grep -Fq '%pgy.param.0.local = alloca %pgy.array.string' "$artifact" || fail "LLVM omitted copy-in"
        grep -Fq '%pgy.param.0.copyout.' "$artifact" || fail "LLVM omitted copyout"
...
for mutation in copyout-carriage copyout-type copyout-pass copyout-abi copyout-layout record-type record-carriage record-abi unknown-scalar-type string-carriage unknown-return-type parameter-count; do
...
echo "[$LABEL] Void record/four-String values + ArrayString copyout parity/negatives: PASS"
