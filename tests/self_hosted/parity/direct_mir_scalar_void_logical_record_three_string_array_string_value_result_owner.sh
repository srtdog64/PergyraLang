#!/usr/bin/env bash
# Exact Void record/three-String value plus ArrayString copyout C/LLVM boundary.
set -euo pipefail
...
LABEL="self-host-direct-mir-scalar-void-record-three-string-array-copyout"
DRIVER="$(pgy_select_optional_exe_binary "${PGY_SELF_DRIVER_BIN:-$ROOT_DIR/bin/pgy-self-driver}")"
CC="${PGY_SELFHOST_CC:-gcc}"; CLANG="${PGY_SELFHOST_CLANG:-clang}"
WORK_REL=".tmp/self_hosted/direct_mir_scalar_void_record_three_string_array_copyout"
WORK_DIR="$ROOT_DIR/$WORK_REL"
SOURCE_REL="tests/self_hosted/fixtures/direct_mir_void_logical_record_three_string_array_string_value_result.pgy"
...
MUTATIONS="$ROOT_DIR/tests/self_hosted/parity/direct_mir_scalar_void_logical_record_three_string_array_string_value_result_mutations.py"
...
grep -Fq 'let ordinal: Int = 2;' "$POLICY" || fail "policy omitted exact String tail"
...
grep -Fq '"name":"AppendBinding"' "$MIR" || fail "producer omitted callable"
grep -Fq '"type":"Array<String>","carriage":"value-result"' "$MIR" || fail "producer omitted ArrayString copyout"
[[ "$(grep -Fo '"type":"String","carriage":"value"' "$MIR" | wc -l)" -ge 3 ]] || fail "producer omitted three String values"
printf 'void-record-three-string-array-copyout-ready\n1\n' >"$WORK_DIR/expected.run"
...
for backend in c llvm; do
    ...
    if [[ "$backend" == c ]]; then
        ...
        [[ "$(grep -Fo 'const char* pgy_param_' <<<"$signature" | wc -l)" -eq 3 ]] || fail "C omitted three String values"
        ...
        grep -Eq 'define internal void @pgy\.scalar\.routine\.[0-9]+\(ptr %pgy\.param\.0\.mutref, %pgy\.scalar\.logical\.record\.value\.[0-9]+ %pgy\.param\.1, ptr %pgy\.param\.2, ptr %pgy\.param\.3, ptr %pgy\.param\.4\)' "$artifact" || fail "LLVM omitted exact signature"
...
echo "[$LABEL] Void record/three-String values + ArrayString copyout parity/negatives: PASS"
