...
pgy_selfhost_verify_driver_rung2_abi_layout_negative() {
    ...
    local missing_row identity_row needle replacement static_id
    ...
    # The producer must now carry a required static row.  The
    # mutation removes the required-field key itself; this must fail before
    # any backend projection can run.  Do not mutate a dynamic tuple here: it
    # would leave the producer-side static-row rung untested.
    ...
    if (cd "$ROOT_DIR" && "${consumer_command[@]}" \
        ...
        echo "[self-host-parity:driver-rung2] $backend missing ABI-layout tuple was accepted: $base" >&2
        return 1
    fi
    grep -Fq \
    ...
    if (cd "$ROOT_DIR" && "${consumer_command[@]}" \
        ...
        echo "[self-host-parity:driver-rung2] $backend wrong ABI-layout identity was accepted: $base" >&2
        return 1
    fi
    grep -Fq \
        "instruction is missing or carries an invalid MIR ABI layout fact" \
