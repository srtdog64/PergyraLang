...
pgy_selfhost_run_driver_rung2_mir_producer_parity() {
    ...
    local actual err missing_graph invalid_graph self_actual source_actual mir_baseline bare_call_missing_graph indexed_missing_target_graph
    for fixture_rel in "${mir_fixture_rows[@]}"; do
        base="$(basename "$fixture_rel" .pgy)"
        mir_json="$BUILD_DIR/${base}.mir.json"
        ...
        if [[ "$base" == "indexed_assignment" ]]; then
            ...
            indexed_missing_target_graph="$BUILD_DIR/${base}_${backend}.missing-target-graph.mir.json"
            sed 's/"expr1_graph"/"expr1_graph_removed"/g' \
                "$self_mir_json" >"$indexed_missing_target_graph"
            if (cd "$ROOT_DIR" && "$driver_bin" --mir-json \
                "$(pgy_selfhost_path_relative_to_root "$indexed_missing_target_graph")" \
                >"$indexed_missing_target_graph.out" \
                2>"$indexed_missing_target_graph.err"); then
            ...
            grep -Fq "MIR instruction expression graph is missing or invalid" \
                "$indexed_missing_target_graph.err" \
                "$indexed_missing_target_graph.out" || {
