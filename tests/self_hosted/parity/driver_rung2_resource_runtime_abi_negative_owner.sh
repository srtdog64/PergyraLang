...
pgy_selfhost_verify_driver_rung2_resource_runtime_abi_negative() {
    ...
    local marker_row markerless_row escaped_row duplicate_row escaped_duplicate_row
    local required_false_row required_kind_row missing_row identity_row
    local payload_row stray_row aux_identity_row aux_injected_row needle replacement
    ...
    marker_row="$self_mir_json"
    if ! grep -Fq '"runtime_call_abi_required":true' "$self_mir_json"; then
        marker_row="$BUILD_DIR/${base}_${backend}.marker-consumer-runtime-row.mir.json"
        needle='"runtime_call_abi":{"owner":"MIRResource","id":'
        replacement='"runtime_call_abi_required":true,"runtime_call_abi":{"owner":"MIRResource","id":'
        if ! pgy_replace_first_literal \
            "$self_mir_json" "$marker_row" "$needle" "$replacement"; then
            echo "[self-host-parity:driver-rung2] $backend consumer runtime marker mutation did not apply: $base" >&2
            return 1
        fi
    fi
    consumer_command=("$driver_bin" --mir-json \
        "$(pgy_selfhost_path_relative_to_root "$marker_row")")
    if [[ "$machine_fixture" -eq 1 ]]; then
        consumer_command+=("$machine_declaration")
    fi
    if ! (cd "$ROOT_DIR" && "${consumer_command[@]}" \
        >"$marker_row.out" 2>"$marker_row.err"); then
        echo "[self-host-parity:driver-rung2] $backend exact true consumer runtime marker was rejected: $base" >&2
        cat "$marker_row.out" "$marker_row.err" >&2
        return 1
    fi

    markerless_row="$self_mir_json"
    if grep -Fq '"runtime_call_abi_required":true,' "$self_mir_json"; then
        markerless_row="$BUILD_DIR/${base}_${backend}.markerless-consumer-runtime-row.mir.json"
        needle='"runtime_call_abi_required":true,'
        replacement=''
        if ! pgy_replace_first_literal \
            "$self_mir_json" "$markerless_row" "$needle" "$replacement"; then
            echo "[self-host-parity:driver-rung2] $backend markerless consumer runtime-row mutation did not apply: $base" >&2
            return 1
        fi
    fi
    consumer_command=("$driver_bin" --mir-json \
        "$(pgy_selfhost_path_relative_to_root "$markerless_row")")
    if [[ "$machine_fixture" -eq 1 ]]; then
        consumer_command+=("$machine_declaration")
    fi
    if ! (cd "$ROOT_DIR" && "${consumer_command[@]}" \
        >"$markerless_row.out" 2>"$markerless_row.err"); then
        echo "[self-host-parity:driver-rung2] $backend markerless canonical consumer runtime row was rejected: $base" >&2
        cat "$markerless_row.out" "$markerless_row.err" >&2
        return 1
    fi

    escaped_row="$BUILD_DIR/${base}_${backend}.escaped-consumer-runtime-row.mir.json"
    needle='"runtime_call_abi":{"owner":"MIRResource","id":'
    replacement='"runtime_call_ab\\i":{"owner":"MIRResource","id":'
    if ! pgy_replace_first_literal \
        "$self_mir_json" "$escaped_row" "$needle" "$replacement"; then
        echo "[self-host-parity:driver-rung2] $backend escaped consumer runtime-key mutation did not apply: $base" >&2
        return 1
    fi
    consumer_command=("$driver_bin" --mir-json \
        "$(pgy_selfhost_path_relative_to_root "$escaped_row")")
    if [[ "$machine_fixture" -eq 1 ]]; then
        consumer_command+=("$machine_declaration")
    fi
    if ! (cd "$ROOT_DIR" && "${consumer_command[@]}" \
        >"$escaped_row.out" 2>"$escaped_row.err"); then
        echo "[self-host-parity:driver-rung2] $backend supported escaped consumer runtime key was rejected: $base" >&2
        cat "$escaped_row.out" "$escaped_row.err" >&2
        return 1
    fi

    duplicate_row="$BUILD_DIR/${base}_${backend}.duplicate-consumer-runtime-row.mir.json"
    needle='"runtime_call_abi":{"owner":"MIRResource","id":'
    replacement='"runtime_call_abi":null,"runtime_call_abi":{"owner":"MIRResource","id":'
    if ! pgy_replace_first_literal \
        "$self_mir_json" "$duplicate_row" "$needle" "$replacement"; then
        echo "[self-host-parity:driver-rung2] $backend duplicate consumer runtime-row mutation did not apply: $base" >&2
        return 1
    fi
    consumer_command=("$driver_bin" --mir-json \
        "$(pgy_selfhost_path_relative_to_root "$duplicate_row")")
    if [[ "$machine_fixture" -eq 1 ]]; then
        consumer_command+=("$machine_declaration")
    fi
    if (cd "$ROOT_DIR" && "${consumer_command[@]}" \
        >"$duplicate_row.out" 2>"$duplicate_row.err"); then
        echo "[self-host-parity:driver-rung2] $backend duplicate consumer runtime row was accepted: $base" >&2
        return 1
    fi
    grep -Fq \
        "resource instruction or consumer is missing its lowered runtime-call ABI row" \
        "$duplicate_row.err" "$duplicate_row.out" || {
        echo "[self-host-parity:driver-rung2] $backend duplicate consumer runtime-row diagnostic drifted: $base" >&2
        cat "$duplicate_row.out" "$duplicate_row.err" >&2
        return 1
    }

    escaped_duplicate_row="$BUILD_DIR/${base}_${backend}.escaped-duplicate-consumer-runtime-row.mir.json"
    needle='"runtime_call_abi":{"owner":"MIRResource","id":'
    replacement='"runtime_call_abi":null,"runtime_call_ab\\i":{"owner":"MIRResource","id":'
    if ! pgy_replace_first_literal \
        "$self_mir_json" "$escaped_duplicate_row" "$needle" "$replacement"; then
        echo "[self-host-parity:driver-rung2] $backend escaped duplicate consumer runtime-row mutation did not apply: $base" >&2
        return 1
    fi
    consumer_command=("$driver_bin" --mir-json \
        "$(pgy_selfhost_path_relative_to_root "$escaped_duplicate_row")")
    if [[ "$machine_fixture" -eq 1 ]]; then
        consumer_command+=("$machine_declaration")
    fi
    if (cd "$ROOT_DIR" && "${consumer_command[@]}" \
        >"$escaped_duplicate_row.out" 2>"$escaped_duplicate_row.err"); then
        echo "[self-host-parity:driver-rung2] $backend plain/escaped duplicate consumer runtime row was accepted: $base" >&2
        return 1
    fi
    grep -Fq \
        "resource instruction or consumer is missing its lowered runtime-call ABI row" \
        "$escaped_duplicate_row.err" "$escaped_duplicate_row.out" || {
        echo "[self-host-parity:driver-rung2] $backend plain/escaped duplicate consumer runtime-row diagnostic drifted: $base" >&2
        cat "$escaped_duplicate_row.out" "$escaped_duplicate_row.err" >&2
        return 1
    }

    required_false_row="$BUILD_DIR/${base}_${backend}.false-consumer-runtime-marker.mir.json"
    needle='"runtime_call_abi_required":true'
    replacement='"runtime_call_abi_required":false'
    if ! grep -Fq "$needle" "$self_mir_json"; then
        needle='"runtime_call_abi":{"owner":"MIRResource","id":'
        replacement='"runtime_call_abi_required":false,"runtime_call_abi":{"owner":"MIRResource","id":'
    fi
    if ! pgy_replace_first_literal \
        "$self_mir_json" "$required_false_row" "$needle" "$replacement"; then
        echo "[self-host-parity:driver-rung2] $backend false consumer runtime-marker mutation did not apply: $base" >&2
        return 1
    fi
    consumer_command=("$driver_bin" --mir-json \
        "$(pgy_selfhost_path_relative_to_root "$required_false_row")")
    if [[ "$machine_fixture" -eq 1 ]]; then
        consumer_command+=("$machine_declaration")
    fi
    if (cd "$ROOT_DIR" && "${consumer_command[@]}" \
        >"$required_false_row.out" 2>"$required_false_row.err"); then
        echo "[self-host-parity:driver-rung2] $backend false consumer runtime marker was accepted: $base" >&2
        return 1
    fi
    grep -Fq \
        "resource instruction or consumer is missing its lowered runtime-call ABI row" \
        "$required_false_row.err" "$required_false_row.out" || {
        echo "[self-host-parity:driver-rung2] $backend false consumer runtime-marker diagnostic drifted: $base" >&2
        cat "$required_false_row.out" "$required_false_row.err" >&2
        return 1
    }

    required_kind_row="$BUILD_DIR/${base}_${backend}.wrong-kind-consumer-runtime-marker.mir.json"
    needle='"runtime_call_abi_required":true'
    replacement='"runtime_call_abi_required":null'
    if ! grep -Fq "$needle" "$self_mir_json"; then
        needle='"runtime_call_abi":{"owner":"MIRResource","id":'
        replacement='"runtime_call_abi_required":null,"runtime_call_abi":{"owner":"MIRResource","id":'
    fi
    if ! pgy_replace_first_literal \
        "$self_mir_json" "$required_kind_row" "$needle" "$replacement"; then
        echo "[self-host-parity:driver-rung2] $backend wrong-kind consumer runtime-marker mutation did not apply: $base" >&2
        return 1
    fi
    consumer_command=("$driver_bin" --mir-json \
        "$(pgy_selfhost_path_relative_to_root "$required_kind_row")")
    if [[ "$machine_fixture" -eq 1 ]]; then
        consumer_command+=("$machine_declaration")
    fi
    if (cd "$ROOT_DIR" && "${consumer_command[@]}" \
        >"$required_kind_row.out" 2>"$required_kind_row.err"); then
        echo "[self-host-parity:driver-rung2] $backend wrong-kind consumer runtime marker was accepted: $base" >&2
        return 1
    fi
    grep -Fq \
        "resource instruction or consumer is missing its lowered runtime-call ABI row" \
        "$required_kind_row.err" "$required_kind_row.out" || {
        echo "[self-host-parity:driver-rung2] $backend wrong-kind consumer runtime-marker diagnostic drifted: $base" >&2
        cat "$required_kind_row.out" "$required_kind_row.err" >&2
        return 1
    }
