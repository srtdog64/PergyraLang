...
src/self_hosted/compiler/direct_mir_scalar_program_expression_admission_owner.pgy|340
EOF
...
CONSUME="src/self_hosted/compiler/direct_mir_scalar_program_expression_admission_owner.pgy"
...
require_text "$CONSUME" 'arena.identities.call_target_syntax_ids[1]'
require_text "$CONSUME" 'SemanticExpressionBindingFormalParameter()'
reject_text "$CONSUME" 'sequence.arena.node_texts[node] == parameter_name'

for owner in "$FACT" "$RESOLVE" "$WRITE" "$READ" "$CONSUME"; do
