...
pgy_require_runnable_binary_here generic-bound-facts "$PGY"
cd "$ROOT_DIR"
mkdir -p .tmp/self_hosted
WORK="$(mktemp -d .tmp/self_hosted/generic-bound-facts.XXXXXX)"
echo "[generic-bound-facts] evidence: $WORK"
SOURCE=tests/self_hosted/fixtures/generic_constraint_admission_fact.pgy
...
for ((i=0; i<15; i++)); do printf 'true\n'; done >"$WORK/expected"
[[ ! -s "$WORK/err" ]]
cmp "$WORK/expected" "$WORK/actual"
echo '[generic-bound-facts] 15 binding/role/diagnostic/nominal-equality controls PASS'
