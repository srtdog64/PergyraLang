#!/usr/bin/env bash
# Real-owner controls; not whole-source/MIR admission or full bootstrap.
set -euo pipefail
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
source "$ROOT_DIR/tests/pgy_binary_path_helpers.sh"
pgy_prepend_windows_runtime_paths
LABEL=self-host-record-constructor-query
...
"$PYTHON" "$ROOT_DIR/tests/self_hosted/parity/logical_record_constructor_query_order.py" "$OWNER"
mkdir -p "$ROOT_DIR/.tmp/self_hosted"
WORK="$(mktemp -d "$ROOT_DIR/.tmp/self_hosted/record-constructor-query.XXXXXX")"
REL="${WORK#"$ROOT_DIR/"}"
SOURCE=tests/self_hosted/fixtures/logical_record_constructor_query.pgy
...
for origin in native public; do
    for backend in c llvm; do
        ...
        echo "[$LABEL] $stem: 17 ordering/arity/identity/digest/ABI controls PASS"
    done
done
"$PYTHON" "$ROOT_DIR/tests/self_hosted/parity/logical_record_constructor_query_pressure.py" "$WORK/native-c.exe"
echo "[$LABEL] native/public C/LLVM constructor query parity: PASS"
