#!/usr/bin/env bash
# Fresh bounded ArrayReverse shares the one sealed Array<Int> GraphPlan.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
source "$ROOT_DIR/tests/pgy_binary_path_helpers.sh"
pgy_prepend_windows_runtime_paths
LABEL="self-host-one-mir-array-int-reverse"
DRIVER="${PGY_SELF_DRIVER_BIN:-$ROOT_DIR/bin/pgy-self-driver}"
CC="${CC:-gcc}"
CLANG="${PGY_SELFHOST_CLANG:-clang}"
PYTHON_BIN="${PYTHON_BIN:-$(command -v python3 || command -v python || true)}"
WORK_REL=".tmp/self_hosted/one_mir_array_int_reverse"
WORK_DIR="$ROOT_DIR/$WORK_REL"
SOURCE_REL="src/self_hosted/codegen/fixture/array_reverse.pgy"
...
for owner in \
    "$ROOT_DIR/src/self_hosted/compiler/direct_mir_scalar_cfg_array_int_reverse_c_operation_owner.pgy" \
    "$ROOT_DIR/src/self_hosted/compiler/direct_mir_scalar_cfg_array_int_reverse_llvm_operation_owner.pgy"; do
    reject_text "$owner" 'source_json'
    reject_text "$owner" 'JsonObjectFactTable'
    reject_text "$owner" 'MirExpressionGraphSequence'
    reject_text "$owner" 'array_reverse.pgy'
    reject_text "$owner" 'pgy_ai_reverse'
done
...
"$PYTHON_BIN" \
    "$ROOT_DIR/tests/self_hosted/parity/one_mir_array_int_reverse_mutations.py" \
...
goods=(program display-only renumbered alternate)
bads=(bad-array-root bad-array-overflow bad-result-source-type \
    bad-result-layout bad-result-value-alias bad-result-name-alias \
    bad-call-target bad-coherent-call bad-call-edge bad-self-use \
    bad-length-target bad-length-receiver bad-index-receiver bad-index-oob \
    bad-index-duplicate bad-extra-result-use bad-missing-log bad-unreachable \
    bad-successor bad-duplicate-reverse)
...
source "$ROOT_DIR/tests/self_hosted/parity/one_mir_array_int_reverse_artifact_contract.sh"
...
compile_run base '3\n2\n1\n3\n'
compile_run alternate '3\n6\n-5\n4\n'
echo "[$LABEL] fresh ArrayReverse executes exact C/LLVM parity"
