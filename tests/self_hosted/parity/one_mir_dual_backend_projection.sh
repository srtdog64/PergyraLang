#!/usr/bin/env bash
# One Pergyra-produced MIR artifact per fixture drives both direct backends.
...
assert_direct_owner_ratchet() {
    ...
    require_file "$DIRECT_ADMISSION_OWNER"
    ...
    # The direct owner may parse admitted MIR facts, but may not reopen the
    # MIR->tree->semantic/source bridge or an oracle canonicalizer.
    for term in \
...
run_positive_case() {
    ...
    mir_rel="$(pgy_selfhost_path_relative_to_root "$MIR_ARTIFACT")"
    # Exactly one Pergyra producer call owns this fixture's MIR. Both backend
...
echo "[$LABEL] hello + let_log one-MIR dual-backend gate ok"
