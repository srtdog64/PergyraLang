...
"$PYTHON_BIN" "$ROOT_DIR/tests/self_hosted/parity/one_mir_inferred_generic_member_mutations.py" "$MIR" "$NATIVE_MIR" compare "$WORK_DIR/self.canonical.json" "$WORK_DIR/native.canonical.json" || fail "native/self member semantic parity drifted"
...
pgy_selfhost_driver_rung2_canonicalize 0 "$DRIVER_BIN" --canonicalize-mir-json "$(root_relative "$VESSEL_MIR")" "$VESSEL_DIR/self.canonical.json"
pgy_selfhost_driver_rung2_canonicalize 0 "$DRIVER_BIN" --canonicalize-oracle-mir-json "$(root_relative "$VESSEL_NATIVE_MIR")" "$VESSEL_DIR/native.canonical.json"
"$PYTHON_BIN" "$ROOT_DIR/tests/self_hosted/parity/one_mir_inferred_generic_member_mutations.py" "$VESSEL_MIR" "$VESSEL_NATIVE_MIR" compare "$VESSEL_DIR/self.canonical.json" "$VESSEL_DIR/native.canonical.json" || fail "native/self vessel nominal semantics drifted"
