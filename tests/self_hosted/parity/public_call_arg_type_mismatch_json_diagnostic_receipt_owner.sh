...
! grep -q '[^[:space:]]' "$WORK_DIR/unadmitted.out" ||
    fail "unadmitted value-parameter verdict gained call admission"
...
echo "[self-host-public-call-arg-type-mismatch-json-diagnostic] exact assignability identity, thirteen semantic contexts, MIR/C/LLVM relay, and value-parameter exclusion: PASS"
