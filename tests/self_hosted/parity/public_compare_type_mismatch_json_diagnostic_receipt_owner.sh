#!/usr/bin/env bash
# Pergyra-owned comparison verdicts retain their three contexts while sharing
# one exact binary-operand public identity through MIR, C, and LLVM requests.
...
WORK_REL=".tmp/self_hosted/public_compare_type_mismatch_json_diagnostic"
...
for base in $COMPARE_FIXTURES; do
    ...
    [[ "$actual_text" == "$expected_text" ]] ||
        fail "$base changed its text code or comparison facts"
...
run_public_failure mir "$WORK_DIR/bad_compare_operand.expected.json" "" \
    --mir --error-format=json \
    src/self_hosted/semantic/fixture/bad_compare_operand.pgy
run_public_failure c "$WORK_DIR/bad_compare_assign.expected.json" \
    "$WORK_REL/invalid-c.bin" --error-format=json --backend=c \
    src/self_hosted/semantic/fixture/bad_compare_assign.pgy \
    -o "$WORK_REL/invalid-c.bin"
run_public_failure llvm "$WORK_DIR/bad_compare_condition.expected.json" \
    "$WORK_REL/invalid-llvm.bin" --error-format=json --backend=llvm \
    src/self_hosted/semantic/fixture/bad_compare_condition.pgy \
...
[[ "$logical_rc" -ne 0 && ! -s "$WORK_DIR/logical.err" ]] ||
    fail "shared-identity logical verdict changed its private channels"
for fact in \
    '"code":"PGY_SEM_BINOP_TYPE_MISMATCH"' \
    '"cause_ir":"semantic:binop:operand_types"' \
    '"fix_source":"align-operand-types-or-overload"'; do
...
grep -Fxq 'message-independent' "$WORK_DIR/probe.out" ||
    fail "message wording changed comparison diagnostic identity"

require_text "$DIAGNOSTIC_OWNER" 'if code == "compare_type_mismatch" {'
...
require_text "$CONTRACT_OWNER" \
    'compare_first.message != compare_second.message'
