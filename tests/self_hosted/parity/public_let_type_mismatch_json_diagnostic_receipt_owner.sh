...
! grep -q '[^[:space:]]' "$WORK_DIR/unadmitted.out" ||
    fail "unadmitted symbol verdict gained let admission"
...
echo "[self-host-public-let-type-mismatch-json-diagnostic] exact assignability identity, thirteen semantic contexts, MIR/C/LLVM relay, and undefined-symbol exclusion: PASS"
