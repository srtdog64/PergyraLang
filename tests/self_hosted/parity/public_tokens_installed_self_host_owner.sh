...
require_text "$LAUNCHER_OWNER" 'if (flags.dump_tokens) {'
require_text "$LAUNCHER_OWNER" 'driver_self_host_tokens_request_supported(&flags)'
require_text "$LAUNCHER_OWNER" 'driver_run_self_host_tokens(argv[0], flags.source_path)'
require_text "$SELECTION_OWNER" 'driver_self_host_tokens_request_supported('
require_text "$SIBLING_OWNER" 'args[0] = (char *)"--tokens";'
