...
expected_status_counts = Counter({"ACTIVE": 9, "BRIDGE": 6, "CLOSED": 6})
expected_pairs = {
...
def validate(rows: list[dict[str, str]]) -> None:
    if {row["owner_id"] for row in rows} != set(expected_pairs):
        fail("owner id set drifted from the accepted 21-row compiler spine")
...
print(
    "[sot-owner-spine] 21 owner rows locked "
    "(CLOSED=6 BRIDGE=6 ACTIVE=9); mutations rejected"
)
PY
